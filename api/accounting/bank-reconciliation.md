# Accounting: Bank Reconciliation API

## Canonical one-to-one API module specification

**API package:** `module-accounting-bank-reconciliation-api`  
**Matching domain module:** `accounting-bank-reconciliation`  
**Application:** Accounting  
**Source feature:** [Bank Reconciliation](../../features/accounting/bank-reconciliation.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Bank Reconciliation domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/bank-reconciliation` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/bank-reconciliation`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/bank-reconciliation?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/bank-reconciliation/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/bank-reconciliation` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/bank-reconciliation/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/bank-reconciliation/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/bank-reconciliation/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "suggested_exact_matches": "example-value",
  "transfers": "example-value",
  "grouped_receipts": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-bank-reconciliation`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-bank-reconciliation.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.bank.reconciliation.list`, `accounting.bank.reconciliation.get`, `accounting.bank.reconciliation.create`, `accounting.bank.reconciliation.update`, and `accounting.bank.reconciliation.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingBankReconciliationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `suggested_exact_matches`, `transfers`, `grouped_receipts`, `fees_interest`, `adjustments`, `statement_balance`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting bank reconciliation API
  version: 1.0.0
paths:
  /api/v1/accounting/bank-reconciliation:
    get:
      operationId: accounting.bank.reconciliation.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingBankReconciliationResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-bank-reconciliation
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingBankReconciliationResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-bank-reconciliation`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-bank-reconciliation`.
