# Accounting: Journal Approvals API

## Canonical one-to-one API module specification

**API package:** `module-accounting-journal-approvals-api`  
**Matching domain module:** `accounting-journal-approvals`  
**Application:** Accounting  
**Source feature:** [Journal Approvals](../../features/accounting/journal-approvals.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Journal Approvals domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Journal sources/types
- thresholds
- preparer/reviewer
- evidence
- rejection
...
- Use `/api/v1/accounting/journal-approvals` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/journal-approvals`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/journal-approvals?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/journal-approvals/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/journal-approvals` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/journal-approvals/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/journal-approvals/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/journal-approvals/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "journal_sources_types": "example-value",
  "thresholds": "example-value",
  "preparer_reviewer": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-journal-approvals`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-journal-approvals.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.journal.approvals.list`, `accounting.journal.approvals.get`, `accounting.journal.approvals.create`, `accounting.journal.approvals.update`, and `accounting.journal.approvals.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingJournalApprovalsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `journal_sources_types`, `thresholds`, `preparer_reviewer`, `evidence`, `rejection`, `posting`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting journal approvals API
  version: 1.0.0
paths:
  /api/v1/accounting/journal-approvals:
    get:
      operationId: accounting.journal.approvals.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingJournalApprovalsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-journal-approvals
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingJournalApprovalsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-journal-approvals`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-journal-approvals`.
