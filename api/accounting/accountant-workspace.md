# Accounting: Accountant Workspace API

## Canonical one-to-one API module specification

**API package:** `module-accounting-accountant-workspace-api`  
**Matching domain module:** `accounting-accountant-workspace`  
**Application:** Accounting  
**Source feature:** [Accountant Workspace](../../features/accounting/accountant-workspace.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Accountant Workspace domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Client/entity portfolio
- status
- deadlines
- alerts
- access
...
- Use `/api/v1/accounting/accountant-workspace` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/accountant-workspace`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/accountant-workspace?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/accountant-workspace/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/accountant-workspace` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/accountant-workspace/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/accountant-workspace/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/accountant-workspace/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-accountant-workspace`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-accountant-workspace.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.accountant.workspace.list`, `accounting.accountant.workspace.get`, `accounting.accountant.workspace.create`, `accounting.accountant.workspace.update`, and `accounting.accountant.workspace.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingAccountantWorkspaceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `client_entity_portfolio`, `status`, `deadlines`, `alerts`, `access`, `notes`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
info:
  ...
  version: 1.0.0
paths:
  /api/v1/accounting/accountant-workspace:
    get:
      operationId: accounting.accountant.workspace.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingAccountantWorkspaceResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-accountant-workspace
        attributes:
          type: object
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-accountant-workspace`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-accountant-workspace`.
