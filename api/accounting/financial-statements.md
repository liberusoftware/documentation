# Accounting: Financial Statements API

## Canonical one-to-one API module specification

**API package:** `module-accounting-financial-statements-api`  
**Matching domain module:** `accounting-financial-statements`  
**Application:** Accounting  
**Source feature:** [Financial Statements](../../features/accounting/financial-statements.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Financial Statements domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/financial-statements` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/financial-statements`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/financial-statements?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/financial-statements/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/financial-statements` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/financial-statements/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/financial-statements/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/financial-statements/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "profit_and_loss": "example-value",
  "balance_sheet": "example-value",
  "cash_flow": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-financial-statements`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-financial-statements.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.financial.statements.list`, `accounting.financial.statements.get`, `accounting.financial.statements.create`, `accounting.financial.statements.update`, and `accounting.financial.statements.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingFinancialStatementsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `profit_and_loss`, `balance_sheet`, `cash_flow`, `changes_in_equity`, `comparative_periods`, `dimensions`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting financial statements API
  version: 1.0.0
paths:
  /api/v1/accounting/financial-statements:
    get:
      operationId: accounting.financial.statements.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingFinancialStatementsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-financial-statements
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingFinancialStatementsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-financial-statements`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-financial-statements`.
