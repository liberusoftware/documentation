# Accounting: Deposits and Clearing API

## Canonical one-to-one API module specification

**API package:** `module-accounting-deposits-and-clearing-api`  
...
This optional API presentation package exposes approved HTTP operations for the Deposits and Clearing domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/deposits-and-clearing` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/deposits-and-clearing`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/deposits-and-clearing?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/deposits-and-clearing/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/deposits-and-clearing` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/deposits-and-clearing/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/deposits-and-clearing/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/deposits-and-clearing/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "undeposited_funds": "example-value",
  "grouped_deposits": "example-value",
  "payment_provider_clearing": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-deposits-and-clearing`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-deposits-and-clearing.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.deposits.and.clearing.list`, `accounting.deposits.and.clearing.get`, `accounting.deposits.and.clearing.create`, `accounting.deposits.and.clearing.update`, and `accounting.deposits.and.clearing.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingDepositsAndClearingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `undeposited_funds`, `grouped_deposits`, `payment_provider_clearing`, `fees`, `payouts`, `timing_differences`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting deposits and clearing API
  version: 1.0.0
paths:
  /api/v1/accounting/deposits-and-clearing:
    get:
      operationId: accounting.deposits.and.clearing.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingDepositsAndClearingResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-deposits-and-clearing
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingDepositsAndClearingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-deposits-and-clearing`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-deposits-and-clearing`.
