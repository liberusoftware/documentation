# Accounting: Anomaly Detection API

## Canonical one-to-one API module specification

**API package:** `module-accounting-anomaly-detection-api`  
**Matching domain module:** `accounting-anomaly-detection`  
**Application:** Accounting  
**Source feature:** [Anomaly Detection](../../features/accounting/anomaly-detection.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Anomaly Detection domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Duplicates
- unusual amounts/accounts/timing
- control breaches
- missing sequences
- fraud indicators
- review queues
...
- Use `/api/v1/accounting/anomaly-detection` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/anomaly-detection`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/anomaly-detection?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/anomaly-detection/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/anomaly-detection` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/anomaly-detection/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/anomaly-detection/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/anomaly-detection/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
```json
{
  ...
  "unusual_amounts_accounts_timing": "example-value",
  "control_breaches": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-anomaly-detection`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-anomaly-detection.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.anomaly.detection.list`, `accounting.anomaly.detection.get`, `accounting.anomaly.detection.create`, `accounting.anomaly.detection.update`, and `accounting.anomaly.detection.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingAnomalyDetectionResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `duplicates`, `unusual_amounts_accounts_timing`, `control_breaches`, `missing_sequences`, `fraud_indicators`, `review_queues`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting anomaly detection API
  version: 1.0.0
paths:
  /api/v1/accounting/anomaly-detection:
    get:
      operationId: accounting.anomaly.detection.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingAnomalyDetectionResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-anomaly-detection
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingAnomalyDetectionResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-anomaly-detection`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-anomaly-detection`.
