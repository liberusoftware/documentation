# Accounting: Three-Way Matching API

## Canonical one-to-one API module specification

**API package:** `module-accounting-three-way-matching-api`  
...
This optional API presentation package exposes approved HTTP operations for the Three-Way Matching domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/three-way-matching` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/three-way-matching`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/three-way-matching?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/three-way-matching/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/three-way-matching` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/three-way-matching/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/three-way-matching/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/three-way-matching/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "po_receipt_bill_matching": "example-value",
  "quantity_price_tax_tolerances": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-three-way-matching`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-three-way-matching.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.three.way.matching.list`, `accounting.three.way.matching.get`, `accounting.three.way.matching.create`, `accounting.three.way.matching.update`, and `accounting.three.way.matching.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingThreeWayMatchingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `po_receipt_bill_matching`, `quantity_price_tax_tolerances`, `exceptions`, `approval`, `partial_matching`, `evidence`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting three way matching API
  version: 1.0.0
paths:
...
components:
  ...
  schemas:
    AccountingThreeWayMatchingResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-three-way-matching
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingThreeWayMatchingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-three-way-matching`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-three-way-matching`.
