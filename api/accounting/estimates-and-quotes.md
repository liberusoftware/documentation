# Accounting: Estimates and Quotes API

## Canonical one-to-one API module specification

**API package:** `module-accounting-estimates-and-quotes-api`  
**Matching domain module:** `accounting-estimates-and-quotes`  
**Application:** Accounting  
**Source feature:** [Estimates and Quotes](../../features/accounting/estimates-and-quotes.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Estimates and Quotes domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Branded estimates/quotes
- items
- terms
- versions
- delivery
...
- Use `/api/v1/accounting/estimates-and-quotes` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/estimates-and-quotes`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/estimates-and-quotes?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/estimates-and-quotes/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/estimates-and-quotes` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/estimates-and-quotes/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/estimates-and-quotes/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/estimates-and-quotes/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "branded_estimates_quotes": "example-value",
  "items": "example-value",
  "terms": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-estimates-and-quotes`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-estimates-and-quotes.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.estimates.and.quotes.list`, `accounting.estimates.and.quotes.get`, `accounting.estimates.and.quotes.create`, `accounting.estimates.and.quotes.update`, and `accounting.estimates.and.quotes.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingEstimatesAndQuotesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `branded_estimates_quotes`, `items`, `terms`, `versions`, `delivery`, `acceptance_decline`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting estimates and quotes API
  version: 1.0.0
paths:
  /api/v1/accounting/estimates-and-quotes:
    get:
      operationId: accounting.estimates.and.quotes.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingEstimatesAndQuotesResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-estimates-and-quotes
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingEstimatesAndQuotesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-estimates-and-quotes`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-estimates-and-quotes`.
