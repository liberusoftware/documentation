# Accounting: Bank Rules API

## Canonical one-to-one API module specification

**API package:** `module-accounting-bank-rules-api`  
**Matching domain module:** `accounting-bank-rules`  
**Application:** Accounting  
**Source feature:** [Bank Rules](../../features/accounting/bank-rules.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Bank Rules domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Payee/text/amount/account conditions
- split/category/tax/dimension actions
- priority
...
- Use `/api/v1/accounting/bank-rules` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/bank-rules`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/bank-rules?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/bank-rules/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/bank-rules` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/bank-rules/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/bank-rules/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/bank-rules/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "payee_text_amount_account_conditions": "example-value",
  "split_category_tax_dimension_actions": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-bank-rules`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-bank-rules.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.bank.rules.list`, `accounting.bank.rules.get`, `accounting.bank.rules.create`, `accounting.bank.rules.update`, and `accounting.bank.rules.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingBankRulesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `payee_text_amount_account_conditions`, `split_category_tax_dimension_actions`, `priority`, `suggestions_automation`, `testing`, `history`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting bank rules API
  version: 1.0.0
paths:
  /api/v1/accounting/bank-rules:
    get:
      operationId: accounting.bank.rules.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingBankRulesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-bank-rules`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-bank-rules`.
