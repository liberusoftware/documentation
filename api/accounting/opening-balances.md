# Accounting: Opening Balances API

## Canonical one-to-one API module specification

**API package:** `module-accounting-opening-balances-api`  
**Matching domain module:** `accounting-opening-balances`  
**Application:** Accounting  
**Source feature:** [Opening Balances](../../features/accounting/opening-balances.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Opening Balances domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Migration date
- account/customer/supplier/bank/item balances
...
- Use `/api/v1/accounting/opening-balances` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/opening-balances`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/opening-balances?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/opening-balances/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/opening-balances` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/opening-balances/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/opening-balances/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/opening-balances/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "migration_date": "example-value",
  "account_customer_supplier_bank_item_balances": "example-value",
  "outstanding_documents": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-opening-balances`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-opening-balances.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.opening.balances.list`, `accounting.opening.balances.get`, `accounting.opening.balances.create`, `accounting.opening.balances.update`, and `accounting.opening.balances.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingOpeningBalancesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `migration_date`, `account_customer_supplier_bank_item_balances`, `outstanding_documents`, `validation`, `approval`, `reconciliation`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting opening balances API
  version: 1.0.0
paths:
  /api/v1/accounting/opening-balances:
    get:
      operationId: accounting.opening.balances.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingOpeningBalancesResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-opening-balances
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingOpeningBalancesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-opening-balances`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-opening-balances`.
