# Accounting: QuickBooks Online Migration API

## Canonical one-to-one API module specification

**API package:** `module-accounting-quickbooks-online-migration-api`  
**Matching domain module:** `accounting-quickbooks-online-migration`  
**Application:** Accounting  
**Source feature:** [QuickBooks Online Migration](../../features/accounting/quickbooks-online-migration.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the QuickBooks Online Migration domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- contacts
- items
- invoices/bills/credits/payments
- journals
- bank data
- projects
- tax
...
- Use `/api/v1/accounting/quickbooks-online-migration` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/quickbooks-online-migration`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/quickbooks-online-migration?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/quickbooks-online-migration/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/quickbooks-online-migration` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/quickbooks-online-migration/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/quickbooks-online-migration/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/quickbooks-online-migration/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
{
  "chart": "example-value",
  "classes_locations": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-quickbooks-online-migration`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-quickbooks-online-migration.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.quickbooks.online.migration.list`, `accounting.quickbooks.online.migration.get`, `accounting.quickbooks.online.migration.create`, `accounting.quickbooks.online.migration.update`, and `accounting.quickbooks.online.migration.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingQuickbooksOnlineMigrationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `chart`, `classes_locations`, `contacts`, `items`, `invoices_bills_credits_payments`, `journals`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting quickbooks online migration API
  version: 1.0.0
paths:
  /api/v1/accounting/quickbooks-online-migration:
    get:
      operationId: accounting.quickbooks.online.migration.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingQuickbooksOnlineMigrationResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-quickbooks-online-migration
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingQuickbooksOnlineMigrationResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-quickbooks-online-migration`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-quickbooks-online-migration`.
