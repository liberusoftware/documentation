# Accounting: Supplier Bills API

## Canonical one-to-one API module specification

**API package:** `module-accounting-supplier-bills-api`  
**Matching domain module:** `accounting-supplier-bills`  
**Application:** Accounting  
**Source feature:** [Supplier Bills](../../features/accounting/supplier-bills.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Supplier Bills domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Draft/approved/posted bills
- capture sources
- line coding
- tax
- due dates
...
- Use `/api/v1/accounting/supplier-bills` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/supplier-bills`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/supplier-bills?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/supplier-bills/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/supplier-bills` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/supplier-bills/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/supplier-bills/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/supplier-bills/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "draft_approved_posted_bills": "example-value",
  "capture_sources": "example-value",
  "line_coding": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-supplier-bills`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-supplier-bills.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.supplier.bills.list`, `accounting.supplier.bills.get`, `accounting.supplier.bills.create`, `accounting.supplier.bills.update`, and `accounting.supplier.bills.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingSupplierBillsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `draft_approved_posted_bills`, `capture_sources`, `line_coding`, `tax`, `due_dates`, `duplicate_detection`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting supplier bills API
  version: 1.0.0
paths:
  /api/v1/accounting/supplier-bills:
    get:
      operationId: accounting.supplier.bills.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingSupplierBillsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-supplier-bills
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingSupplierBillsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-supplier-bills`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-supplier-bills`.
