# Accounting: Bill Payments API

## Canonical one-to-one API module specification

**API package:** `module-accounting-bill-payments-api`  
**Matching domain module:** `accounting-bill-payments`  
**Application:** Accounting  
**Source feature:** [Bill Payments](../../features/accounting/bill-payments.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Bill Payments domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Payment proposals
- due-date/discount optimization
...
- Use `/api/v1/accounting/bill-payments` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/bill-payments`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/bill-payments?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/bill-payments/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/bill-payments` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/bill-payments/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/bill-payments/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/bill-payments/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "payment_proposals": "example-value",
  "due_date_discount_optimization": "example-value",
  "bank_details_validation": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-bill-payments`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-bill-payments.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.bill.payments.list`, `accounting.bill.payments.get`, `accounting.bill.payments.create`, `accounting.bill.payments.update`, and `accounting.bill.payments.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingBillPaymentsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `payment_proposals`, `due_date_discount_optimization`, `bank_details_validation`, `maker_checker_approval`, `files_apis`, `remittances`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting bill payments API
  version: 1.0.0
paths:
  /api/v1/accounting/bill-payments:
    get:
      operationId: accounting.bill.payments.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingBillPaymentsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-bill-payments
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingBillPaymentsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-bill-payments`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-bill-payments`.
