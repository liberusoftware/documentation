# Accounting: Cash Collection Assistant API

## Canonical one-to-one API module specification

**API package:** `module-accounting-cash-collection-assistant-api`  
**Matching domain module:** `accounting-cash-collection-assistant`  
**Application:** Accounting  
**Source feature:** [Cash Collection Assistant](../../features/accounting/cash-collection-assistant.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Cash Collection Assistant domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Invoice-risk prioritization
- reminder drafts/timing
- promise tracking
- policy
- approval
- outcome feedback
...
- Use `/api/v1/accounting/cash-collection-assistant` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/cash-collection-assistant`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/cash-collection-assistant?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/cash-collection-assistant/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/cash-collection-assistant` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/cash-collection-assistant/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/cash-collection-assistant/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/cash-collection-assistant/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "invoice_risk_prioritization": "example-value",
  "reminder_drafts_timing": "example-value",
  "promise_tracking": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-cash-collection-assistant`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-cash-collection-assistant.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.cash.collection.assistant.list`, `accounting.cash.collection.assistant.get`, `accounting.cash.collection.assistant.create`, `accounting.cash.collection.assistant.update`, and `accounting.cash.collection.assistant.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingCashCollectionAssistantResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `invoice_risk_prioritization`, `reminder_drafts_timing`, `promise_tracking`, `policy`, `approval`, `outcome_feedback`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting cash collection assistant API
  version: 1.0.0
paths:
  /api/v1/accounting/cash-collection-assistant:
    get:
      operationId: accounting.cash.collection.assistant.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingCashCollectionAssistantResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-cash-collection-assistant
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingCashCollectionAssistantResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-cash-collection-assistant`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-cash-collection-assistant`.
