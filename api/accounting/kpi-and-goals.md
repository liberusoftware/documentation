# Accounting: KPI and Goals API

## Canonical one-to-one API module specification

**API package:** `module-accounting-kpi-and-goals-api`  
**Matching domain module:** `accounting-kpi-and-goals`  
**Application:** Accounting  
**Source feature:** [KPI and Goals](../../features/accounting/kpi-and-goals.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the KPI and Goals domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/kpi-and-goals` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/kpi-and-goals`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/kpi-and-goals?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/kpi-and-goals/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/kpi-and-goals` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/kpi-and-goals/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/kpi-and-goals/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/kpi-and-goals/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "governed_metrics": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-kpi-and-goals`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-kpi-and-goals.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.kpi.and.goals.list`, `accounting.kpi.and.goals.get`, `accounting.kpi.and.goals.create`, `accounting.kpi.and.goals.update`, and `accounting.kpi.and.goals.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingKpiAndGoalsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `governed_metrics`, `targets`, `owners`, `thresholds`, `periods`, `alerts`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting kpi and goals API
  version: 1.0.0
paths:
  /api/v1/accounting/kpi-and-goals:
    get:
      operationId: accounting.kpi.and.goals.list
      security:
        - sanctum: []
      parameters:
  ...
  schemas:
    AccountingKpiAndGoalsResource:
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-kpi-and-goals
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingKpiAndGoalsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-kpi-and-goals`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-kpi-and-goals`.
