# Accounting: Payroll Liabilities API

## Canonical one-to-one API module specification

**API package:** `module-accounting-payroll-liabilities-api`  
**Matching domain module:** `accounting-payroll-liabilities`  
**Application:** Accounting  
**Source feature:** [Payroll Liabilities](../../features/accounting/payroll-liabilities.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Payroll Liabilities domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Agency/payee balances
- due dates
- payments
- allocations
- exceptions
...
- Use `/api/v1/accounting/payroll-liabilities` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/payroll-liabilities`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/payroll-liabilities?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/payroll-liabilities/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/payroll-liabilities` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/payroll-liabilities/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/payroll-liabilities/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/payroll-liabilities/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "agency_payee_balances": "example-value",
  "due_dates": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-payroll-liabilities`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-payroll-liabilities.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.payroll.liabilities.list`, `accounting.payroll.liabilities.get`, `accounting.payroll.liabilities.create`, `accounting.payroll.liabilities.update`, and `accounting.payroll.liabilities.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingPayrollLiabilitiesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `agency_payee_balances`, `due_dates`, `payments`, `allocations`, `exceptions`, `reconciliation`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting payroll liabilities API
  version: 1.0.0
paths:
  /api/v1/accounting/payroll-liabilities:
    get:
      operationId: accounting.payroll.liabilities.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingPayrollLiabilitiesResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-payroll-liabilities
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingPayrollLiabilitiesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-payroll-liabilities`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-payroll-liabilities`.
