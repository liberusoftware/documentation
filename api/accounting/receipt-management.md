# Accounting: Receipt Management API

## Canonical one-to-one API module specification

**API package:** `module-accounting-receipt-management-api`  
**Matching domain module:** `accounting-receipt-management`  
**Application:** Accounting  
**Source feature:** [Receipt Management](../../features/accounting/receipt-management.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Receipt Management domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Receipt inbox
- matching to bank/card/expense/bill
...
- Use `/api/v1/accounting/receipt-management` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/receipt-management`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/receipt-management?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/receipt-management/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/receipt-management` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/receipt-management/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/receipt-management/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/receipt-management/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "receipt_inbox": "example-value",
  "matching_to_bank_card_expense_bill": "example-value",
  "missing_receipt_requests": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-receipt-management`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-receipt-management.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.receipt.management.list`, `accounting.receipt.management.get`, `accounting.receipt.management.create`, `accounting.receipt.management.update`, and `accounting.receipt.management.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingReceiptManagementResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `receipt_inbox`, `matching_to_bank_card_expense_bill`, `missing_receipt_requests`, `annotations`, `retention`, `audit`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting receipt management API
  version: 1.0.0
paths:
  /api/v1/accounting/receipt-management:
    get:
      operationId: accounting.receipt.management.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingReceiptManagementResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-receipt-management
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingReceiptManagementResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-receipt-management`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-receipt-management`.
