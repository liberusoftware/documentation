# Accounting: Sales Tax and GST API

## Canonical one-to-one API module specification

**API package:** `module-accounting-sales-tax-and-gst-api`  
**Matching domain module:** `accounting-sales-tax-and-gst`  
**Application:** Accounting  
**Source feature:** [Sales Tax and GST](../../features/accounting/sales-tax-and-gst.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Sales Tax and GST domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Registrations/nexus
- destination/origin rules
- taxable bases
- liabilities
- adjustments
- return periods
...
- Use `/api/v1/accounting/sales-tax-and-gst` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/sales-tax-and-gst`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/sales-tax-and-gst?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/sales-tax-and-gst/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/sales-tax-and-gst` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/sales-tax-and-gst/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/sales-tax-and-gst/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/sales-tax-and-gst/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "registrations_nexus": "example-value",
  "destination_origin_rules": "example-value",
  "taxable_bases": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-sales-tax-and-gst`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-sales-tax-and-gst.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.sales.tax.and.gst.list`, `accounting.sales.tax.and.gst.get`, `accounting.sales.tax.and.gst.create`, `accounting.sales.tax.and.gst.update`, and `accounting.sales.tax.and.gst.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingSalesTaxAndGstResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `registrations_nexus`, `destination_origin_rules`, `taxable_bases`, `liabilities`, `adjustments`, `return_periods`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting sales tax and gst API
  version: 1.0.0
paths:
  /api/v1/accounting/sales-tax-and-gst:
    get:
      operationId: accounting.sales.tax.and.gst.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingSalesTaxAndGstResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-sales-tax-and-gst
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingSalesTaxAndGstResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-sales-tax-and-gst`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-sales-tax-and-gst`.
