# Accounting: Matching Intelligence API

## Canonical one-to-one API module specification

**API package:** `module-accounting-matching-intelligence-api`  
**Matching domain module:** `accounting-matching-intelligence`  
**Application:** Accounting  
**Source feature:** [Matching Intelligence](../../features/accounting/matching-intelligence.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Matching Intelligence domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Bank/document/payment/settlement matching suggestions
- confidence
- evidence
- feedback
...
- Use `/api/v1/accounting/matching-intelligence` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/matching-intelligence`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/matching-intelligence?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/matching-intelligence/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/matching-intelligence` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/matching-intelligence/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/matching-intelligence/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/matching-intelligence/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "bank_document_payment_settlement_matching_suggestions": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-matching-intelligence`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-matching-intelligence.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.matching.intelligence.list`, `accounting.matching.intelligence.get`, `accounting.matching.intelligence.create`, `accounting.matching.intelligence.update`, and `accounting.matching.intelligence.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingMatchingIntelligenceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `bank_document_payment_settlement_matching_suggestions`, `confidence`, `evidence`, `feedback`, `safe_automation_thresholds`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting matching intelligence API
  version: 1.0.0
paths:
  /api/v1/accounting/matching-intelligence:
    get:
      operationId: accounting.matching.intelligence.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingMatchingIntelligenceResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-matching-intelligence
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingMatchingIntelligenceResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-matching-intelligence`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-matching-intelligence`.
