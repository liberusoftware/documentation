# Accounting: Account Reconciliations API

## Canonical one-to-one API module specification

**API package:** `module-accounting-account-reconciliations-api`  
**Matching domain module:** `accounting-account-reconciliations`  
**Application:** Accounting  
**Source feature:** [Account Reconciliations](../../features/accounting/account-reconciliations.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Account Reconciliations domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- GL account templates
- source balances
- supporting items
- preparer/reviewer
- aging
...
- Use `/api/v1/accounting/account-reconciliations` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/account-reconciliations`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/account-reconciliations?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/account-reconciliations/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/account-reconciliations` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/account-reconciliations/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/account-reconciliations/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/account-reconciliations/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "gl_account_templates": "example-value",
  "source_balances": "example-value",
  "supporting_items": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-account-reconciliations`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-account-reconciliations.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.account.reconciliations.list`, `accounting.account.reconciliations.get`, `accounting.account.reconciliations.create`, `accounting.account.reconciliations.update`, and `accounting.account.reconciliations.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingAccountReconciliationsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `gl_account_templates`, `source_balances`, `supporting_items`, `preparer_reviewer`, `aging`, `certification`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting account reconciliations API
  version: 1.0.0
paths:
  /api/v1/accounting/account-reconciliations:
    get:
      operationId: accounting.account.reconciliations.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingAccountReconciliationsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-account-reconciliations
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingAccountReconciliationsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-account-reconciliations`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-account-reconciliations`.
