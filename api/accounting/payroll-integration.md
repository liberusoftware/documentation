# Accounting: Payroll Integration API

## Canonical one-to-one API module specification

**API package:** `module-accounting-payroll-integration-api`  
**Matching domain module:** `accounting-payroll-integration`  
**Application:** Accounting  
**Source feature:** [Payroll Integration](../../features/accounting/payroll-integration.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Payroll Integration domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Provider/period/run identity
- employee/contractor references
- import validation
- dimensions/projects
- provider adapters
...
- Use `/api/v1/accounting/payroll-integration` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/payroll-integration`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/payroll-integration?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/payroll-integration/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/payroll-integration` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/payroll-integration/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/payroll-integration/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/payroll-integration/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "provider_period_run_identity": "example-value",
  "employee_contractor_references": "example-value",
  "import_validation": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-payroll-integration`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-payroll-integration.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.payroll.integration.list`, `accounting.payroll.integration.get`, `accounting.payroll.integration.create`, `accounting.payroll.integration.update`, and `accounting.payroll.integration.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingPayrollIntegrationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `provider_period_run_identity`, `employee_contractor_references`, `import_validation`, `dimensions_projects`, `provider_adapters`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting payroll integration API
  version: 1.0.0
paths:
  /api/v1/accounting/payroll-integration:
    get:
      operationId: accounting.payroll.integration.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingPayrollIntegrationResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-payroll-integration
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingPayrollIntegrationResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-payroll-integration`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-payroll-integration`.
