# Accounting: Coding Suggestions API

## Canonical one-to-one API module specification

**API package:** `module-accounting-coding-suggestions-api`  
**Matching domain module:** `accounting-coding-suggestions`  
**Application:** Accounting  
**Source feature:** [Coding Suggestions](../../features/accounting/coding-suggestions.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Coding Suggestions domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Account/tax/dimension/payee suggestions
- confidence
- explanations
- user feedback
- policy thresholds
...
- Use `/api/v1/accounting/coding-suggestions` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/coding-suggestions`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/coding-suggestions?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/coding-suggestions/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/coding-suggestions` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/coding-suggestions/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/coding-suggestions/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/coding-suggestions/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "account_tax_dimension_payee_suggestions": "example-value",
  "confidence": "example-value",
  "explanations": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-coding-suggestions`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-coding-suggestions.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.coding.suggestions.list`, `accounting.coding.suggestions.get`, `accounting.coding.suggestions.create`, `accounting.coding.suggestions.update`, and `accounting.coding.suggestions.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingCodingSuggestionsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `account_tax_dimension_payee_suggestions`, `confidence`, `explanations`, `user_feedback`, `policy_thresholds`, `review`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting coding suggestions API
  version: 1.0.0
paths:
  /api/v1/accounting/coding-suggestions:
    get:
      operationId: accounting.coding.suggestions.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingCodingSuggestionsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-coding-suggestions
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingCodingSuggestionsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-coding-suggestions`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-coding-suggestions`.
