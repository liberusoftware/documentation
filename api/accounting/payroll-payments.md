# Accounting: Payroll Payments API

## Canonical one-to-one API module specification

**API package:** `module-accounting-payroll-payments-api`  
**Matching domain module:** `accounting-payroll-payments`  
**Application:** Accounting  
**Source feature:** [Payroll Payments](../../features/accounting/payroll-payments.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Payroll Payments domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Net-pay and liability batch references
- approvals
...
- Use `/api/v1/accounting/payroll-payments` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/payroll-payments`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/payroll-payments?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/payroll-payments/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/payroll-payments` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/payroll-payments/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/payroll-payments/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/payroll-payments/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "net_pay_and_liability_batch_references": "example-value",
  "approvals": "example-value",
  "bank_provider_status": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-payroll-payments`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-payroll-payments.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.payroll.payments.list`, `accounting.payroll.payments.get`, `accounting.payroll.payments.create`, `accounting.payroll.payments.update`, and `accounting.payroll.payments.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingPayrollPaymentsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `net_pay_and_liability_batch_references`, `approvals`, `bank_provider_status`, `failures`, `bank_reconciliation`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting payroll payments API
  version: 1.0.0
paths:
  /api/v1/accounting/payroll-payments:
    get:
      operationId: accounting.payroll.payments.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingPayrollPaymentsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-payroll-payments
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingPayrollPaymentsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-payroll-payments`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-payroll-payments`.
