# Accounting: Migration Framework API

## Canonical one-to-one API module specification

**API package:** `module-accounting-migration-framework-api`  
**Matching domain module:** `accounting-migration-framework`  
**Application:** Accounting  
**Source feature:** [Migration Framework](../../features/accounting/migration-framework.md)  
...
- Use `/api/v1/accounting/migration-framework` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/migration-framework`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/migration-framework?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/migration-framework/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/migration-framework` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/migration-framework/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/migration-framework/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/migration-framework/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "source_inventory": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-migration-framework`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-migration-framework.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.migration.framework.list`, `accounting.migration.framework.get`, `accounting.migration.framework.create`, `accounting.migration.framework.update`, and `accounting.migration.framework.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingMigrationFrameworkResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `source_inventory`, `mapping`, `dry_run`, `transforms`, `validation`, `batches`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting migration framework API
  version: 1.0.0
paths:
  /api/v1/accounting/migration-framework:
    get:
      operationId: accounting.migration.framework.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingMigrationFrameworkResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-migration-framework
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingMigrationFrameworkResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-migration-framework`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-migration-framework`.
