# Accounting: Credit Notes and Adjustments API

## Canonical one-to-one API module specification

**API package:** `module-accounting-credit-notes-and-adjustments-api`  
**Matching domain module:** `accounting-credit-notes-and-adjustments`  
**Application:** Accounting  
**Source feature:** [Credit Notes and Adjustments](../../features/accounting/credit-notes-and-adjustments.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Credit Notes and Adjustments domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- approval
- invoice allocation
- refund/store-credit references
- tax correction
- audit
...
- Use `/api/v1/accounting/credit-notes-and-adjustments` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/credit-notes-and-adjustments`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/credit-notes-and-adjustments?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/credit-notes-and-adjustments/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/credit-notes-and-adjustments` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/credit-notes-and-adjustments/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/credit-notes-and-adjustments/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/credit-notes-and-adjustments/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "full_partial_credits": "example-value",
  "reasons": "example-value",
  "approval": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-credit-notes-and-adjustments`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-credit-notes-and-adjustments.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.credit.notes.and.adjustments.list`, `accounting.credit.notes.and.adjustments.get`, `accounting.credit.notes.and.adjustments.create`, `accounting.credit.notes.and.adjustments.update`, and `accounting.credit.notes.and.adjustments.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingCreditNotesAndAdjustmentsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `full_partial_credits`, `reasons`, `approval`, `invoice_allocation`, `refund_store_credit_references`, `tax_correction`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting credit notes and adjustments API
  version: 1.0.0
paths:
  /api/v1/accounting/credit-notes-and-adjustments:
    get:
      operationId: accounting.credit.notes.and.adjustments.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingCreditNotesAndAdjustmentsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-credit-notes-and-adjustments
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingCreditNotesAndAdjustmentsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-credit-notes-and-adjustments`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-credit-notes-and-adjustments`.
