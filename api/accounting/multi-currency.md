# Accounting: Multi-Currency API

## Canonical one-to-one API module specification

**API package:** `module-accounting-multi-currency-api`  
**Matching domain module:** `accounting-multi-currency`  
**Application:** Accounting  
**Source feature:** [Multi-Currency](../../features/accounting/multi-currency.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Multi-Currency domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Transaction/functional/reporting currencies
- rates
- revaluation
...
- Use `/api/v1/accounting/multi-currency` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/multi-currency`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/multi-currency?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/multi-currency/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/multi-currency` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/multi-currency/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/multi-currency/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/multi-currency/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "transaction_functional_reporting_currencies": "example-value",
  "rates": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-multi-currency`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-multi-currency.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.multi.currency.list`, `accounting.multi.currency.get`, `accounting.multi.currency.create`, `accounting.multi.currency.update`, and `accounting.multi.currency.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingMultiCurrencyResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `transaction_functional_reporting_currencies`, `rates`, `revaluation`, `realized_unrealized_gains`, `historical_rates`, `reconciliation`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting multi currency API
  version: 1.0.0
paths:
  /api/v1/accounting/multi-currency:
    get:
      operationId: accounting.multi.currency.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingMultiCurrencyResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-multi-currency
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingMultiCurrencyResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-multi-currency`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-multi-currency`.
