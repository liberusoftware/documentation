# Accounting: Debt and Loans API

## Canonical one-to-one API module specification

**API package:** `module-accounting-debt-and-loans-api`  
**Matching domain module:** `accounting-debt-and-loans`  
**Application:** Accounting  
**Source feature:** [Debt and Loans](../../features/accounting/debt-and-loans.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Debt and Loans domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/debt-and-loans` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/debt-and-loans`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/debt-and-loans?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/debt-and-loans/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/debt-and-loans` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/debt-and-loans/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/debt-and-loans/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/debt-and-loans/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "facilities": "example-value",
  "drawdowns": "example-value",
  "repayments": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-debt-and-loans`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-debt-and-loans.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.debt.and.loans.list`, `accounting.debt.and.loans.get`, `accounting.debt.and.loans.create`, `accounting.debt.and.loans.update`, and `accounting.debt.and.loans.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingDebtAndLoansResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `facilities`, `drawdowns`, `repayments`, `interest_fees`, `schedules`, `covenants`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting debt and loans API
  version: 1.0.0
paths:
  /api/v1/accounting/debt-and-loans:
    get:
      operationId: accounting.debt.and.loans.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingDebtAndLoansResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-debt-and-loans
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingDebtAndLoansResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-debt-and-loans`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-debt-and-loans`.
