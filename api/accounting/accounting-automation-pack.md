# Accounting: Accounting Automation Pack API

## Canonical one-to-one API module specification

**API package:** `module-accounting-automation-pack-api`  
**Matching domain module:** `accounting-automation-pack`  
**Application:** Accounting  
**Source feature:** [Accounting Automation Pack](../../features/accounting/accounting-automation-pack.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Accounting Automation Pack domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Financial triggers/actions/recipes
- approvals
- schedules
- simulation
- idempotency
...
- Use `/api/v1/accounting/accounting-automation-pack` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/accounting-automation-pack`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/accounting-automation-pack?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/accounting-automation-pack/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/accounting-automation-pack` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/accounting-automation-pack/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/accounting-automation-pack/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/accounting-automation-pack/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "financial_triggers_actions_recipes": "example-value",
  "approvals": "example-value",
  "schedules": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-automation-pack`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-automation-pack.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.automation.pack.list`, `accounting.automation.pack.get`, `accounting.automation.pack.create`, `accounting.automation.pack.update`, and `accounting.automation.pack.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingAutomationPackResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `financial_triggers_actions_recipes`, `approvals`, `schedules`, `simulation`, `idempotency`, `generic_automation_integration`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting automation pack API
  version: 1.0.0
paths:
  /api/v1/accounting/accounting-automation-pack:
    get:
      operationId: accounting.automation.pack.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingAutomationPackResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-automation-pack
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingAutomationPackResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-automation-pack`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-automation-pack`.
