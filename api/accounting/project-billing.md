# Accounting: Project Billing API

## Canonical one-to-one API module specification

**API package:** `module-accounting-project-billing-api`  
**Matching domain module:** `accounting-project-billing`  
**Application:** Accounting  
**Source feature:** [Project Billing](../../features/accounting/project-billing.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Project Billing domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/project-billing` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/project-billing`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/project-billing?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/project-billing/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/project-billing` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/project-billing/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/project-billing/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/project-billing/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "fixed_fee": "example-value",
  "time_material": "example-value",
  "milestone": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-project-billing`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-project-billing.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.project.billing.list`, `accounting.project.billing.get`, `accounting.project.billing.create`, `accounting.project.billing.update`, and `accounting.project.billing.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingProjectBillingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `fixed_fee`, `time_material`, `milestone`, `progress`, `retainer`, `billable_time_expense`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting project billing API
  version: 1.0.0
paths:
  /api/v1/accounting/project-billing:
    get:
      operationId: accounting.project.billing.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingProjectBillingResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-project-billing
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingProjectBillingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-project-billing`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-project-billing`.
