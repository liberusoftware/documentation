# Accounting: Construction Tax API

## Canonical one-to-one API module specification

**API package:** `module-accounting-construction-tax-api`  
**Matching domain module:** `accounting-construction-tax`  
**Application:** Accounting  
**Source feature:** [Construction Tax](../../features/accounting/construction-tax.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Construction Tax domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- CIS-style subcontractor verification
- deduction rates
- statements
- monthly returns
- corrections
...
- Use `/api/v1/accounting/construction-tax` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/construction-tax`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/construction-tax?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/construction-tax/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/construction-tax` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/construction-tax/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/construction-tax/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/construction-tax/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "cis_style_subcontractor_verification": "example-value",
  "deduction_rates": "example-value",
  "statements": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-construction-tax`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-construction-tax.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.construction.tax.list`, `accounting.construction.tax.get`, `accounting.construction.tax.create`, `accounting.construction.tax.update`, and `accounting.construction.tax.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingConstructionTaxResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `cis_style_subcontractor_verification`, `deduction_rates`, `statements`, `monthly_returns`, `corrections`, `filing_adapters`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting construction tax API
  version: 1.0.0
paths:
  /api/v1/accounting/construction-tax:
    get:
      operationId: accounting.construction.tax.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingConstructionTaxResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-construction-tax
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingConstructionTaxResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-construction-tax`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-construction-tax`.
