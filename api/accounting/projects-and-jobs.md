# Accounting: Projects and Jobs API

## Canonical one-to-one API module specification

**API package:** `module-accounting-projects-and-jobs-api`  
...
This optional API presentation package exposes approved HTTP operations for the Projects and Jobs domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/projects-and-jobs` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/projects-and-jobs`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/projects-and-jobs?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/projects-and-jobs/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/projects-and-jobs` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/projects-and-jobs/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/projects-and-jobs/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/projects-and-jobs/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
```json
{
  ...
  "project_job_hierarchy": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-projects-and-jobs`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-projects-and-jobs.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.projects.and.jobs.list`, `accounting.projects.and.jobs.get`, `accounting.projects.and.jobs.create`, `accounting.projects.and.jobs.update`, and `accounting.projects.and.jobs.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingProjectsAndJobsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `customers`, `project_job_hierarchy`, `status`, `dates`, `managers`, `budgets`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting projects and jobs API
  version: 1.0.0
paths:
  /api/v1/accounting/projects-and-jobs:
    get:
      operationId: accounting.projects.and.jobs.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingProjectsAndJobsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-projects-and-jobs
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingProjectsAndJobsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-projects-and-jobs`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-projects-and-jobs`.
