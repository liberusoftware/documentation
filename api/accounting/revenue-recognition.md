# Accounting: Revenue Recognition API

## Canonical one-to-one API module specification

**API package:** `module-accounting-revenue-recognition-api`  
...
This optional API presentation package exposes approved HTTP operations for the Revenue Recognition domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/revenue-recognition` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/revenue-recognition`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/revenue-recognition?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/revenue-recognition/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/revenue-recognition` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/revenue-recognition/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/revenue-recognition/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/revenue-recognition/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "obligations_schedules": "example-value",
  "allocation_references": "example-value",
  "deferred_accrued_revenue": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-revenue-recognition`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-revenue-recognition.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.revenue.recognition.list`, `accounting.revenue.recognition.get`, `accounting.revenue.recognition.create`, `accounting.revenue.recognition.update`, and `accounting.revenue.recognition.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingRevenueRecognitionResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `obligations_schedules`, `allocation_references`, `deferred_accrued_revenue`, `recognition_runs`, `modifications`, `reconciliation_optional`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting revenue recognition API
  version: 1.0.0
paths:
  /api/v1/accounting/revenue-recognition:
    get:
      operationId: accounting.revenue.recognition.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingRevenueRecognitionResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-revenue-recognition
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingRevenueRecognitionResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-revenue-recognition`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-revenue-recognition`.
