# SAP: Partners and Portals API

## Canonical one-to-one API module specification

**API package:** `module-sap-partners-and-portals-api`  
**Matching domain module:** `sap-partners-and-portals`  
**Application:** SAP  
**Source feature:** [Partners and Portals](../../features/sap/partners-and-portals.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Partners and Portals domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/sap/partners-and-portals` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/sap/partners-and-portals`
...
|---|---|---|---|
| `GET` | `/api/v1/sap/partners-and-portals?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/sap/partners-and-portals/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/sap/partners-and-portals` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/sap/partners-and-portals/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/sap/partners-and-portals/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/sap/partners-and-portals/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
```json
{
  ...
  "resellers": "example-value",
  "partners": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `sap-partners-and-portals`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/sap-partners-and-portals.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `sap.partners.and.portals.list`, `sap.partners.and.portals.get`, `sap.partners.and.portals.create`, `sap.partners.and.portals.update`, and `sap.partners.and.portals.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `SapPartnersAndPortalsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `suppliers`, `resellers`, `partners`, `customers`, `employees`, `delegated_self_service`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: sap partners and portals API
  version: 1.0.0
paths:
  /api/v1/sap/partners-and-portals:
    get:
      operationId: sap.partners.and.portals.list
      security:
        - sanctum: []
      parameters:
  ...
  schemas:
    SapPartnersAndPortalsResource:
      ...
      properties:
        ...
        type:
          type: string
          const: sap-partners-and-portals
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/SapPartnersAndPortalsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `sap-partners-and-portals`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `sap-partners-and-portals`.
