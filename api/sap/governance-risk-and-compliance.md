# SAP: Governance, Risk and Compliance API

## Canonical one-to-one API module specification

**API package:** `module-sap-governance-risk-and-compliance-api`  
**Matching domain module:** `sap-governance-risk-and-compliance`  
**Application:** SAP  
**Source feature:** [Governance, Risk and Compliance](../../features/sap/governance-risk-and-compliance.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Governance, Risk and Compliance domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/sap/governance-risk-and-compliance` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/sap/governance-risk-and-compliance`
...
|---|---|---|---|
| `GET` | `/api/v1/sap/governance-risk-and-compliance?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/sap/governance-risk-and-compliance/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/sap/governance-risk-and-compliance` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/sap/governance-risk-and-compliance/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/sap/governance-risk-and-compliance/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/sap/governance-risk-and-compliance/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
This module owns a versioned OpenAPI 3.1 fragment for `sap-governance-risk-and-compliance`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/sap-governance-risk-and-compliance.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `sap.governance.risk.and.compliance.list`, `sap.governance.risk.and.compliance.get`, `sap.governance.risk.and.compliance.create`, `sap.governance.risk.and.compliance.update`, and `sap.governance.risk.and.compliance.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `SapGovernanceRiskAndComplianceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `policies`, `controls`, `risks`, `audits`, `evidence`, `incidents`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: sap governance risk and compliance API
  version: 1.0.0
paths:
  /api/v1/sap/governance-risk-and-compliance:
    get:
      operationId: sap.governance.risk.and.compliance.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    SapGovernanceRiskAndComplianceResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: sap-governance-risk-and-compliance
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/SapGovernanceRiskAndComplianceResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `sap-governance-risk-and-compliance`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `sap-governance-risk-and-compliance`.
