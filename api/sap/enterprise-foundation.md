# SAP: Enterprise Foundation API

## Canonical one-to-one API module specification

**API package:** `module-sap-enterprise-foundation-api`  
**Matching domain module:** `sap-enterprise-foundation`  
**Application:** SAP  
**Source feature:** [Enterprise Foundation](../../features/sap/enterprise-foundation.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Enterprise Foundation domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/sap/enterprise-foundation` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/sap/enterprise-foundation`
...
|---|---|---|---|
| `GET` | `/api/v1/sap/enterprise-foundation?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/sap/enterprise-foundation/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/sap/enterprise-foundation` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/sap/enterprise-foundation/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/sap/enterprise-foundation/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/sap/enterprise-foundation/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "organization_hierarchy": "example-value",
  "legal_entities": "example-value",
  "master_data": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `sap-enterprise-foundation`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/sap-enterprise-foundation.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `sap.enterprise.foundation.list`, `sap.enterprise.foundation.get`, `sap.enterprise.foundation.create`, `sap.enterprise.foundation.update`, and `sap.enterprise.foundation.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `SapEnterpriseFoundationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `organization_hierarchy`, `legal_entities`, `master_data`, `dimensions`, `workflow`, `approvals`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: sap enterprise foundation API
  version: 1.0.0
paths:
  /api/v1/sap/enterprise-foundation:
    get:
      operationId: sap.enterprise.foundation.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    SapEnterpriseFoundationResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: sap-enterprise-foundation
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/SapEnterpriseFoundationResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `sap-enterprise-foundation`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `sap-enterprise-foundation`.
