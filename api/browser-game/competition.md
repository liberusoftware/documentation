# Browser Game: Competition API

## Canonical one-to-one API module specification

**API package:** `module-browser-game-competition-api`  
**Matching domain module:** `browser-game-competition`  
**Application:** Browser Game  
**Source feature:** [Competition](../../features/browser-game/competition.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Competition domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/browser-game/competition` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/browser-game/competition`
...
|---|---|---|---|
| `GET` | `/api/v1/browser-game/competition?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/browser-game/competition/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/browser-game/competition` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/browser-game/competition/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/browser-game/competition/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/browser-game/competition/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "pvp_modes": "example-value",
  "matchmaking": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `browser-game-competition`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/browser-game-competition.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `browser.game.competition.list`, `browser.game.competition.get`, `browser.game.competition.create`, `browser.game.competition.update`, and `browser.game.competition.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `BrowserGameCompetitionResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `pvp_modes`, `matchmaking`, `seasons`, `rankings`, `leaderboards`, `rewards`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: browser game competition API
  version: 1.0.0
paths:
  /api/v1/browser-game/competition:
    get:
      operationId: browser.game.competition.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    BrowserGameCompetitionResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: browser-game-competition
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/BrowserGameCompetitionResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `browser-game-competition`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `browser-game-competition`.
