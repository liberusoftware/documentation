# Billing: Collections API

## Canonical one-to-one API module specification

**API package:** `module-billing-collections-api`  
**Matching domain module:** `billing-collections`  
**Application:** Billing  
**Source feature:** [Collections](../../features/billing/collections.md)  
...
- Use `/api/v1/billing/collections` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/billing/collections`
...
|---|---|---|---|
| `GET` | `/api/v1/billing/collections?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/billing/collections/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/billing/collections` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/billing/collections/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/billing/collections/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/billing/collections/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "retries": "example-value",
  "dunning": "example-value",
  "reminders": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `billing-collections`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/billing-collections.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `billing.collections.list`, `billing.collections.get`, `billing.collections.create`, `billing.collections.update`, and `billing.collections.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `BillingCollectionsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `retries`, `dunning`, `reminders`, `promises`, `credit_control`, `suspension`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: billing collections API
  version: 1.0.0
paths:
  /api/v1/billing/collections:
    get:
      operationId: billing.collections.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/BillingCollectionsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `billing-collections`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `billing-collections`.
