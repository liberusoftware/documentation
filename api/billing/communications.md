...
**API package:** `module-billing-communications-api`  
**Matching domain module:** `billing-communications`  
**Application:** Billing  
**Source feature:** [Communications](../../features/billing/communications.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Communications domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- VoIP/SMS service inventory
- number lifecycle
- usage/rating imports
- provider adapters
...
- Use `/api/v1/billing/communications` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/billing/communications`
...
|---|---|---|---|
| `GET` | `/api/v1/billing/communications?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/billing/communications/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/billing/communications` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/billing/communications/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/billing/communications/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/billing/communications/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "voip_sms_service_inventory": "example-value",
  "number_lifecycle": "example-value",
  "usage_rating_imports": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `billing-communications`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/billing-communications.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `billing.communications.list`, `billing.communications.get`, `billing.communications.create`, `billing.communications.update`, and `billing.communications.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `BillingCommunicationsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `voip_sms_service_inventory`, `number_lifecycle`, `usage_rating_imports`, `provider_adapters`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: billing communications API
  version: 1.0.0
paths:
  /api/v1/billing/communications:
    get:
      operationId: billing.communications.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/BillingCommunicationsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `billing-communications`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `billing-communications`.
