# Control Panel: API and Automation API

## Canonical one-to-one API module specification

**API package:** `module-control-panel-api-and-automation-api`  
**Matching domain module:** `control-panel-api-and-automation`  
**Application:** Control Panel  
**Source feature:** [API and Automation](../../features/control-panel/api-and-automation.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the API and Automation domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/control-panel/api-and-automation` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/control-panel/api-and-automation`
...
|---|---|---|---|
| `GET` | `/api/v1/control-panel/api-and-automation?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/control-panel/api-and-automation/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/control-panel/api-and-automation` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/control-panel/api-and-automation/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/control-panel/api-and-automation/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/control-panel/api-and-automation/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "scoped_api": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `control-panel-api-and-automation`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/control-panel-api-and-automation.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `control.panel.api.and.automation.list`, `control.panel.api.and.automation.get`, `control.panel.api.and.automation.create`, `control.panel.api.and.automation.update`, and `control.panel.api.and.automation.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `ControlPanelApiAndAutomationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `scoped_api`, `webhooks`, `cli`, `templates`, `schedules`, `orchestration`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: control panel api and automation API
  version: 1.0.0
paths:
  /api/v1/control-panel/api-and-automation:
    get:
      operationId: control.panel.api.and.automation.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    ControlPanelApiAndAutomationResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: control-panel-api-and-automation
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/ControlPanelApiAndAutomationResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `control-panel-api-and-automation`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `control-panel-api-and-automation`.
