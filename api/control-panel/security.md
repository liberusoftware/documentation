...
**API package:** `module-control-panel-security-api`  
**Matching domain module:** `control-panel-security`  
**Application:** Control Panel  
**Source feature:** [Security](../../features/control-panel/security.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Security domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- malware
- intrusion controls
- vulnerability and compliance status
...
- Use `/api/v1/control-panel/security` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/control-panel/security`
...
|---|---|---|---|
| `GET` | `/api/v1/control-panel/security?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/control-panel/security/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/control-panel/security` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/control-panel/security/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/control-panel/security/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/control-panel/security/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "hardening": "example-value",
  "patching": "example-value",
  "mfa_rbac": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `control-panel-security`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/control-panel-security.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `control.panel.security.list`, `control.panel.security.get`, `control.panel.security.create`, `control.panel.security.update`, and `control.panel.security.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `ControlPanelSecurityResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `hardening`, `patching`, `mfa_rbac`, `secrets`, `malware`, `intrusion_controls`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: control panel security API
  version: 1.0.0
paths:
  /api/v1/control-panel/security:
    get:
      operationId: control.panel.security.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/ControlPanelSecurityResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `control-panel-security`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `control-panel-security`.
