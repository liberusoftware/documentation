...
**API package:** `module-real-estate-marketing-api`  
**Matching domain module:** `real-estate-marketing`  
**Application:** Real Estate  
**Source feature:** [Marketing](../../features/real-estate/marketing.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Marketing domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/real-estate/marketing` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/real-estate/marketing`
...
|---|---|---|---|
| `GET` | `/api/v1/real-estate/marketing?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/real-estate/marketing/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/real-estate/marketing` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/real-estate/marketing/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/real-estate/marketing/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/real-estate/marketing/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
```json
{
  ...
  "boards": "example-value",
  "brochures": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `real-estate-marketing`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/real-estate-marketing.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `real.estate.marketing.list`, `real.estate.marketing.get`, `real.estate.marketing.create`, `real.estate.marketing.update`, and `real.estate.marketing.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `RealEstateMarketingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `campaigns`, `boards`, `brochures`, `portals`, `website`, `social_email`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
info:
  ...
  version: 1.0.0
paths:
  /api/v1/real-estate/marketing:
    get:
      operationId: real.estate.marketing.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/RealEstateMarketingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `real-estate-marketing`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `real-estate-marketing`.
