# Real Estate: Media and Documents API

## Canonical one-to-one API module specification

**API package:** `module-real-estate-media-and-documents-api`  
**Matching domain module:** `real-estate-media-and-documents`  
**Application:** Real Estate  
**Source feature:** [Media and Documents](../../features/real-estate/media-and-documents.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Media and Documents domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/real-estate/media-and-documents` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/real-estate/media-and-documents`
...
|---|---|---|---|
| `GET` | `/api/v1/real-estate/media-and-documents?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/real-estate/media-and-documents/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/real-estate/media-and-documents` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/real-estate/media-and-documents/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/real-estate/media-and-documents/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/real-estate/media-and-documents/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
This module owns a versioned OpenAPI 3.1 fragment for `real-estate-media-and-documents`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/real-estate-media-and-documents.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `real.estate.media.and.documents.list`, `real.estate.media.and.documents.get`, `real.estate.media.and.documents.create`, `real.estate.media.and.documents.update`, and `real.estate.media.and.documents.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `RealEstateMediaAndDocumentsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `photos`, `floorplans`, `video`, `certificates`, `rights`, `ordering`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: real estate media and documents API
  version: 1.0.0
paths:
  /api/v1/real-estate/media-and-documents:
    get:
      operationId: real.estate.media.and.documents.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    RealEstateMediaAndDocumentsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: real-estate-media-and-documents
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/RealEstateMediaAndDocumentsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `real-estate-media-and-documents`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `real-estate-media-and-documents`.
