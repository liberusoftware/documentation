# Social Network: Engagement API

## Canonical one-to-one API module specification

**API package:** `module-social-network-engagement-api`  
**Matching domain module:** `social-network-engagement`  
**Application:** Social Network  
**Source feature:** [Engagement](../../features/social-network/engagement.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Engagement domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/social-network/engagement` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/social-network/engagement`
...
|---|---|---|---|
| `GET` | `/api/v1/social-network/engagement?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/social-network/engagement/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/social-network/engagement` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/social-network/engagement/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/social-network/engagement/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/social-network/engagement/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "reactions": "example-value",
  "comments": "example-value",
  "replies": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `social-network-engagement`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/social-network-engagement.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `social.network.engagement.list`, `social.network.engagement.get`, `social.network.engagement.create`, `social.network.engagement.update`, and `social.network.engagement.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `SocialNetworkEngagementResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `reactions`, `comments`, `replies`, `sharing`, `bookmarks`, `counters`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: social network engagement API
  version: 1.0.0
paths:
  /api/v1/social-network/engagement:
    get:
      operationId: social.network.engagement.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    SocialNetworkEngagementResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: social-network-engagement
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/SocialNetworkEngagementResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `social-network-engagement`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `social-network-engagement`.
