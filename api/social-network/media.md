# Social Network: Media API

## Canonical one-to-one API module specification

**API package:** `module-social-network-media-api`  
**Matching domain module:** `social-network-media`  
**Application:** Social Network  
**Source feature:** [Media](../../features/social-network/media.md)  
...
- Use `/api/v1/social-network/media` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/social-network/media`
...
|---|---|---|---|
| `GET` | `/api/v1/social-network/media?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/social-network/media/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/social-network/media` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/social-network/media/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/social-network/media/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/social-network/media/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
This module owns a versioned OpenAPI 3.1 fragment for `social-network-media`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/social-network-media.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `social.network.media.list`, `social.network.media.get`, `social.network.media.create`, `social.network.media.update`, and `social.network.media.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `SocialNetworkMediaResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `images`, `video`, `audio`, `files`, `processing`, `alt_text`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: social network media API
  version: 1.0.0
paths:
  /api/v1/social-network/media:
    get:
      operationId: social.network.media.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/SocialNetworkMediaResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `social-network-media`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `social-network-media`.
