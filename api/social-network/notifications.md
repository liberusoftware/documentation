...
**API package:** `module-social-network-notifications-api`  
**Matching domain module:** `social-network-notifications`  
**Application:** Social Network  
**Source feature:** [Notifications](../../features/social-network/notifications.md)  
...
- Use `/api/v1/social-network/notifications` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/social-network/notifications`
...
|---|---|---|---|
| `GET` | `/api/v1/social-network/notifications?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/social-network/notifications/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/social-network/notifications` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/social-network/notifications/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/social-network/notifications/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/social-network/notifications/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
```json
{
  ...
  "in_app_email_push_delivery": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `social-network-notifications`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/social-network-notifications.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `social.network.notifications.list`, `social.network.notifications.get`, `social.network.notifications.create`, `social.network.notifications.update`, and `social.network.notifications.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `SocialNetworkNotificationsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `preferences`, `in_app_email_push_delivery`, `grouping`, `digests`, `quiet_hours`, `read_state`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: social network notifications API
  version: 1.0.0
paths:
  /api/v1/social-network/notifications:
    get:
      operationId: social.network.notifications.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    SocialNetworkNotificationsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: social-network-notifications
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/SocialNetworkNotificationsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `social-network-notifications`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `social-network-notifications`.
