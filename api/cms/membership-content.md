# CMS: Membership Content API

## Canonical one-to-one API module specification

**API package:** `module-cms-membership-content-api`  
**Matching domain module:** `cms-membership-content`  
**Application:** CMS  
**Source feature:** [Membership Content](../../features/cms/membership-content.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Membership Content domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Gated content
- access rules
- memberships/entitlement references
- drip schedules
- downloads
- portal integration
...
- Use `/api/v1/cms/membership-content` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/membership-content`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/membership-content?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/membership-content/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/membership-content` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/membership-content/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/membership-content/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/membership-content/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "gated_content": "example-value",
  "access_rules": "example-value",
  "memberships_entitlement_references": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-membership-content`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-membership-content.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.membership.content.list`, `cms.membership.content.get`, `cms.membership.content.create`, `cms.membership.content.update`, and `cms.membership.content.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsMembershipContentResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `gated_content`, `access_rules`, `memberships_entitlement_references`, `drip_schedules`, `downloads`, `portal_integration`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms membership content API
  version: 1.0.0
paths:
  /api/v1/cms/membership-content:
    get:
      operationId: cms.membership.content.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsMembershipContentResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-membership-content`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-membership-content`.
