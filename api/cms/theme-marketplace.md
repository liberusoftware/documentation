...
**API package:** `module-cms-theme-marketplace-api`  
**Matching domain module:** `cms-theme-marketplace`  
**Application:** CMS  
**Source feature:** [Theme Marketplace](../../features/cms/theme-marketplace.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Theme Marketplace domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/cms/theme-marketplace` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/theme-marketplace`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/theme-marketplace?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/theme-marketplace/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/theme-marketplace` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/theme-marketplace/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/theme-marketplace/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/theme-marketplace/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "theme_manifests": "example-value",
  "compatibility": "example-value",
  "previews": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-theme-marketplace`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-theme-marketplace.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.theme.marketplace.list`, `cms.theme.marketplace.get`, `cms.theme.marketplace.create`, `cms.theme.marketplace.update`, and `cms.theme.marketplace.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsThemeMarketplaceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `theme_manifests`, `compatibility`, `previews`, `licenses`, `child_themes`, `installation`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms theme marketplace API
  version: 1.0.0
paths:
  /api/v1/cms/theme-marketplace:
    get:
      operationId: cms.theme.marketplace.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsThemeMarketplaceResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-theme-marketplace`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-theme-marketplace`.
