...
**API package:** `module-cms-forums-integration-api`  
**Matching domain module:** `cms-forums-integration`  
**Application:** CMS  
**Source feature:** [Forums Integration](../../features/cms/forums-integration.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Forums Integration domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Forum/community provider contracts
...
- Use `/api/v1/cms/forums-integration` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/forums-integration`
...
```json
{
  "forum_community_provider_contracts": "example-value",
  "content_profile_references": "example-value",
  "recent_discussions": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-forums-integration`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-forums-integration.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.forums.integration.list`, `cms.forums.integration.get`, `cms.forums.integration.create`, `cms.forums.integration.update`, and `cms.forums.integration.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsForumsIntegrationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `forum_community_provider_contracts`, `content_profile_references`, `recent_discussions`, `moderation_links`, `sso_context`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms forums integration API
  version: 1.0.0
paths:
  /api/v1/cms/forums-integration:
    get:
      operationId: cms.forums.integration.list
      security:
        - sanctum: []
      parameters:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-forums-integration`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-forums-integration`.
