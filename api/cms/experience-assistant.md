...
**API package:** `module-cms-experience-assistant-api`  
**Matching domain module:** `cms-experience-assistant`  
**Application:** CMS  
**Source feature:** [Experience Assistant](../../features/cms/experience-assistant.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Experience Assistant domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Layout/block suggestions
...
- Use `/api/v1/cms/experience-assistant` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/experience-assistant`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/experience-assistant?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/experience-assistant/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/experience-assistant` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/experience-assistant/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/experience-assistant/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/experience-assistant/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "layout_block_suggestions": "example-value",
  "brand_design_constraints": "example-value",
  "responsive_accessibility_checks": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-experience-assistant`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-experience-assistant.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.experience.assistant.list`, `cms.experience.assistant.get`, `cms.experience.assistant.create`, `cms.experience.assistant.update`, and `cms.experience.assistant.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsExperienceAssistantResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `layout_block_suggestions`, `brand_design_constraints`, `responsive_accessibility_checks`, `preview`, `approval`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms experience assistant API
  version: 1.0.0
paths:
  /api/v1/cms/experience-assistant:
    get:
      operationId: cms.experience.assistant.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CmsExperienceAssistantResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: cms-experience-assistant
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsExperienceAssistantResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-experience-assistant`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-experience-assistant`.
