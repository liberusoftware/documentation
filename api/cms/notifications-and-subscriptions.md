# CMS: Notifications and Subscriptions API

## Canonical one-to-one API module specification

**API package:** `module-cms-notifications-and-subscriptions-api`  
...
This optional API presentation package exposes approved HTTP operations for the Notifications and Subscriptions domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Follow content/terms/authors
- publication/update digests
- frequency
- channel preferences
- unsubscribe
- delivery state
...
- Use `/api/v1/cms/notifications-and-subscriptions` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/notifications-and-subscriptions`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/notifications-and-subscriptions?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/notifications-and-subscriptions/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/notifications-and-subscriptions` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/notifications-and-subscriptions/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/notifications-and-subscriptions/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/notifications-and-subscriptions/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "follow_content_terms_authors": "example-value",
  "publication_update_digests": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-notifications-and-subscriptions`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-notifications-and-subscriptions.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.notifications.and.subscriptions.list`, `cms.notifications.and.subscriptions.get`, `cms.notifications.and.subscriptions.create`, `cms.notifications.and.subscriptions.update`, and `cms.notifications.and.subscriptions.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsNotificationsAndSubscriptionsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `follow_content_terms_authors`, `publication_update_digests`, `frequency`, `channel_preferences`, `unsubscribe`, `delivery_state`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms notifications and subscriptions API
  version: 1.0.0
paths:
  /api/v1/cms/notifications-and-subscriptions:
    get:
      operationId: cms.notifications.and.subscriptions.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CmsNotificationsAndSubscriptionsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: cms-notifications-and-subscriptions
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsNotificationsAndSubscriptionsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-notifications-and-subscriptions`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-notifications-and-subscriptions`.
