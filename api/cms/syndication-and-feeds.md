# CMS: Syndication and Feeds API

## Canonical one-to-one API module specification

**API package:** `module-cms-syndication-and-feeds-api`  
**Matching domain module:** `cms-syndication-and-feeds`  
**Application:** CMS  
**Source feature:** [Syndication and Feeds](../../features/cms/syndication-and-feeds.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Syndication and Feeds domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- RSS/Atom and structured feeds
- outbound syndication
- inbound feed imports
- mappings
- deduplication
- attribution
- schedules
...
- Use `/api/v1/cms/syndication-and-feeds` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/syndication-and-feeds`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/syndication-and-feeds?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/syndication-and-feeds/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/syndication-and-feeds` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/syndication-and-feeds/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/syndication-and-feeds/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/syndication-and-feeds/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "rss_atom_and_structured_feeds": "example-value",
  "outbound_syndication": "example-value",
  "inbound_feed_imports": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-syndication-and-feeds`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-syndication-and-feeds.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.syndication.and.feeds.list`, `cms.syndication.and.feeds.get`, `cms.syndication.and.feeds.create`, `cms.syndication.and.feeds.update`, and `cms.syndication.and.feeds.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsSyndicationAndFeedsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `rss_atom_and_structured_feeds`, `outbound_syndication`, `inbound_feed_imports`, `mappings`, `deduplication`, `attribution`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms syndication and feeds API
  version: 1.0.0
paths:
  /api/v1/cms/syndication-and-feeds:
    get:
      operationId: cms.syndication.and.feeds.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CmsSyndicationAndFeedsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: cms-syndication-and-feeds
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsSyndicationAndFeedsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-syndication-and-feeds`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-syndication-and-feeds`.
