# CMS: Hook and Event SDK API

## Canonical one-to-one API module specification

**API package:** `module-cms-hook-and-event-sdk-api`  
**Matching domain module:** `cms-hook-and-event-sdk`  
**Application:** CMS  
**Source feature:** [Hook and Event SDK](../../features/cms/hook-and-event-sdk.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Hook and Event SDK domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Stable content/editor/render/admin/CLI events
...
- Use `/api/v1/cms/hook-and-event-sdk` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/hook-and-event-sdk`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/hook-and-event-sdk?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/hook-and-event-sdk/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/hook-and-event-sdk` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/hook-and-event-sdk/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/hook-and-event-sdk/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/hook-and-event-sdk/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "stable_content_editor_render_admin_cli_events": "example-value",
  "typed_extension_points": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-hook-and-event-sdk`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-hook-and-event-sdk.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.hook.and.event.sdk.list`, `cms.hook.and.event.sdk.get`, `cms.hook.and.event.sdk.create`, `cms.hook.and.event.sdk.update`, and `cms.hook.and.event.sdk.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsHookAndEventSdkResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `stable_content_editor_render_admin_cli_events`, `typed_extension_points`, `priorities`, `isolation`, `diagnostics`, `deprecation`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms hook and event sdk API
  version: 1.0.0
paths:
  /api/v1/cms/hook-and-event-sdk:
    get:
      operationId: cms.hook.and.event.sdk.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsHookAndEventSdkResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-hook-and-event-sdk`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-hook-and-event-sdk`.
