# CMS: Localization API

## Canonical one-to-one API module specification

**API package:** `module-cms-localization-api`  
**Matching domain module:** `cms-localization`  
**Application:** CMS  
**Source feature:** [Localization](../../features/cms/localization.md)  
...
- Content/configuration/menu/taxonomy translation
- locale variants
- fallback
- localized slugs
- direction
...
- Use `/api/v1/cms/localization` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/localization`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/localization?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/localization/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/localization` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/localization/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/localization/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/localization/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "content_configuration_menu_taxonomy_translation": "example-value",
  "locale_variants": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-localization`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-localization.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.localization.list`, `cms.localization.get`, `cms.localization.create`, `cms.localization.update`, and `cms.localization.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsLocalizationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `content_configuration_menu_taxonomy_translation`, `locale_variants`, `fallback`, `localized_slugs`, `direction`, `completeness`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms localization API
  version: 1.0.0
paths:
  /api/v1/cms/localization:
    get:
      operationId: cms.localization.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsLocalizationResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-localization`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-localization`.
