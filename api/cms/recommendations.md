...
**API package:** `module-cms-recommendations-api`  
**Matching domain module:** `cms-recommendations`  
**Application:** CMS  
**Source feature:** [Recommendations](../../features/cms/recommendations.md)  
...
- Popular/latest/related/trending/editorial lists
- pluggable ranking
- audience context
- exclusions
- explanation
...
- Use `/api/v1/cms/recommendations` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/recommendations`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/recommendations?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/recommendations/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/recommendations` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/recommendations/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/recommendations/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/recommendations/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "popular_latest_related_trending_editorial_lists": "example-value",
  "pluggable_ranking": "example-value",
  "audience_context": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-recommendations`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-recommendations.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.recommendations.list`, `cms.recommendations.get`, `cms.recommendations.create`, `cms.recommendations.update`, and `cms.recommendations.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsRecommendationsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `popular_latest_related_trending_editorial_lists`, `pluggable_ranking`, `audience_context`, `exclusions`, `explanation`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms recommendations API
  version: 1.0.0
paths:
  /api/v1/cms/recommendations:
    get:
      operationId: cms.recommendations.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsRecommendationsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-recommendations`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-recommendations`.
