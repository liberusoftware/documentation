# CMS: Static Publishing API

## Canonical one-to-one API module specification

**API package:** `module-cms-static-publishing-api`  
**Matching domain module:** `cms-static-publishing`  
**Application:** CMS  
**Source feature:** [Static Publishing](../../features/cms/static-publishing.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Static Publishing domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/cms/static-publishing` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/static-publishing`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/static-publishing?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/static-publishing/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/static-publishing` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/static-publishing/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/static-publishing/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/static-publishing/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "full_incremental_builds": "example-value",
  "route_manifests": "example-value",
  "cache_invalidation": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-static-publishing`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-static-publishing.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.static.publishing.list`, `cms.static.publishing.get`, `cms.static.publishing.create`, `cms.static.publishing.update`, and `cms.static.publishing.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsStaticPublishingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `full_incremental_builds`, `route_manifests`, `cache_invalidation`, `deployment_adapters`, `preview_builds`, `rollback`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms static publishing API
  version: 1.0.0
paths:
  /api/v1/cms/static-publishing:
    get:
      operationId: cms.static.publishing.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsStaticPublishingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-static-publishing`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-static-publishing`.
