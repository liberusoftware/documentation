...
**API package:** `module-cms-multisite-api`  
**Matching domain module:** `cms-multisite`  
**Application:** CMS  
**Source feature:** [Multisite](../../features/cms/multisite.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Multisite domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Site network
- shared/isolated content
- domains
- site admins
- quotas
- site lifecycle
- network operations
- safe cross-site references
...
- Use `/api/v1/cms/multisite` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/multisite`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/multisite?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/multisite/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/multisite` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/multisite/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/multisite/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/multisite/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "site_network": "example-value",
  "shared_isolated_content": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-multisite`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-multisite.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.multisite.list`, `cms.multisite.get`, `cms.multisite.create`, `cms.multisite.update`, and `cms.multisite.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsMultisiteResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `site_network`, `shared_isolated_content`, `domains`, `site_admins`, `quotas`, `site_lifecycle`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms multisite API
  version: 1.0.0
paths:
  /api/v1/cms/multisite:
    get:
      operationId: cms.multisite.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsMultisiteResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-multisite`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-multisite`.
