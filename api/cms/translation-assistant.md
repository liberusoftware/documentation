# CMS: Translation Assistant API

## Canonical one-to-one API module specification

**API package:** `module-cms-translation-assistant-api`  
**Matching domain module:** `cms-translation-assistant`  
**Application:** CMS  
**Source feature:** [Translation Assistant](../../features/cms/translation-assistant.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Translation Assistant domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/cms/translation-assistant` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/translation-assistant`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/translation-assistant?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/translation-assistant/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/translation-assistant` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/translation-assistant/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/translation-assistant/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/translation-assistant/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "draft_translations": "example-value",
  "glossary_style_enforcement": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-translation-assistant`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-translation-assistant.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.translation.assistant.list`, `cms.translation.assistant.get`, `cms.translation.assistant.create`, `cms.translation.assistant.update`, and `cms.translation.assistant.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsTranslationAssistantResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `draft_translations`, `glossary_style_enforcement`, `confidence`, `review`, `provider_model_provenance`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms translation assistant API
  version: 1.0.0
paths:
  /api/v1/cms/translation-assistant:
    get:
      operationId: cms.translation.assistant.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CmsTranslationAssistantResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: cms-translation-assistant
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsTranslationAssistantResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-translation-assistant`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-translation-assistant`.
