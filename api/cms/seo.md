...
**API package:** `module-cms-seo-api`  
**Matching domain module:** `cms-seo`  
**Application:** CMS  
**Source feature:** [SEO](../../features/cms/seo.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the SEO domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/cms/seo` as the default module route prefix unless the application owns and documents a compatible façade path.
...
|---|---|---|---|
| `GET` | `/api/v1/cms/seo?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/seo/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/seo` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/seo/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/seo/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/seo/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "titles_descriptions": "example-value",
  "canonical_urls": "example-value",
  "robots": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-seo`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-seo.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.seo.list`, `cms.seo.get`, `cms.seo.create`, `cms.seo.update`, and `cms.seo.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsSeoResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `titles_descriptions`, `canonical_urls`, `robots`, `structured_data`, `social_cards`, `hreflang`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsSeoResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-seo`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-seo`.
