# CMS: Block Editor API

## Canonical one-to-one API module specification

**API package:** `module-cms-block-editor-api`  
**Matching domain module:** `cms-block-editor`  
**Application:** CMS  
**Source feature:** [Block Editor](../../features/cms/block-editor.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Block Editor domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/cms/block-editor` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/block-editor`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/block-editor?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/block-editor/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/block-editor` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/block-editor/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/block-editor/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/block-editor/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "typed_content_blocks": "example-value",
  "nesting_rules": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-block-editor`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-block-editor.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.block.editor.list`, `cms.block.editor.get`, `cms.block.editor.create`, `cms.block.editor.update`, and `cms.block.editor.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsBlockEditorResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `typed_content_blocks`, `nesting_rules`, `patterns`, `reusable_global_blocks`, `locking`, `transforms`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms block editor API
  version: 1.0.0
paths:
  /api/v1/cms/block-editor:
    get:
      operationId: cms.block.editor.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsBlockEditorResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-block-editor`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-block-editor`.
