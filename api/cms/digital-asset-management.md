# CMS: Digital Asset Management API

## Canonical one-to-one API module specification

**API package:** `module-cms-digital-asset-management-api`  
**Matching domain module:** `cms-digital-asset-management`  
**Application:** CMS  
**Source feature:** [Digital Asset Management](../../features/cms/digital-asset-management.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Digital Asset Management domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/cms/digital-asset-management` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/digital-asset-management`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/digital-asset-management?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/digital-asset-management/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/digital-asset-management` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/digital-asset-management/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/digital-asset-management/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/digital-asset-management/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "rights": "example-value",
  "licenses": "example-value",
  "attribution": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-digital-asset-management`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-digital-asset-management.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.digital.asset.management.list`, `cms.digital.asset.management.get`, `cms.digital.asset.management.create`, `cms.digital.asset.management.update`, and `cms.digital.asset.management.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsDigitalAssetManagementResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `rights`, `licenses`, `attribution`, `consent_model_releases`, `expiry`, `brand_assets`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms digital asset management API
  version: 1.0.0
paths:
  /api/v1/cms/digital-asset-management:
    get:
      operationId: cms.digital.asset.management.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CmsDigitalAssetManagementResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: cms-digital-asset-management
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsDigitalAssetManagementResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-digital-asset-management`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-digital-asset-management`.
