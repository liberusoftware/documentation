# CMS: Offline and PWA API

## Canonical one-to-one API module specification

**API package:** `module-cms-offline-and-pwa-api`  
**Matching domain module:** `cms-offline-and-pwa`  
**Application:** CMS  
**Source feature:** [Offline and PWA](../../features/cms/offline-and-pwa.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Offline and PWA domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Manifest/service-worker integration points
...
- Use `/api/v1/cms/offline-and-pwa` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/offline-and-pwa`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/offline-and-pwa?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/offline-and-pwa/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/offline-and-pwa` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/offline-and-pwa/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/offline-and-pwa/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/offline-and-pwa/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "manifest_service_worker_integration_points": "example-value",
  "content_caching_policy": "example-value",
  "offline_pages": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-offline-and-pwa`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-offline-and-pwa.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.offline.and.pwa.list`, `cms.offline.and.pwa.get`, `cms.offline.and.pwa.create`, `cms.offline.and.pwa.update`, and `cms.offline.and.pwa.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsOfflineAndPwaResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `manifest_service_worker_integration_points`, `content_caching_policy`, `offline_pages`, `update_behavior`, `install_metadata`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms offline and pwa API
  version: 1.0.0
paths:
  /api/v1/cms/offline-and-pwa:
    get:
      operationId: cms.offline.and.pwa.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsOfflineAndPwaResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-offline-and-pwa`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-offline-and-pwa`.
