...
**API package:** `module-cms-regions-and-widgets-api`  
**Matching domain module:** `cms-regions-and-widgets`  
**Application:** CMS  
**Source feature:** [Regions and Widgets](../../features/cms/regions-and-widgets.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Regions and Widgets domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- placement
- ordering
- visibility by route/context/audience
- schedules
- caching
...
- Use `/api/v1/cms/regions-and-widgets` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/regions-and-widgets`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/regions-and-widgets?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/regions-and-widgets/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/regions-and-widgets` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/regions-and-widgets/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/regions-and-widgets/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/regions-and-widgets/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "theme_regions": "example-value",
  "reusable_widgets": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-regions-and-widgets`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-regions-and-widgets.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.regions.and.widgets.list`, `cms.regions.and.widgets.get`, `cms.regions.and.widgets.create`, `cms.regions.and.widgets.update`, and `cms.regions.and.widgets.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsRegionsAndWidgetsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `theme_regions`, `reusable_widgets`, `placement`, `ordering`, `visibility_by_route_context_audience`, `schedules`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms regions and widgets API
  version: 1.0.0
paths:
  /api/v1/cms/regions-and-widgets:
    get:
      operationId: cms.regions.and.widgets.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsRegionsAndWidgetsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-regions-and-widgets`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-regions-and-widgets`.
