# CMS: Media Assistant API

## Canonical one-to-one API module specification

**API package:** `module-cms-media-assistant-api`  
**Matching domain module:** `cms-media-assistant`  
**Application:** CMS  
**Source feature:** [Media Assistant](../../features/cms/media-assistant.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Media Assistant domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Alt-text/caption/transcript suggestions
- tagging
- crops
- rights warnings
- human review
...
- Use `/api/v1/cms/media-assistant` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/media-assistant`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/media-assistant?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/media-assistant/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/media-assistant` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/media-assistant/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/media-assistant/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/media-assistant/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "alt_text_caption_transcript_suggestions": "example-value",
  "tagging": "example-value",
  "crops": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-media-assistant`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-media-assistant.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.media.assistant.list`, `cms.media.assistant.get`, `cms.media.assistant.create`, `cms.media.assistant.update`, and `cms.media.assistant.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsMediaAssistantResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `alt_text_caption_transcript_suggestions`, `tagging`, `crops`, `rights_warnings`, `human_review`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms media assistant API
  version: 1.0.0
paths:
  /api/v1/cms/media-assistant:
    get:
      operationId: cms.media.assistant.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsMediaAssistantResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-media-assistant`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-media-assistant`.
