# CMS: Analytics Integration API

## Canonical one-to-one API module specification

**API package:** `module-cms-analytics-integration-api`  
**Matching domain module:** `cms-analytics-integration`  
**Application:** CMS  
**Source feature:** [Analytics Integration](../../features/cms/analytics-integration.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Analytics Integration domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Canonical content/view/conversion events
- consent-aware analytics mappings
- dashboards
- Boilerplate analytics adapters
...
- Use `/api/v1/cms/analytics-integration` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/analytics-integration`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/analytics-integration?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/analytics-integration/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/analytics-integration` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/analytics-integration/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/analytics-integration/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/analytics-integration/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "canonical_content_view_conversion_events": "example-value",
  "consent_aware_analytics_mappings": "example-value",
  "dashboards": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-analytics-integration`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-analytics-integration.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.analytics.integration.list`, `cms.analytics.integration.get`, `cms.analytics.integration.create`, `cms.analytics.integration.update`, and `cms.analytics.integration.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsAnalyticsIntegrationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `canonical_content_view_conversion_events`, `consent_aware_analytics_mappings`, `dashboards`, `boilerplate_analytics_adapters`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms analytics integration API
  version: 1.0.0
paths:
  /api/v1/cms/analytics-integration:
    get:
      operationId: cms.analytics.integration.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CmsAnalyticsIntegrationResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: cms-analytics-integration
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsAnalyticsIntegrationResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-analytics-integration`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-analytics-integration`.
