...
**API package:** `module-cms-polls-and-surveys-api`  
**Matching domain module:** `cms-polls-and-surveys`  
**Application:** CMS  
**Source feature:** [Polls and Surveys](../../features/cms/polls-and-surveys.md)  
...
- Questions
- branching
- anonymous/authenticated response policy
- schedules
- results
- exports
- privacy controls
...
- Use `/api/v1/cms/polls-and-surveys` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/polls-and-surveys`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/polls-and-surveys?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/polls-and-surveys/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/polls-and-surveys` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/polls-and-surveys/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/polls-and-surveys/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/polls-and-surveys/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
```json
{
  ...
  "anonymous_authenticated_response_policy": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-polls-and-surveys`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-polls-and-surveys.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.polls.and.surveys.list`, `cms.polls.and.surveys.get`, `cms.polls.and.surveys.create`, `cms.polls.and.surveys.update`, and `cms.polls.and.surveys.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsPollsAndSurveysResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `questions`, `branching`, `anonymous_authenticated_response_policy`, `schedules`, `results`, `exports`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsPollsAndSurveysResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-polls-and-surveys`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-polls-and-surveys`.
