# CRM: Lead Qualification API

## Canonical one-to-one API module specification

**API package:** `module-crm-lead-qualification-api`  
**Matching domain module:** `crm-lead-qualification`  
**Application:** CRM  
**Source feature:** [Lead Qualification](../../features/crm/lead-qualification.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Lead Qualification domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Lifecycle stages
- fit/engagement scoring
- MQL/PQL/SQL/service-qualified rules
...
- Use `/api/v1/crm/lead-qualification` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/lead-qualification`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/lead-qualification?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/lead-qualification/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/lead-qualification` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/lead-qualification/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/lead-qualification/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/lead-qualification/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "lifecycle_stages": "example-value",
  "fit_engagement_scoring": "example-value",
  "mql_pql_sql_service_qualified_rules": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-lead-qualification`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-lead-qualification.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.lead.qualification.list`, `crm.lead.qualification.get`, `crm.lead.qualification.create`, `crm.lead.qualification.update`, and `crm.lead.qualification.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmLeadQualificationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `lifecycle_stages`, `fit_engagement_scoring`, `mql_pql_sql_service_qualified_rules`, `qualification_frameworks`, `disqualification`, `nurture`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm lead qualification API
  version: 1.0.0
paths:
  /api/v1/crm/lead-qualification:
    get:
      operationId: crm.lead.qualification.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmLeadQualificationResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-lead-qualification`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-lead-qualification`.
