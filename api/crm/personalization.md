# CRM: Personalization API

## Canonical one-to-one API module specification

**API package:** `module-crm-personalization-api`  
**Matching domain module:** `crm-personalization`  
**Application:** CRM  
**Source feature:** [Personalization](../../features/crm/personalization.md)  
...
- Rules and decisioning for content
- offer
- channel
- send time
- locale
- lifecycle
- customer attributes with holdouts and fallback
...
- Use `/api/v1/crm/personalization` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/personalization`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/personalization?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/personalization/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/personalization` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/personalization/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/personalization/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/personalization/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "rules_and_decisioning_for_content": "example-value",
  "offer": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-personalization`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-personalization.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.personalization.list`, `crm.personalization.get`, `crm.personalization.create`, `crm.personalization.update`, and `crm.personalization.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmPersonalizationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `rules_and_decisioning_for_content`, `offer`, `channel`, `send_time`, `locale`, `lifecycle`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm personalization API
  version: 1.0.0
paths:
  /api/v1/crm/personalization:
    get:
      operationId: crm.personalization.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmPersonalizationResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-personalization`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-personalization`.
