# CRM: Deal Registration API

## Canonical one-to-one API module specification

**API package:** `module-crm-deal-registration-api`  
**Matching domain module:** `crm-deal-registration`  
**Application:** CRM  
**Source feature:** [Deal Registration](../../features/crm/deal-registration.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Deal Registration domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Partner-submitted leads/deals
...
- Use `/api/v1/crm/deal-registration` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/deal-registration`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/deal-registration?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/deal-registration/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/deal-registration` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/deal-registration/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/deal-registration/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/deal-registration/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "partner_submitted_leads_deals": "example-value",
  "duplicate_conflict_checks": "example-value",
  "territory_rules": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-deal-registration`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-deal-registration.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.deal.registration.list`, `crm.deal.registration.get`, `crm.deal.registration.create`, `crm.deal.registration.update`, and `crm.deal.registration.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmDealRegistrationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `partner_submitted_leads_deals`, `duplicate_conflict_checks`, `territory_rules`, `approval`, `protection_windows`, `collaboration`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm deal registration API
  version: 1.0.0
paths:
  /api/v1/crm/deal-registration:
    get:
      operationId: crm.deal.registration.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmDealRegistrationResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-deal-registration`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-deal-registration`.
