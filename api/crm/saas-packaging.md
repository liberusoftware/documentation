# CRM: SaaS Packaging API

## Canonical one-to-one API module specification

**API package:** `module-crm-saas-packaging-api`  
**Matching domain module:** `crm-saas-packaging`  
**Application:** CRM  
**Source feature:** [SaaS Packaging](../../features/crm/saas-packaging.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the SaaS Packaging domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/saas-packaging` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/saas-packaging`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/saas-packaging?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/saas-packaging/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/saas-packaging` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/saas-packaging/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/saas-packaging/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/saas-packaging/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
{
  "plans": "example-value",
  "feature_module_entitlements": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-saas-packaging`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-saas-packaging.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.saas.packaging.list`, `crm.saas.packaging.get`, `crm.saas.packaging.create`, `crm.saas.packaging.update`, and `crm.saas.packaging.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmSaasPackagingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `plans`, `feature_module_entitlements`, `limits`, `trials`, `signup_provisioning`, `upgrade_downgrade`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm saas packaging API
  version: 1.0.0
paths:
  /api/v1/crm/saas-packaging:
    get:
      operationId: crm.saas.packaging.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmSaasPackagingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-saas-packaging`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-saas-packaging`.
