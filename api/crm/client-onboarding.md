# CRM: Client Onboarding API

## Canonical one-to-one API module specification

**API package:** `module-crm-client-onboarding-api`  
...
This optional API presentation package exposes approved HTTP operations for the Client Onboarding domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Intake
- domain/provider connections
- data import
...
- Use `/api/v1/crm/client-onboarding` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/client-onboarding`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/client-onboarding?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/client-onboarding/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/client-onboarding` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/client-onboarding/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/client-onboarding/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/client-onboarding/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "intake": "example-value",
  "domain_provider_connections": "example-value",
  "data_import": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-client-onboarding`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-client-onboarding.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.client.onboarding.list`, `crm.client.onboarding.get`, `crm.client.onboarding.create`, `crm.client.onboarding.update`, and `crm.client.onboarding.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmClientOnboardingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `intake`, `domain_provider_connections`, `data_import`, `snapshot_application`, `verification`, `launch_checklist`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm client onboarding API
  version: 1.0.0
paths:
  /api/v1/crm/client-onboarding:
    get:
      operationId: crm.client.onboarding.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmClientOnboardingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-client-onboarding`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-client-onboarding`.
