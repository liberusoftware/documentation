# CRM: Feedback and Voice of Customer API

## Canonical one-to-one API module specification

**API package:** `module-crm-feedback-and-voice-of-customer-api`  
...
This optional API presentation package exposes approved HTTP operations for the Feedback and Voice of Customer domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/feedback-and-voice-of-customer` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/feedback-and-voice-of-customer`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/feedback-and-voice-of-customer?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/feedback-and-voice-of-customer/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/feedback-and-voice-of-customer` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/feedback-and-voice-of-customer/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/feedback-and-voice-of-customer/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/feedback-and-voice-of-customer/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-feedback-and-voice-of-customer`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-feedback-and-voice-of-customer.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.feedback.and.voice.of.customer.list`, `crm.feedback.and.voice.of.customer.get`, `crm.feedback.and.voice.of.customer.create`, `crm.feedback.and.voice.of.customer.update`, and `crm.feedback.and.voice.of.customer.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmFeedbackAndVoiceOfCustomerResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `csat`, `nps`, `ces`, `custom_surveys`, `sampling`, `delivery`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm feedback and voice of customer API
  version: 1.0.0
paths:
  /api/v1/crm/feedback-and-voice-of-customer:
    get:
      operationId: crm.feedback.and.voice.of.customer.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmFeedbackAndVoiceOfCustomerResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-feedback-and-voice-of-customer
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmFeedbackAndVoiceOfCustomerResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-feedback-and-voice-of-customer`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-feedback-and-voice-of-customer`.
