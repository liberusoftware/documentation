# CRM: Usage Wallet and Rebilling API

## Canonical one-to-one API module specification

**API package:** `module-crm-usage-wallet-and-rebilling-api`  
**Matching domain module:** `crm-usage-wallet-and-rebilling`  
**Application:** CRM  
**Source feature:** [Usage Wallet and Rebilling](../../features/crm/usage-wallet-and-rebilling.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Usage Wallet and Rebilling domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/usage-wallet-and-rebilling` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/usage-wallet-and-rebilling`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/usage-wallet-and-rebilling?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/usage-wallet-and-rebilling/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/usage-wallet-and-rebilling` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/usage-wallet-and-rebilling/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/usage-wallet-and-rebilling/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/usage-wallet-and-rebilling/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "provider_usage_imports": "example-value",
  "wallets_credits": "example-value",
  "threshold_reloads": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-usage-wallet-and-rebilling`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-usage-wallet-and-rebilling.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.usage.wallet.and.rebilling.list`, `crm.usage.wallet.and.rebilling.get`, `crm.usage.wallet.and.rebilling.create`, `crm.usage.wallet.and.rebilling.update`, and `crm.usage.wallet.and.rebilling.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmUsageWalletAndRebillingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `provider_usage_imports`, `wallets_credits`, `threshold_reloads`, `cost_markup_rules`, `client_charges`, `exceptions`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm usage wallet and rebilling API
  version: 1.0.0
paths:
  /api/v1/crm/usage-wallet-and-rebilling:
    get:
      operationId: crm.usage.wallet.and.rebilling.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmUsageWalletAndRebillingResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-usage-wallet-and-rebilling
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmUsageWalletAndRebillingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-usage-wallet-and-rebilling`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-usage-wallet-and-rebilling`.
