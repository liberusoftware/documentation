...
**API package:** `module-crm-web-intent-api`  
**Matching domain module:** `crm-web-intent`  
**Application:** CRM  
**Source feature:** [Web Intent](../../features/crm/web-intent.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Web Intent domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- First-party visits
- account identification adapters
...
- Use `/api/v1/crm/web-intent` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/web-intent`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/web-intent?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/web-intent/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/web-intent` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/web-intent/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/web-intent/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/web-intent/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "first_party_visits": "example-value",
  "account_identification_adapters": "example-value",
  "page_content_engagement": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-web-intent`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-web-intent.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.web.intent.list`, `crm.web.intent.get`, `crm.web.intent.create`, `crm.web.intent.update`, and `crm.web.intent.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmWebIntentResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `first_party_visits`, `account_identification_adapters`, `page_content_engagement`, `scoring_signals`, `alerts`, `buyer_intent`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm web intent API
  version: 1.0.0
paths:
  /api/v1/crm/web-intent:
    get:
      operationId: crm.web.intent.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmWebIntentResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-web-intent`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-web-intent`.
