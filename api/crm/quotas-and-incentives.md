# CRM: Quotas and Incentives API

## Canonical one-to-one API module specification

**API package:** `module-crm-quotas-and-incentives-api`  
**Matching domain module:** `crm-quotas-and-incentives`  
**Application:** CRM  
**Source feature:** [Quotas and Incentives](../../features/crm/quotas-and-incentives.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Quotas and Incentives domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/quotas-and-incentives` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/quotas-and-incentives`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/quotas-and-incentives?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/quotas-and-incentives/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/quotas-and-incentives` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/quotas-and-incentives/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/quotas-and-incentives/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/quotas-and-incentives/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-quotas-and-incentives`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-quotas-and-incentives.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.quotas.and.incentives.list`, `crm.quotas.and.incentives.get`, `crm.quotas.and.incentives.create`, `crm.quotas.and.incentives.update`, and `crm.quotas.and.incentives.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmQuotasAndIncentivesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `quotas`, `ramps`, `crediting`, `splits`, `territories`, `attainment`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm quotas and incentives API
  version: 1.0.0
paths:
  /api/v1/crm/quotas-and-incentives:
    get:
      operationId: crm.quotas.and.incentives.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmQuotasAndIncentivesResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-quotas-and-incentives
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmQuotasAndIncentivesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-quotas-and-incentives`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-quotas-and-incentives`.
