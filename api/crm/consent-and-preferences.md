# CRM: Consent and Preferences API

## Canonical one-to-one API module specification

**API package:** `module-crm-consent-and-preferences-api`  
**Matching domain module:** `crm-consent-and-preferences`  
**Application:** CRM  
**Source feature:** [Consent and Preferences](../../features/crm/consent-and-preferences.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Consent and Preferences domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Lawful basis
- subscriptions
- channel/topic preferences
- suppression
- quiet hours
- recording consent
- proof
...
- Use `/api/v1/crm/consent-and-preferences` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/consent-and-preferences`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/consent-and-preferences?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/consent-and-preferences/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/consent-and-preferences` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/consent-and-preferences/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/consent-and-preferences/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/consent-and-preferences/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "lawful_basis": "example-value",
  "subscriptions": "example-value",
  "channel_topic_preferences": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-consent-and-preferences`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-consent-and-preferences.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.consent.and.preferences.list`, `crm.consent.and.preferences.get`, `crm.consent.and.preferences.create`, `crm.consent.and.preferences.update`, and `crm.consent.and.preferences.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmConsentAndPreferencesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `lawful_basis`, `subscriptions`, `channel_topic_preferences`, `suppression`, `quiet_hours`, `recording_consent`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm consent and preferences API
  version: 1.0.0
paths:
  /api/v1/crm/consent-and-preferences:
    get:
      operationId: crm.consent.and.preferences.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmConsentAndPreferencesResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-consent-and-preferences
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmConsentAndPreferencesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-consent-and-preferences`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-consent-and-preferences`.
