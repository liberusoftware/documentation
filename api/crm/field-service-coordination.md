# CRM: Field Service Coordination API

## Canonical one-to-one API module specification

**API package:** `module-crm-field-service-coordination-api`  
**Matching domain module:** `crm-field-service-coordination`  
**Application:** CRM  
**Source feature:** [Field Service Coordination](../../features/crm/field-service-coordination.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Field Service Coordination domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- dispatch references
- technician/contractor visibility
- asset/service history
- Maintenance module handoff
...
- Use `/api/v1/crm/field-service-coordination` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/field-service-coordination`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/field-service-coordination?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/field-service-coordination/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/field-service-coordination` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/field-service-coordination/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/field-service-coordination/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/field-service-coordination/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "work_types": "example-value",
  "service_appointments": "example-value",
  "dispatch_references": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-field-service-coordination`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-field-service-coordination.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.field.service.coordination.list`, `crm.field.service.coordination.get`, `crm.field.service.coordination.create`, `crm.field.service.coordination.update`, and `crm.field.service.coordination.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmFieldServiceCoordinationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `work_types`, `service_appointments`, `dispatch_references`, `technician_contractor_visibility`, `asset_service_history`, `maintenance_module_handoff`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm field service coordination API
  version: 1.0.0
paths:
  /api/v1/crm/field-service-coordination:
    get:
      operationId: crm.field.service.coordination.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmFieldServiceCoordinationResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-field-service-coordination
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmFieldServiceCoordinationResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-field-service-coordination`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-field-service-coordination`.
