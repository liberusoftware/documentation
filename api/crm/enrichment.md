...
**API package:** `module-crm-enrichment-api`  
**Matching domain module:** `crm-enrichment`  
**Application:** CRM  
**Source feature:** [Enrichment](../../features/crm/enrichment.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Enrichment domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Company/contact firmographic
- demographic
- technographic
- social
- verification
...
- Use `/api/v1/crm/enrichment` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/enrichment`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/enrichment?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/enrichment/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/enrichment` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/enrichment/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/enrichment/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/enrichment/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "company_contact_firmographic": "example-value",
  "demographic": "example-value",
  "technographic": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-enrichment`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-enrichment.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.enrichment.list`, `crm.enrichment.get`, `crm.enrichment.create`, `crm.enrichment.update`, and `crm.enrichment.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmEnrichmentResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `company_contact_firmographic`, `demographic`, `technographic`, `social`, `verification`, `change_monitoring`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm enrichment API
  version: 1.0.0
paths:
  /api/v1/crm/enrichment:
    get:
      operationId: crm.enrichment.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmEnrichmentResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-enrichment`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-enrichment`.
