# CRM: Marketing Development Funds API

## Canonical one-to-one API module specification

**API package:** `module-crm-marketing-development-funds-api`  
**Matching domain module:** `crm-marketing-development-funds`  
**Application:** CRM  
**Source feature:** [Marketing Development Funds](../../features/crm/marketing-development-funds.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Marketing Development Funds domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/marketing-development-funds` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/marketing-development-funds`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/marketing-development-funds?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/marketing-development-funds/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/marketing-development-funds` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/marketing-development-funds/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/marketing-development-funds/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/marketing-development-funds/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-marketing-development-funds`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-marketing-development-funds.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.marketing.development.funds.list`, `crm.marketing.development.funds.get`, `crm.marketing.development.funds.create`, `crm.marketing.development.funds.update`, and `crm.marketing.development.funds.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmMarketingDevelopmentFundsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `requests`, `budgets`, `plans`, `approvals`, `evidence`, `claims`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm marketing development funds API
  version: 1.0.0
paths:
  /api/v1/crm/marketing-development-funds:
    get:
      operationId: crm.marketing.development.funds.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmMarketingDevelopmentFundsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-marketing-development-funds
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmMarketingDevelopmentFundsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-marketing-development-funds`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-marketing-development-funds`.
