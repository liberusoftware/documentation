# CRM: Marketing Agent API

## Canonical one-to-one API module specification

**API package:** `module-crm-marketing-agent-api`  
**Matching domain module:** `crm-marketing-agent`  
**Application:** CRM  
**Source feature:** [Marketing Agent](../../features/crm/marketing-agent.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Marketing Agent domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Audience/campaign/content assistance
- variations
- journey recommendations
- brand/consent checks
- experiments
- approval workflow
...
- Use `/api/v1/crm/marketing-agent` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/marketing-agent`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/marketing-agent?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/marketing-agent/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/marketing-agent` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/marketing-agent/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/marketing-agent/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/marketing-agent/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "audience_campaign_content_assistance": "example-value",
  "variations": "example-value",
  "journey_recommendations": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-marketing-agent`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-marketing-agent.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.marketing.agent.list`, `crm.marketing.agent.get`, `crm.marketing.agent.create`, `crm.marketing.agent.update`, and `crm.marketing.agent.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmMarketingAgentResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `audience_campaign_content_assistance`, `variations`, `journey_recommendations`, `brand_consent_checks`, `experiments`, `approval_workflow`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm marketing agent API
  version: 1.0.0
paths:
  /api/v1/crm/marketing-agent:
    get:
      operationId: crm.marketing.agent.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmMarketingAgentResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-marketing-agent`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-marketing-agent`.
