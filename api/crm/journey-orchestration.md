# CRM: Journey Orchestration API

## Canonical one-to-one API module specification

**API package:** `module-crm-journey-orchestration-api`  
...
This optional API presentation package exposes approved HTTP operations for the Journey Orchestration domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Triggered/scheduled journeys
- branching
- waits
- goals
- re-entry
...
- Use `/api/v1/crm/journey-orchestration` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/journey-orchestration`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/journey-orchestration?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/journey-orchestration/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/journey-orchestration` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/journey-orchestration/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/journey-orchestration/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/journey-orchestration/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "triggered_scheduled_journeys": "example-value",
  "branching": "example-value",
  "waits": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-journey-orchestration`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-journey-orchestration.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.journey.orchestration.list`, `crm.journey.orchestration.get`, `crm.journey.orchestration.create`, `crm.journey.orchestration.update`, and `crm.journey.orchestration.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmJourneyOrchestrationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `triggered_scheduled_journeys`, `branching`, `waits`, `goals`, `re_entry`, `suppression`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm journey orchestration API
  version: 1.0.0
paths:
  /api/v1/crm/journey-orchestration:
    get:
      operationId: crm.journey.orchestration.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmJourneyOrchestrationResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-journey-orchestration
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmJourneyOrchestrationResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-journey-orchestration`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-journey-orchestration`.
