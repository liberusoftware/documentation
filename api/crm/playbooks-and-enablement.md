# CRM: Playbooks and Enablement API

## Canonical one-to-one API module specification

**API package:** `module-crm-playbooks-and-enablement-api`  
**Matching domain module:** `crm-playbooks-and-enablement`  
**Application:** CRM  
**Source feature:** [Playbooks and Enablement](../../features/crm/playbooks-and-enablement.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Playbooks and Enablement domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/playbooks-and-enablement` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/playbooks-and-enablement`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/playbooks-and-enablement?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/playbooks-and-enablement/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/playbooks-and-enablement` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/playbooks-and-enablement/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/playbooks-and-enablement/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/playbooks-and-enablement/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "guided_scripts": "example-value",
  "qualification_cards": "example-value",
  "battlecards": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-playbooks-and-enablement`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-playbooks-and-enablement.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.playbooks.and.enablement.list`, `crm.playbooks.and.enablement.get`, `crm.playbooks.and.enablement.create`, `crm.playbooks.and.enablement.update`, and `crm.playbooks.and.enablement.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmPlaybooksAndEnablementResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `guided_scripts`, `qualification_cards`, `battlecards`, `onboarding_content`, `required_evidence`, `coaching_checklists`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm playbooks and enablement API
  version: 1.0.0
paths:
  /api/v1/crm/playbooks-and-enablement:
    get:
      operationId: crm.playbooks.and.enablement.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmPlaybooksAndEnablementResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-playbooks-and-enablement
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmPlaybooksAndEnablementResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-playbooks-and-enablement`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-playbooks-and-enablement`.
