# CRM: Orders and Payments Workspace API

## Canonical one-to-one API module specification

**API package:** `module-crm-orders-and-payments-workspace-api`  
**Matching domain module:** `crm-orders-and-payments-workspace`  
**Application:** CRM  
**Source feature:** [Orders and Payments Workspace](../../features/crm/orders-and-payments-workspace.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Orders and Payments Workspace domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Payment links
- deposits
- order/subscription references
- invoice/payment status
- refunds/disputes visibility
- Billing/Ecommerce handoff
...
- Use `/api/v1/crm/orders-and-payments-workspace` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/orders-and-payments-workspace`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/orders-and-payments-workspace?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/orders-and-payments-workspace/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/orders-and-payments-workspace` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/orders-and-payments-workspace/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/orders-and-payments-workspace/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/orders-and-payments-workspace/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "payment_links": "example-value",
  "deposits": "example-value",
  "order_subscription_references": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-orders-and-payments-workspace`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-orders-and-payments-workspace.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.orders.and.payments.workspace.list`, `crm.orders.and.payments.workspace.get`, `crm.orders.and.payments.workspace.create`, `crm.orders.and.payments.workspace.update`, and `crm.orders.and.payments.workspace.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmOrdersAndPaymentsWorkspaceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `payment_links`, `deposits`, `order_subscription_references`, `invoice_payment_status`, `refunds_disputes_visibility`, `billing_ecommerce_handoff`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm orders and payments workspace API
  version: 1.0.0
paths:
  /api/v1/crm/orders-and-payments-workspace:
    get:
      operationId: crm.orders.and.payments.workspace.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmOrdersAndPaymentsWorkspaceResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-orders-and-payments-workspace
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmOrdersAndPaymentsWorkspaceResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-orders-and-payments-workspace`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-orders-and-payments-workspace`.
