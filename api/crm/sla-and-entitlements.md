# CRM: SLA and Entitlements API

## Canonical one-to-one API module specification

**API package:** `module-crm-sla-and-entitlements-api`  
**Matching domain module:** `crm-sla-and-entitlements`  
**Application:** CRM  
**Source feature:** [SLA and Entitlements](../../features/crm/sla-and-entitlements.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the SLA and Entitlements domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/sla-and-entitlements` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/sla-and-entitlements`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/sla-and-entitlements?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/sla-and-entitlements/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/sla-and-entitlements` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/sla-and-entitlements/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/sla-and-entitlements/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/sla-and-entitlements/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "service_contracts": "example-value",
  "coverage": "example-value",
  "business_calendars": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-sla-and-entitlements`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-sla-and-entitlements.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.sla.and.entitlements.list`, `crm.sla.and.entitlements.get`, `crm.sla.and.entitlements.create`, `crm.sla.and.entitlements.update`, and `crm.sla.and.entitlements.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmSlaAndEntitlementsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `service_contracts`, `coverage`, `business_calendars`, `response_resolution_targets`, `milestones`, `pauses`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm sla and entitlements API
  version: 1.0.0
paths:
  /api/v1/crm/sla-and-entitlements:
    get:
      operationId: crm.sla.and.entitlements.list
      security:
        - sanctum: []
      parameters:
  ...
  schemas:
    CrmSlaAndEntitlementsResource:
      ...
      properties:
        ...
        type:
          type: string
          const: crm-sla-and-entitlements
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmSlaAndEntitlementsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-sla-and-entitlements`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-sla-and-entitlements`.
