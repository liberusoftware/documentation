# CRM: Goals and Performance API

## Canonical one-to-one API module specification

**API package:** `module-crm-goals-and-performance-api`  
**Matching domain module:** `crm-goals-and-performance`  
**Application:** CRM  
**Source feature:** [Goals and Performance](../../features/crm/goals-and-performance.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Goals and Performance domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Individual/team/company goals
- activity/outcome targets
- scorecards
- leaderboards
- reviews
- coaching plans
...
- Use `/api/v1/crm/goals-and-performance` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/goals-and-performance`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/goals-and-performance?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/goals-and-performance/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/goals-and-performance` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/goals-and-performance/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/goals-and-performance/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/goals-and-performance/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "individual_team_company_goals": "example-value",
  "activity_outcome_targets": "example-value",
  "scorecards": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-goals-and-performance`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-goals-and-performance.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.goals.and.performance.list`, `crm.goals.and.performance.get`, `crm.goals.and.performance.create`, `crm.goals.and.performance.update`, and `crm.goals.and.performance.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmGoalsAndPerformanceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `individual_team_company_goals`, `activity_outcome_targets`, `scorecards`, `leaderboards`, `reviews`, `coaching_plans`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm goals and performance API
  version: 1.0.0
paths:
  /api/v1/crm/goals-and-performance:
    get:
      operationId: crm.goals.and.performance.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmGoalsAndPerformanceResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-goals-and-performance
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmGoalsAndPerformanceResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-goals-and-performance`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-goals-and-performance`.
