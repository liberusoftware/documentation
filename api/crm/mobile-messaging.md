# CRM: Mobile Messaging API

## Canonical one-to-one API module specification

**API package:** `module-crm-mobile-messaging-api`  
**Matching domain module:** `crm-mobile-messaging`  
**Application:** CRM  
**Source feature:** [Mobile Messaging](../../features/crm/mobile-messaging.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Mobile Messaging domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Consent-aware SMS/MMS/WhatsApp/push campaigns
- templates
- short links
- keywords
- opt-in/out
...
- Use `/api/v1/crm/mobile-messaging` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/mobile-messaging`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/mobile-messaging?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/mobile-messaging/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/mobile-messaging` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/mobile-messaging/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/mobile-messaging/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/mobile-messaging/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "consent_aware_sms_mms_whatsapp_push_campaigns": "example-value",
  "templates": "example-value",
  "short_links": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-mobile-messaging`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-mobile-messaging.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.mobile.messaging.list`, `crm.mobile.messaging.get`, `crm.mobile.messaging.create`, `crm.mobile.messaging.update`, and `crm.mobile.messaging.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmMobileMessagingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `consent_aware_sms_mms_whatsapp_push_campaigns`, `templates`, `short_links`, `keywords`, `opt_in_out`, `quiet_hours`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm mobile messaging API
  version: 1.0.0
paths:
  /api/v1/crm/mobile-messaging:
    get:
      operationId: crm.mobile.messaging.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmMobileMessagingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-mobile-messaging`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-mobile-messaging`.
