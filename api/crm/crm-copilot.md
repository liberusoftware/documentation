...
**API package:** `module-crm-copilot-api`  
**Matching domain module:** `crm-copilot`  
**Application:** CRM  
**Source feature:** [CRM Copilot](../../features/crm/crm-copilot.md)  
**Architecture:** [API.md](../../API.md) · [MODULES.md](../../MODULES.md) · [TESTING.md](../../TESTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the CRM Copilot domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- record updates
- task creation
- action confirmation using the Automation AI Gateway
...
- Use `/api/v1/crm/crm-copilot` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/crm-copilot`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/crm-copilot?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/crm-copilot/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/crm-copilot` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/crm-copilot/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/crm-copilot/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/crm-copilot/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-copilot`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-copilot.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.copilot.list`, `crm.copilot.get`, `crm.copilot.create`, `crm.copilot.update`, and `crm.copilot.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmCopilotResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `permission_bounded_search`, `summaries`, `preparation`, `drafting`, `record_updates`, `task_creation`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
info:
  ...
  version: 1.0.0
paths:
  /api/v1/crm/crm-copilot:
    get:
      operationId: crm.copilot.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmCopilotResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-copilot`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-copilot`.
