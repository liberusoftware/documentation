...
**API package:** `module-accounting-bank-accounts-api`  
**Matching domain module:** `accounting-bank-accounts`  
**Application:** Accounting  
**Source feature:** [Bank Accounts](../features/bank-accounts.md)  
**Architecture:** [API.md](../ACCOUNTING.md) · [MODULES.md](../ACCOUNTING.md) · [TESTING.md](../ACCOUNTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Bank Accounts domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Bank/current/savings/credit/loan/cash accounts
- ownership
- currency
- opening data
- feeds
...
- Use `/api/v1/accounting/bank-accounts` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/bank-accounts`
...
| -------- | ----------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/accounting/bank-accounts?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/accounting/bank-accounts/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/accounting/bank-accounts`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/accounting/bank-accounts/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/accounting/bank-accounts/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/accounting/bank-accounts/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "bank_current_savings_credit_loan_cash_accounts": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-bank-accounts`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-bank-accounts.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.bank.accounts.list`, `accounting.bank.accounts.get`, `accounting.bank.accounts.create`, `accounting.bank.accounts.update`, and `accounting.bank.accounts.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingBankAccountsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `bank_current_savings_credit_loan_cash_accounts`, `ownership`, `currency`, `opening_data`, `feeds`, `lifecycle`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting bank accounts API
  version: 1.0.0
paths:
  /api/v1/accounting/bank-accounts:
    get:
      operationId: accounting.bank.accounts.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingBankAccountsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-bank-accounts
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/AccountingBankAccountsResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-bank-accounts`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-bank-accounts`.
