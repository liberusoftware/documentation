# Accounting: Customer Payments API

## Canonical one-to-one API module specification

**API package:** `module-accounting-customer-payments-api`  
...
This optional API presentation package exposes approved HTTP operations for the Customer Payments domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/customer-payments` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/customer-payments`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/customer-payments?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/customer-payments/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/customer-payments` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/customer-payments/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/customer-payments/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/customer-payments/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
```json
{
  ...
  "gateway_bank_references": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-customer-payments`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-customer-payments.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.customer.payments.list`, `accounting.customer.payments.get`, `accounting.customer.payments.create`, `accounting.customer.payments.update`, and `accounting.customer.payments.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingCustomerPaymentsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `receipts`, `payment_links`, `gateway_bank_references`, `deposits`, `undeposited_funds`, `allocations`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting customer payments API
  version: 1.0.0
paths:
  /api/v1/accounting/customer-payments:
    get:
      operationId: accounting.customer.payments.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingCustomerPaymentsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-customer-payments
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingCustomerPaymentsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-customer-payments`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-customer-payments`.
