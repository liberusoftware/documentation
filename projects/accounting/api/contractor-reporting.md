# Accounting: Contractor Reporting API

## Canonical one-to-one API module specification

**API package:** `module-accounting-contractor-reporting-api`  
**Matching domain module:** `accounting-contractor-reporting`  
**Application:** Accounting  
**Source feature:** [Contractor Reporting](../features/contractor-reporting.md)  
**Architecture:** [API.md](../ACCOUNTING.md) · [MODULES.md](../ACCOUNTING.md) · [TESTING.md](../ACCOUNTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Contractor Reporting domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- 1099-style thresholds
- classifications
- payee validation
- forms
- electronic filing adapters
- corrections
- evidence
...
- Use `/api/v1/accounting/contractor-reporting` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/contractor-reporting`
...
| -------- | ------------------------------------------------------------------------ | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/accounting/contractor-reporting?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/accounting/contractor-reporting/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/accounting/contractor-reporting`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/accounting/contractor-reporting/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/accounting/contractor-reporting/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/accounting/contractor-reporting/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "1099_style_thresholds": "example-value",
  "classifications": "example-value",
  "payee_validation": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-contractor-reporting`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-contractor-reporting.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.contractor.reporting.list`, `accounting.contractor.reporting.get`, `accounting.contractor.reporting.create`, `accounting.contractor.reporting.update`, and `accounting.contractor.reporting.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingContractorReportingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `1099_style_thresholds`, `classifications`, `payee_validation`, `forms`, `electronic_filing_adapters`, `corrections`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting contractor reporting API
  version: 1.0.0
paths:
  /api/v1/accounting/contractor-reporting:
    get:
      operationId: accounting.contractor.reporting.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingContractorReportingResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-contractor-reporting
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/AccountingContractorReportingResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-contractor-reporting`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-contractor-reporting`.
