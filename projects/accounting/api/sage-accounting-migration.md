# Accounting: Sage Accounting Migration API

## Canonical one-to-one API module specification

**API package:** `module-accounting-sage-accounting-migration-api`  
**Matching domain module:** `accounting-sage-accounting-migration`  
**Application:** Accounting  
**Source feature:** [Sage Accounting Migration](../features/sage-accounting-migration.md)  
**Architecture:** [API.md](../ACCOUNTING.md) · [MODULES.md](../ACCOUNTING.md) · [TESTING.md](../ACCOUNTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Sage Accounting Migration domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/sage-accounting-migration` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/sage-accounting-migration`
...
| -------- | ----------------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/accounting/sage-accounting-migration?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/accounting/sage-accounting-migration/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/accounting/sage-accounting-migration`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/accounting/sage-accounting-migration/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/accounting/sage-accounting-migration/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/accounting/sage-accounting-migration/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "chart_analysis_types": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-sage-accounting-migration`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-sage-accounting-migration.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.sage.accounting.migration.list`, `accounting.sage.accounting.migration.get`, `accounting.sage.accounting.migration.create`, `accounting.sage.accounting.migration.update`, and `accounting.sage.accounting.migration.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingSageAccountingMigrationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `chart_analysis_types`, `contacts`, `products`, `sales_purchases`, `payments`, `bank`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting sage accounting migration API
  version: 1.0.0
paths:
  /api/v1/accounting/sage-accounting-migration:
    get:
      operationId: accounting.sage.accounting.migration.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingSageAccountingMigrationResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-sage-accounting-migration
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/AccountingSageAccountingMigrationResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-sage-accounting-migration`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-sage-accounting-migration`.
