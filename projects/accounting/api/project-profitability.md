# Accounting: Project Profitability API

## Canonical one-to-one API module specification

**API package:** `module-accounting-project-profitability-api`  
**Matching domain module:** `accounting-project-profitability`  
**Application:** Accounting  
**Source feature:** [Project Profitability](../features/project-profitability.md)  
**Architecture:** [API.md](../ACCOUNTING.md) · [MODULES.md](../ACCOUNTING.md) · [TESTING.md](../ACCOUNTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Project Profitability domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/project-profitability` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/project-profitability`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/project-profitability?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/project-profitability/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/project-profitability` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/project-profitability/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/project-profitability/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/project-profitability/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-project-profitability`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-project-profitability.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.project.profitability.list`, `accounting.project.profitability.get`, `accounting.project.profitability.create`, `accounting.project.profitability.update`, and `accounting.project.profitability.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingProjectProfitabilityResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `revenue`, `cost`, `margin`, `estimates`, `committed_actual`, `unbilled_wip`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting project profitability API
  version: 1.0.0
paths:
  /api/v1/accounting/project-profitability:
    get:
      operationId: accounting.project.profitability.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingProjectProfitabilityResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-project-profitability
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingProjectProfitabilityResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-project-profitability`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-project-profitability`.
