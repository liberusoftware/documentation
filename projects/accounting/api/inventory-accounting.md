# Accounting: Inventory Accounting API

## Canonical one-to-one API module specification

**API package:** `module-accounting-inventory-accounting-api`  
**Matching domain module:** `accounting-inventory-accounting`  
**Application:** Accounting  
**Source feature:** [Inventory Accounting](../features/inventory-accounting.md)  
**Architecture:** [API.md](../ACCOUNTING.md) · [MODULES.md](../ACCOUNTING.md) · [TESTING.md](../ACCOUNTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Inventory Accounting domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Perpetual/periodic methods
- valuation
- cost layers
- cost of sales
- landed cost
...
- Use `/api/v1/accounting/inventory-accounting` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/inventory-accounting`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/inventory-accounting?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/inventory-accounting/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/inventory-accounting` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/inventory-accounting/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/inventory-accounting/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/inventory-accounting/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "perpetual_periodic_methods": "example-value",
  "valuation": "example-value",
  "cost_layers": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-inventory-accounting`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-inventory-accounting.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.inventory.accounting.list`, `accounting.inventory.accounting.get`, `accounting.inventory.accounting.create`, `accounting.inventory.accounting.update`, and `accounting.inventory.accounting.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingInventoryAccountingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `perpetual_periodic_methods`, `valuation`, `cost_layers`, `cost_of_sales`, `landed_cost`, `adjustments`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting inventory accounting API
  version: 1.0.0
paths:
  /api/v1/accounting/inventory-accounting:
    get:
      operationId: accounting.inventory.accounting.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingInventoryAccountingResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-inventory-accounting
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingInventoryAccountingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-inventory-accounting`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-inventory-accounting`.
