# Accounting: Payment Reconciliation API

## Canonical one-to-one API module specification

**API package:** `module-accounting-payment-reconciliation-api`  
**Matching domain module:** `accounting-payment-reconciliation`  
**Application:** Accounting  
**Source feature:** [Payment Reconciliation](../features/payment-reconciliation.md)  
**Architecture:** [API.md](../ACCOUNTING.md) · [MODULES.md](../ACCOUNTING.md) · [TESTING.md](../ACCOUNTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Payment Reconciliation domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Gateway/merchant settlement imports
- gross-to-net fees/refunds/disputes
- matching
- missing items
- provider drift
...
- Use `/api/v1/accounting/payment-reconciliation` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/payment-reconciliation`
...
| -------- | -------------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/accounting/payment-reconciliation?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/accounting/payment-reconciliation/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/accounting/payment-reconciliation`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/accounting/payment-reconciliation/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/accounting/payment-reconciliation/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/accounting/payment-reconciliation/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "gateway_merchant_settlement_imports": "example-value",
  "gross_to_net_fees_refunds_disputes": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-payment-reconciliation`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-payment-reconciliation.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.payment.reconciliation.list`, `accounting.payment.reconciliation.get`, `accounting.payment.reconciliation.create`, `accounting.payment.reconciliation.update`, and `accounting.payment.reconciliation.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingPaymentReconciliationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `gateway_merchant_settlement_imports`, `gross_to_net_fees_refunds_disputes`, `matching`, `missing_items`, `provider_drift`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
paths:
  /api/v1/accounting/payment-reconciliation:
    get:
      operationId: accounting.payment.reconciliation.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingPaymentReconciliationResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-payment-reconciliation
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/AccountingPaymentReconciliationResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-payment-reconciliation`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-payment-reconciliation`.
