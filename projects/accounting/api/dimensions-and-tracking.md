...
**API package:** `module-accounting-dimensions-and-tracking-api`  
**Matching domain module:** `accounting-dimensions-and-tracking`  
**Application:** Accounting  
**Source feature:** [Dimensions and Tracking](../features/dimensions-and-tracking.md)  
**Architecture:** [API.md](../ACCOUNTING.md) · [MODULES.md](../ACCOUNTING.md) · [TESTING.md](../ACCOUNTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Dimensions and Tracking domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/dimensions-and-tracking` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/dimensions-and-tracking`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/dimensions-and-tracking?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/dimensions-and-tracking/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/dimensions-and-tracking` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/dimensions-and-tracking/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/dimensions-and-tracking/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/dimensions-and-tracking/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "classes_categories": "example-value",
  "locations": "example-value",
  "departments": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-dimensions-and-tracking`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-dimensions-and-tracking.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.dimensions.and.tracking.list`, `accounting.dimensions.and.tracking.get`, `accounting.dimensions.and.tracking.create`, `accounting.dimensions.and.tracking.update`, and `accounting.dimensions.and.tracking.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingDimensionsAndTrackingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `classes_categories`, `locations`, `departments`, `cost_profit_centers`, `projects`, `tags`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting dimensions and tracking API
  version: 1.0.0
paths:
  /api/v1/accounting/dimensions-and-tracking:
    get:
      operationId: accounting.dimensions.and.tracking.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingDimensionsAndTrackingResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-dimensions-and-tracking
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingDimensionsAndTrackingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-dimensions-and-tracking`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-dimensions-and-tracking`.
