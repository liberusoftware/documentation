# Accounting: Supplier Portal API

## Canonical one-to-one API module specification

**API package:** `module-accounting-supplier-portal-api`  
**Matching domain module:** `accounting-supplier-portal`  
**Application:** Accounting  
**Source feature:** [Supplier Portal](../features/supplier-portal.md)  
**Architecture:** [API.md](../ACCOUNTING.md) · [MODULES.md](../ACCOUNTING.md) · [TESTING.md](../ACCOUNTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Supplier Portal domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Profile/bank-change requests
- purchase orders
- invoices
- statements
- disputes
...
- Use `/api/v1/accounting/supplier-portal` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/supplier-portal`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/supplier-portal?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/supplier-portal/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/supplier-portal` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/supplier-portal/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/supplier-portal/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/supplier-portal/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "profile_bank_change_requests": "example-value",
  "purchase_orders": "example-value",
  "invoices": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-supplier-portal`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-supplier-portal.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.supplier.portal.list`, `accounting.supplier.portal.get`, `accounting.supplier.portal.create`, `accounting.supplier.portal.update`, and `accounting.supplier.portal.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingSupplierPortalResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `profile_bank_change_requests`, `purchase_orders`, `invoices`, `statements`, `disputes`, `payment_status`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting supplier portal API
  version: 1.0.0
paths:
  /api/v1/accounting/supplier-portal:
    get:
      operationId: accounting.supplier.portal.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingSupplierPortalResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-supplier-portal
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingSupplierPortalResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-supplier-portal`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-supplier-portal`.
