# Accounting: Goods and Service Receipts API

## Canonical one-to-one API module specification

**API package:** `module-accounting-goods-and-service-receipts-api`  
**Matching domain module:** `accounting-goods-and-service-receipts`  
**Application:** Accounting  
**Source feature:** [Goods and Service Receipts](../features/goods-and-service-receipts.md)  
**Architecture:** [API.md](../ACCOUNTING.md) · [MODULES.md](../ACCOUNTING.md) · [TESTING.md](../ACCOUNTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Goods and Service Receipts domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/goods-and-service-receipts` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/goods-and-service-receipts`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/goods-and-service-receipts?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/goods-and-service-receipts/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/goods-and-service-receipts` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/goods-and-service-receipts/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/goods-and-service-receipts/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/goods-and-service-receipts/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
```json
{
  ...
  "service_confirmation": "example-value",
  "quantities": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-goods-and-service-receipts`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-goods-and-service-receipts.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.goods.and.service.receipts.list`, `accounting.goods.and.service.receipts.get`, `accounting.goods.and.service.receipts.create`, `accounting.goods.and.service.receipts.update`, and `accounting.goods.and.service.receipts.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingGoodsAndServiceReceiptsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `receipts`, `service_confirmation`, `quantities`, `variance`, `returns`, `attachments`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting goods and service receipts API
  version: 1.0.0
paths:
  /api/v1/accounting/goods-and-service-receipts:
    get:
      operationId: accounting.goods.and.service.receipts.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingGoodsAndServiceReceiptsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-goods-and-service-receipts
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingGoodsAndServiceReceiptsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-goods-and-service-receipts`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-goods-and-service-receipts`.
