# Accounting: Custom Report Builder API

## Canonical one-to-one API module specification

**API package:** `module-accounting-custom-report-builder-api`  
...
This optional API presentation package exposes approved HTTP operations for the Custom Report Builder domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Governed measures/dimensions
- filters
- grouping
- formulas
- comparisons
...
- Use `/api/v1/accounting/custom-report-builder` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/custom-report-builder`
...
| -------- | ------------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/accounting/custom-report-builder?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/accounting/custom-report-builder/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/accounting/custom-report-builder`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/accounting/custom-report-builder/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/accounting/custom-report-builder/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/accounting/custom-report-builder/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "governed_measures_dimensions": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-custom-report-builder`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-custom-report-builder.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.custom.report.builder.list`, `accounting.custom.report.builder.get`, `accounting.custom.report.builder.create`, `accounting.custom.report.builder.update`, and `accounting.custom.report.builder.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingCustomReportBuilderResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `governed_measures_dimensions`, `filters`, `grouping`, `formulas`, `comparisons`, `layouts`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting custom report builder API
  version: 1.0.0
paths:
  /api/v1/accounting/custom-report-builder:
    get:
      operationId: accounting.custom.report.builder.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingCustomReportBuilderResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-custom-report-builder
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/AccountingCustomReportBuilderResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-custom-report-builder`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-custom-report-builder`.
