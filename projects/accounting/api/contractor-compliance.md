# Accounting: Contractor Compliance API

## Canonical one-to-one API module specification

**API package:** `module-accounting-contractor-compliance-api`  
**Matching domain module:** `accounting-contractor-compliance`  
**Application:** Accounting  
**Source feature:** [Contractor Compliance](../features/contractor-compliance.md)  
**Architecture:** [API.md](../ACCOUNTING.md) · [MODULES.md](../ACCOUNTING.md) · [TESTING.md](../ACCOUNTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Contractor Compliance domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- status/evidence
- withholding schemes
- CIS/1099-style classifications
- deductions
- statements
- regional exports
...
- Use `/api/v1/accounting/contractor-compliance` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/contractor-compliance`
...
| -------- | ------------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/accounting/contractor-compliance?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/accounting/contractor-compliance/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/accounting/contractor-compliance`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/accounting/contractor-compliance/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/accounting/contractor-compliance/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/accounting/contractor-compliance/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "contractor_records": "example-value",
  "status_evidence": "example-value",
  "withholding_schemes": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-contractor-compliance`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-contractor-compliance.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.contractor.compliance.list`, `accounting.contractor.compliance.get`, `accounting.contractor.compliance.create`, `accounting.contractor.compliance.update`, and `accounting.contractor.compliance.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingContractorComplianceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `contractor_records`, `status_evidence`, `withholding_schemes`, `cis_1099_style_classifications`, `deductions`, `statements`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting contractor compliance API
  version: 1.0.0
paths:
  /api/v1/accounting/contractor-compliance:
    get:
      operationId: accounting.contractor.compliance.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingContractorComplianceResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-contractor-compliance
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/AccountingContractorComplianceResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-contractor-compliance`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-contractor-compliance`.
