# Accounting: Purchase Orders API

## Canonical one-to-one API module specification

**API package:** `module-accounting-purchase-orders-api`  
...
- Use `/api/v1/accounting/purchase-orders` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/purchase-orders`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/purchase-orders?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/purchase-orders/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/purchase-orders` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/purchase-orders/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/purchase-orders/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/purchase-orders/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "supplier_items": "example-value",
  "prices": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-purchase-orders`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-purchase-orders.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.purchase.orders.list`, `accounting.purchase.orders.get`, `accounting.purchase.orders.create`, `accounting.purchase.orders.update`, and `accounting.purchase.orders.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingPurchaseOrdersResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `supplier_items`, `prices`, `quantities`, `delivery`, `approval`, `issue`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting purchase orders API
  version: 1.0.0
paths:
  /api/v1/accounting/purchase-orders:
    get:
      operationId: accounting.purchase.orders.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingPurchaseOrdersResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-purchase-orders
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingPurchaseOrdersResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-purchase-orders`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-purchase-orders`.
