# Accounting: Branch and Location Accounting API

## Canonical one-to-one API module specification

**API package:** `module-accounting-branch-and-location-accounting-api`  
**Matching domain module:** `accounting-branch-and-location-accounting`  
**Application:** Accounting  
**Source feature:** [Branch and Location Accounting](../features/branch-and-location-accounting.md)  
**Architecture:** [API.md](../ACCOUNTING.md) · [MODULES.md](../ACCOUNTING.md) · [TESTING.md](../ACCOUNTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Branch and Location Accounting domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Branch/location dimensions or books
- local sequences/tax
- allocations
- performance
...
- Use `/api/v1/accounting/branch-and-location-accounting` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/branch-and-location-accounting`
...
| -------- | ---------------------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/accounting/branch-and-location-accounting?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/accounting/branch-and-location-accounting/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/accounting/branch-and-location-accounting`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/accounting/branch-and-location-accounting/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/accounting/branch-and-location-accounting/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/accounting/branch-and-location-accounting/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "branch_location_dimensions_or_books": "example-value",
  "local_sequences_tax": "example-value",
  "allocations": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-branch-and-location-accounting`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-branch-and-location-accounting.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.branch.and.location.accounting.list`, `accounting.branch.and.location.accounting.get`, `accounting.branch.and.location.accounting.create`, `accounting.branch.and.location.accounting.update`, and `accounting.branch.and.location.accounting.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingBranchAndLocationAccountingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `branch_location_dimensions_or_books`, `local_sequences_tax`, `allocations`, `performance`, `statutory_requirements`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting branch and location accounting API
  version: 1.0.0
paths:
  /api/v1/accounting/branch-and-location-accounting:
    get:
      operationId: accounting.branch.and.location.accounting.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingBranchAndLocationAccountingResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-branch-and-location-accounting
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/AccountingBranchAndLocationAccountingResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-branch-and-location-accounting`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-branch-and-location-accounting`.
