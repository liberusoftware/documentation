# Accounting: Close Management API

## Canonical one-to-one API module specification

**API package:** `module-accounting-close-management-api`  
**Matching domain module:** `accounting-close-management`  
**Application:** Accounting  
**Source feature:** [Close Management](../features/close-management.md)  
**Architecture:** [API.md](../ACCOUNTING.md) · [MODULES.md](../ACCOUNTING.md) · [TESTING.md](../ACCOUNTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Close Management domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/accounting/close-management` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/close-management`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/close-management?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/close-management/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/close-management` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/close-management/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/close-management/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/close-management/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "checklist": "example-value",
  "owners": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-close-management`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-close-management.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.close.management.list`, `accounting.close.management.get`, `accounting.close.management.create`, `accounting.close.management.update`, and `accounting.close.management.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingCloseManagementResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `checklist`, `owners`, `dependencies`, `due_dates`, `evidence`, `reconciliations`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting close management API
  version: 1.0.0
paths:
  /api/v1/accounting/close-management:
    get:
      operationId: accounting.close.management.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AccountingCloseManagementResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: accounting-close-management
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingCloseManagementResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-close-management`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-close-management`.
