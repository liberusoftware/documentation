# Accounting: Sales Orders API

## Canonical one-to-one API module specification

**API package:** `module-accounting-sales-orders-api`  
**Matching domain module:** `accounting-sales-orders`  
**Application:** Accounting  
**Source feature:** [Sales Orders](../features/sales-orders.md)  
**Architecture:** [API.md](../ACCOUNTING.md) · [MODULES.md](../ACCOUNTING.md) · [TESTING.md](../ACCOUNTING.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Sales Orders domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Accepted demand
- allocation/fulfillment references
- deposits
- partial invoicing
- status
- cancellation
- Billing/Ecommerce integration
...
- Use `/api/v1/accounting/sales-orders` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/accounting/sales-orders`
...
|---|---|---|---|
| `GET` | `/api/v1/accounting/sales-orders?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/accounting/sales-orders/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/accounting/sales-orders` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/accounting/sales-orders/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/accounting/sales-orders/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/accounting/sales-orders/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "accepted_demand": "example-value",
  "allocation_fulfillment_references": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `accounting-sales-orders`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/accounting-sales-orders.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `accounting.sales.orders.list`, `accounting.sales.orders.get`, `accounting.sales.orders.create`, `accounting.sales.orders.update`, and `accounting.sales.orders.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AccountingSalesOrdersResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `accepted_demand`, `allocation_fulfillment_references`, `deposits`, `partial_invoicing`, `status`, `cancellation`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: accounting sales orders API
  version: 1.0.0
paths:
  /api/v1/accounting/sales-orders:
    get:
      operationId: accounting.sales.orders.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AccountingSalesOrdersResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `accounting-sales-orders`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `accounting-sales-orders`.
