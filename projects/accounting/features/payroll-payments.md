...
**Domain module:** `module-accounting-payroll-payments`
...
The Payroll Payments module owns Net-pay and liability batch references, approvals, bank/provider status, failures, and bank reconciliation. It is the authoritative boundary for this capability and must remain independently installable, testable, versioned, enabled, and reusable across compatible Liberu applications.

## 2. Full feature scope

- **Net-pay and liability batch references:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- **Bank/provider status:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- **Bank reconciliation:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- Provide matching optional presentation packages only when required: `module-accounting-payroll-payments-filament`, `module-accounting-payroll-payments-livewire`, and `module-accounting-payroll-payments-api`.
...
- Verify net-pay and liability batch references across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
...
- Verify bank/provider status across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
...
- Verify bank reconciliation across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
