...
**Domain module:** `module-accounting-client-collaboration`
...
The Client Collaboration module owns Secure document requests, questions, tasks, discussions, approvals, status, reminders, and evidence. It is the authoritative boundary for this capability and must remain independently installable, testable, versioned, enabled, and reusable across compatible Liberu applications.

## 2. Full feature scope

- **Secure document requests:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- Provide matching optional presentation packages only when required: `module-accounting-client-collaboration-filament`, `module-accounting-client-collaboration-livewire`, and `module-accounting-client-collaboration-api`.
...
- Verify secure document requests across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
