...
**Domain module:** `module-accounting-payment-reconciliation`
...
The Payment Reconciliation module owns Gateway/merchant settlement imports, gross-to-net fees/refunds/disputes, matching, missing items, and provider drift. It is the authoritative boundary for this capability and must remain independently installable, testable, versioned, enabled, and reusable across compatible Liberu applications.

## 2. Full feature scope

- **Gateway/merchant settlement imports:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
- **Gross-to-net fees/refunds/disputes:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- **Missing items:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
- **Provider drift:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- Provide matching optional presentation packages only when required: `module-accounting-payment-reconciliation-filament`, `module-accounting-payment-reconciliation-livewire`, and `module-accounting-payment-reconciliation-api`.
...
- Verify gateway/merchant settlement imports across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
- Verify gross-to-net fees/refunds/disputes across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
...
- Verify missing items across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
- Verify provider drift across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
