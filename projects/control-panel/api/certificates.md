# Control Panel: Certificates API

## Canonical one-to-one API module specification

**API package:** `module-control-panel-certificates-api`  
**Matching domain module:** `control-panel-certificates`  
**Application:** Control Panel  
**Source feature:** [Certificates](../features/certificates.md)  
**Architecture:** [API.md](../CONTROL-PANEL.md) · [MODULES.md](../CONTROL-PANEL.md) · [TESTING.md](../CONTROL-PANEL.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Certificates domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/control-panel/certificates` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/control-panel/certificates`
...
|---|---|---|---|
| `GET` | `/api/v1/control-panel/certificates?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/control-panel/certificates/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/control-panel/certificates` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/control-panel/certificates/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/control-panel/certificates/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/control-panel/certificates/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "acme_accounts": "example-value",
  "issuance": "example-value",
  "deployment": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `control-panel-certificates`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/control-panel-certificates.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `control.panel.certificates.list`, `control.panel.certificates.get`, `control.panel.certificates.create`, `control.panel.certificates.update`, and `control.panel.certificates.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `ControlPanelCertificatesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `acme_accounts`, `issuance`, `deployment`, `renewal`, `revocation`, `expiry_monitoring`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
components:
  ...
  schemas:
    ControlPanelCertificatesResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: control-panel-certificates
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/ControlPanelCertificatesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `control-panel-certificates`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `control-panel-certificates`.
