# Real Estate: Valuations API

## Canonical one-to-one API module specification

**API package:** `module-real-estate-valuations-api`
**Matching domain module:** `real-estate-valuations`
**Application:** Real Estate
**Source feature:** [Valuations](../features/valuations.md)
**Architecture:** [API.md](../REAL-ESTATE.md) · [MODULES.md](../REAL-ESTATE.md) · [TESTING.md](../REAL-ESTATE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Valuations domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/real-estate/valuations` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/real-estate/valuations`
...
| -------- | --------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/real-estate/valuations?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/real-estate/valuations/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/real-estate/valuations`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/real-estate/valuations/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/real-estate/valuations/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/real-estate/valuations/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "appraisals": "example-value",
  "comparables": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `real-estate-valuations`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/real-estate-valuations.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `real.estate.valuations.list`, `real.estate.valuations.get`, `real.estate.valuations.create`, `real.estate.valuations.update`, and `real.estate.valuations.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `RealEstateValuationsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `appraisals`, `comparables`, `pricing`, `fees`, `recommendations`, `follow_up`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: real estate valuations API
  version: 1.0.0
paths:
  /api/v1/real-estate/valuations:
    get:
      operationId: real.estate.valuations.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/RealEstateValuationsResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `real-estate-valuations`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `real-estate-valuations`.
