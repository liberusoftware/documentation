...
**API package:** `module-real-estate-sales-progression-api`
...
This optional API presentation package exposes approved HTTP operations for the Sales Progression domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/real-estate/sales-progression` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/real-estate/sales-progression`
...
| -------- | ---------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/real-estate/sales-progression?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/real-estate/sales-progression/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/real-estate/sales-progression`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/real-estate/sales-progression/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/real-estate/sales-progression/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/real-estate/sales-progression/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |
...
{
  "chain": "example-value",
  "milestones": "example-value",
  "memoranda": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `real-estate-sales-progression`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/real-estate-sales-progression.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `real.estate.sales.progression.list`, `real.estate.sales.progression.get`, `real.estate.sales.progression.create`, `real.estate.sales.progression.update`, and `real.estate.sales.progression.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `RealEstateSalesProgressionResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `chain`, `milestones`, `memoranda`, `legal_finance_contacts`, `dependencies`, `risks`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: real estate sales progression API
  version: 1.0.0
paths:
  /api/v1/real-estate/sales-progression:
    get:
      operationId: real.estate.sales.progression.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    RealEstateSalesProgressionResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: real-estate-sales-progression
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/RealEstateSalesProgressionResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `real-estate-sales-progression`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `real-estate-sales-progression`.
