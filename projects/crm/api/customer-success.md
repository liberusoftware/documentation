# CRM: Customer Success API

## Canonical one-to-one API module specification

**API package:** `module-crm-customer-success-api`  
...
This optional API presentation package exposes approved HTTP operations for the Customer Success domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/customer-success` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/customer-success`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/customer-success?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/customer-success/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/customer-success` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/customer-success/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/customer-success/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/customer-success/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "customer_segments": "example-value",
  "lifecycle": "example-value",
  "onboarding_plans": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-customer-success`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-customer-success.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.customer.success.list`, `crm.customer.success.get`, `crm.customer.success.create`, `crm.customer.success.update`, and `crm.customer.success.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmCustomerSuccessResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `customer_segments`, `lifecycle`, `onboarding_plans`, `success_plans`, `objectives`, `health_scores`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm customer success API
  version: 1.0.0
paths:
  /api/v1/crm/customer-success:
    get:
      operationId: crm.customer.success.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmCustomerSuccessResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-customer-success`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-customer-success`.
