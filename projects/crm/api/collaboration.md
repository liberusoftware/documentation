...
**API package:** `module-crm-collaboration-api`  
...
- handoffs
- internal channels
- collaboration-provider adapters
...
- Use `/api/v1/crm/collaboration` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/collaboration`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/collaboration?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/collaboration/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/collaboration` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/collaboration/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/collaboration/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/collaboration/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "record_comments": "example-value",
  "mentions": "example-value",
  "followers": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-collaboration`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-collaboration.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.collaboration.list`, `crm.collaboration.get`, `crm.collaboration.create`, `crm.collaboration.update`, and `crm.collaboration.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmCollaborationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `record_comments`, `mentions`, `followers`, `shared_work_queues`, `approvals`, `handoffs`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm collaboration API
  version: 1.0.0
paths:
  /api/v1/crm/collaboration:
    get:
      operationId: crm.collaboration.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmCollaborationResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-collaboration`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-collaboration`.
