# CRM: Customer Data Platform API

## Canonical one-to-one API module specification

**API package:** `module-crm-customer-data-platform-api`  
**Matching domain module:** `crm-customer-data-platform`  
**Application:** CRM  
**Source feature:** [Customer Data Platform](../features/customer-data-platform.md)  
**Architecture:** [API.md](../CRM.md) · [MODULES.md](../CRM.md) · [TESTING.md](../CRM.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Customer Data Platform domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- audience activation
- event ingestion
- consent-aware calculated attributes
- profile lineage
...
- Use `/api/v1/crm/customer-data-platform` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/customer-data-platform`
...
| -------- | ------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/crm/customer-data-platform?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/crm/customer-data-platform/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/crm/customer-data-platform`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/crm/customer-data-platform/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/crm/customer-data-platform/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/crm/customer-data-platform/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "source_profiles": "example-value",
  "identity_resolution": "example-value",
  "unified_profiles": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-customer-data-platform`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-customer-data-platform.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.customer.data.platform.list`, `crm.customer.data.platform.get`, `crm.customer.data.platform.create`, `crm.customer.data.platform.update`, and `crm.customer.data.platform.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmCustomerDataPlatformResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `source_profiles`, `identity_resolution`, `unified_profiles`, `audience_activation`, `event_ingestion`, `consent_aware_calculated_attributes`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm customer data platform API
  version: 1.0.0
paths:
  /api/v1/crm/customer-data-platform:
    get:
      operationId: crm.customer.data.platform.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmCustomerDataPlatformResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-customer-data-platform
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/CrmCustomerDataPlatformResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-customer-data-platform`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-customer-data-platform`.
