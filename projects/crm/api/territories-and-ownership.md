...
**API package:** `module-crm-territories-and-ownership-api`  
**Matching domain module:** `crm-territories-and-ownership`  
**Application:** CRM  
**Source feature:** [Territories and Ownership](../features/territories-and-ownership.md)  
...
- Use `/api/v1/crm/territories-and-ownership` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/territories-and-ownership`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/territories-and-ownership?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/territories-and-ownership/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/territories-and-ownership` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/territories-and-ownership/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/territories-and-ownership/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/territories-and-ownership/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
```json
{
  ...
  "books_of_business": "example-value",
  "teams": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-territories-and-ownership`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-territories-and-ownership.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.territories.and.ownership.list`, `crm.territories.and.ownership.get`, `crm.territories.and.ownership.create`, `crm.territories.and.ownership.update`, and `crm.territories.and.ownership.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmTerritoriesAndOwnershipResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `territories`, `books_of_business`, `teams`, `queues`, `assignment_rules`, `round_robin_capacity_routing`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm territories and ownership API
  version: 1.0.0
paths:
  /api/v1/crm/territories-and-ownership:
    get:
      operationId: crm.territories.and.ownership.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmTerritoriesAndOwnershipResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-territories-and-ownership
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmTerritoriesAndOwnershipResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-territories-and-ownership`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-territories-and-ownership`.
