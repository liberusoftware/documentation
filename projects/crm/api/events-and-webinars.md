# CRM: Events and Webinars API

## Canonical one-to-one API module specification

**API package:** `module-crm-events-and-webinars-api`  
...
This optional API presentation package exposes approved HTTP operations for the Events and Webinars domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/events-and-webinars` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/events-and-webinars`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/events-and-webinars?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/events-and-webinars/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/events-and-webinars` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/events-and-webinars/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/events-and-webinars/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/events-and-webinars/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "physical_virtual_events": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-events-and-webinars`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-events-and-webinars.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.events.and.webinars.list`, `crm.events.and.webinars.get`, `crm.events.and.webinars.create`, `crm.events.and.webinars.update`, and `crm.events.and.webinars.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmEventsAndWebinarsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `physical_virtual_events`, `registration`, `capacity`, `tickets_attendance`, `sessions`, `speakers`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm events and webinars API
  version: 1.0.0
paths:
  /api/v1/crm/events-and-webinars:
    get:
      operationId: crm.events.and.webinars.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmEventsAndWebinarsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-events-and-webinars`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-events-and-webinars`.
