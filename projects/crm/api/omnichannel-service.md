# CRM: Omnichannel Service API

## Canonical one-to-one API module specification

**API package:** `module-crm-omnichannel-service-api`  
**Matching domain module:** `crm-omnichannel-service`  
**Application:** CRM  
**Source feature:** [Omnichannel Service](../features/omnichannel-service.md)  
**Architecture:** [API.md](../CRM.md) · [MODULES.md](../CRM.md) · [TESTING.md](../CRM.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Omnichannel Service domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Email/chat/social/phone intake
- routing
- agent workspace
- macros
- suggested replies
...
- Use `/api/v1/crm/omnichannel-service` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/omnichannel-service`
...
| -------- | ---------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/crm/omnichannel-service?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/crm/omnichannel-service/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/crm/omnichannel-service`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/crm/omnichannel-service/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/crm/omnichannel-service/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/crm/omnichannel-service/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "email_chat_social_phone_intake": "example-value",
  "routing": "example-value",
  "agent_workspace": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-omnichannel-service`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-omnichannel-service.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.omnichannel.service.list`, `crm.omnichannel.service.get`, `crm.omnichannel.service.create`, `crm.omnichannel.service.update`, and `crm.omnichannel.service.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmOmnichannelServiceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `email_chat_social_phone_intake`, `routing`, `agent_workspace`, `macros`, `suggested_replies`, `swarming`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm omnichannel service API
  version: 1.0.0
paths:
  /api/v1/crm/omnichannel-service:
    get:
      operationId: crm.omnichannel.service.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/CrmOmnichannelServiceResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-omnichannel-service`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-omnichannel-service`.
