...
**API package:** `module-crm-scheduling-api`  
...
- Personal/team/round-robin links
- availability
- buffers
- minimum notice
- questions
...
- Use `/api/v1/crm/scheduling` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/scheduling`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/scheduling?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/scheduling/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/scheduling` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/scheduling/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/scheduling/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/scheduling/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "personal_team_round_robin_links": "example-value",
  "availability": "example-value",
  "buffers": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-scheduling`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-scheduling.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.scheduling.list`, `crm.scheduling.get`, `crm.scheduling.create`, `crm.scheduling.update`, and `crm.scheduling.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmSchedulingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `personal_team_round_robin_links`, `availability`, `buffers`, `minimum_notice`, `questions`, `reminders`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm scheduling API
  version: 1.0.0
paths:
  /api/v1/crm/scheduling:
    get:
      operationId: crm.scheduling.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmSchedulingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-scheduling`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-scheduling`.
