# CRM: Account-Based Marketing API

## Canonical one-to-one API module specification

**API package:** `module-crm-account-based-marketing-api`
...
This optional API presentation package exposes approved HTTP operations for the Account-Based Marketing domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/account-based-marketing` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/account-based-marketing`
...
| -------- | -------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/crm/account-based-marketing?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/crm/account-based-marketing/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/crm/account-based-marketing`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/crm/account-based-marketing/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/crm/account-based-marketing/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/crm/account-based-marketing/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

`json
{
  "target_accounts": "example-value",
  "tiers": "example-value",
  "buying_committees": "example-value"
}
`
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-account-based-marketing`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-account-based-marketing.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.account.based.marketing.list`, `crm.account.based.marketing.get`, `crm.account.based.marketing.create`, `crm.account.based.marketing.update`, and `crm.account.based.marketing.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmAccountBasedMarketingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `target_accounts`, `tiers`, `buying_committees`, `intent`, `engagement_rollups`, `account_audiences`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm account based marketing API
  version: 1.0.0
paths:
  /api/v1/crm/account-based-marketing:
    get:
      operationId: crm.account.based.marketing.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmAccountBasedMarketingResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-account-based-marketing
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmAccountBasedMarketingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-account-based-marketing`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-account-based-marketing`.
