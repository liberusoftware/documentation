# CRM: Proposals and Quotes API

## Canonical one-to-one API module specification

**API package:** `module-crm-proposals-and-quotes-api`  
**Matching domain module:** `crm-proposals-and-quotes`  
**Application:** CRM  
**Source feature:** [Proposals and Quotes](../features/proposals-and-quotes.md)  
**Architecture:** [API.md](../CRM.md) · [MODULES.md](../CRM.md) · [TESTING.md](../CRM.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Proposals and Quotes domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/proposals-and-quotes` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/proposals-and-quotes`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/proposals-and-quotes?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/proposals-and-quotes/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/proposals-and-quotes` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/proposals-and-quotes/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/proposals-and-quotes/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/proposals-and-quotes/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "branded_templates": "example-value",
  "sections": "example-value",
  "scope": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-proposals-and-quotes`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-proposals-and-quotes.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.proposals.and.quotes.list`, `crm.proposals.and.quotes.get`, `crm.proposals.and.quotes.create`, `crm.proposals.and.quotes.update`, and `crm.proposals.and.quotes.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmProposalsAndQuotesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `branded_templates`, `sections`, `scope`, `line_items`, `optional_choices`, `versions`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm proposals and quotes API
  version: 1.0.0
paths:
  /api/v1/crm/proposals-and-quotes:
    get:
      operationId: crm.proposals.and.quotes.list
      security:
        - sanctum: []
      parameters:
  ...
  schemas:
    CrmProposalsAndQuotesResource:
      ...
      properties:
        ...
        type:
          type: string
          const: crm-proposals-and-quotes
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmProposalsAndQuotesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-proposals-and-quotes`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-proposals-and-quotes`.
