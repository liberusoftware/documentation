# CRM: Business Process Management API

## Canonical one-to-one API module specification

**API package:** `module-crm-business-process-management-api`
**Matching domain module:** `crm-business-process-management`
**Application:** CRM
**Source feature:** [Business Process Management](../features/business-process-management.md)
**Architecture:** [API.md](../CRM.md) · [MODULES.md](../CRM.md) · [TESTING.md](../CRM.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Business Process Management domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/business-process-management` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/business-process-management`
...
| -------- | ------------------------------------------------------------------------ | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/crm/business-process-management?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/crm/business-process-management/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/crm/business-process-management`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/crm/business-process-management/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/crm/business-process-management/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/crm/business-process-management/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "guided_stages": "example-value",
  "required_steps_data": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-business-process-management`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-business-process-management.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.business.process.management.list`, `crm.business.process.management.get`, `crm.business.process.management.create`, `crm.business.process.management.update`, and `crm.business.process.management.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmBusinessProcessManagementResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `guided_stages`, `required_steps_data`, `approvals`, `validations`, `service_processes`, `exception_routing`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm business process management API
  version: 1.0.0
paths:
  /api/v1/crm/business-process-management:
    get:
      operationId: crm.business.process.management.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmBusinessProcessManagementResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-business-process-management
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/CrmBusinessProcessManagementResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-business-process-management`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-business-process-management`.
