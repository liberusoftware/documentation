# CRM: Revenue Lifecycle API

## Canonical one-to-one API module specification

**API package:** `module-crm-revenue-lifecycle-api`  
**Matching domain module:** `crm-revenue-lifecycle`  
**Application:** CRM  
**Source feature:** [Revenue Lifecycle](../features/revenue-lifecycle.md)  
**Architecture:** [API.md](../CRM.md) · [MODULES.md](../CRM.md) · [TESTING.md](../CRM.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Revenue Lifecycle domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Opportunity-to-order orchestration
...
- Use `/api/v1/crm/revenue-lifecycle` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/revenue-lifecycle`
...
| -------- | -------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/crm/revenue-lifecycle?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/crm/revenue-lifecycle/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/crm/revenue-lifecycle`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/crm/revenue-lifecycle/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/crm/revenue-lifecycle/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/crm/revenue-lifecycle/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "opportunity_to_order_orchestration": "example-value",
  "assets_installed_products": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-revenue-lifecycle`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-revenue-lifecycle.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.revenue.lifecycle.list`, `crm.revenue.lifecycle.get`, `crm.revenue.lifecycle.create`, `crm.revenue.lifecycle.update`, and `crm.revenue.lifecycle.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmRevenueLifecycleResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `opportunity_to_order_orchestration`, `assets_installed_products`, `entitlements`, `renewals`, `upgrades`, `downgrades`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm revenue lifecycle API
  version: 1.0.0
paths:
  /api/v1/crm/revenue-lifecycle:
    get:
      operationId: crm.revenue.lifecycle.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/CrmRevenueLifecycleResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-revenue-lifecycle`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-revenue-lifecycle`.
