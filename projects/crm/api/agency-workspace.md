# CRM: Agency Workspace API

## Canonical one-to-one API module specification

**API package:** `module-crm-agency-workspace-api`  
**Matching domain module:** `crm-agency-workspace`  
**Application:** CRM  
**Source feature:** [Agency Workspace](../features/agency-workspace.md)  
**Architecture:** [API.md](../CRM.md) · [MODULES.md](../CRM.md) · [TESTING.md](../CRM.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Agency Workspace domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/agency-workspace` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/agency-workspace`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/agency-workspace?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/agency-workspace/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/agency-workspace` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/agency-workspace/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/agency-workspace/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/agency-workspace/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "agency_client_hierarchy": "example-value",
  "sub_accounts": "example-value",
  "delegated_administration": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-agency-workspace`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-agency-workspace.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.agency.workspace.list`, `crm.agency.workspace.get`, `crm.agency.workspace.create`, `crm.agency.workspace.update`, and `crm.agency.workspace.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmAgencyWorkspaceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `agency_client_hierarchy`, `sub_accounts`, `delegated_administration`, `usage_visibility`, `support_access`, `branding`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm agency workspace API
  version: 1.0.0
paths:
  /api/v1/crm/agency-workspace:
    get:
      operationId: crm.agency.workspace.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmAgencyWorkspaceResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-agency-workspace`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-agency-workspace`.
