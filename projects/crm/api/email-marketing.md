...
**API package:** `module-crm-email-marketing-api`  
**Matching domain module:** `crm-email-marketing`  
**Application:** CRM  
**Source feature:** [Email Marketing](../features/email-marketing.md)  
**Architecture:** [API.md](../CRM.md) · [MODULES.md](../CRM.md) · [TESTING.md](../CRM.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Email Marketing domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Drag-and-drop and code templates
- personalization
- dynamic content
- test sends
- inbox previews
...
- Use `/api/v1/crm/email-marketing` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/email-marketing`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/email-marketing?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/email-marketing/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/email-marketing` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/email-marketing/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/email-marketing/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/email-marketing/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "drag_and_drop_and_code_templates": "example-value",
  "personalization": "example-value",
  "dynamic_content": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-email-marketing`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-email-marketing.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.email.marketing.list`, `crm.email.marketing.get`, `crm.email.marketing.create`, `crm.email.marketing.update`, and `crm.email.marketing.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmEmailMarketingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `drag_and_drop_and_code_templates`, `personalization`, `dynamic_content`, `test_sends`, `inbox_previews`, `deliverability_controls`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm email marketing API
  version: 1.0.0
paths:
  /api/v1/crm/email-marketing:
    get:
      operationId: crm.email.marketing.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmEmailMarketingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-email-marketing`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-email-marketing`.
