# CRM: CRM Product Workspace API

## Canonical one-to-one API module specification

**API package:** `module-crm-product-workspace-api`  
**Matching domain module:** `crm-product-workspace`  
**Application:** CRM  
**Source feature:** [CRM Product Workspace](../features/crm-product-workspace.md)  
**Architecture:** [API.md](../CRM.md) · [MODULES.md](../CRM.md) · [TESTING.md](../CRM.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the CRM Product Workspace domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Sales-facing catalog projections
- bundles
- entitlements
- price-book references
- product eligibility
- governed Billing/Ecommerce synchronization
...
- Use `/api/v1/crm/crm-product-workspace` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/crm-product-workspace`
...
| -------- | ------------------------------------------------------------------ | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/crm/crm-product-workspace?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/crm/crm-product-workspace/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/crm/crm-product-workspace`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/crm/crm-product-workspace/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/crm/crm-product-workspace/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/crm/crm-product-workspace/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "sales_facing_catalog_projections": "example-value",
  "bundles": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-product-workspace`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-product-workspace.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.product.workspace.list`, `crm.product.workspace.get`, `crm.product.workspace.create`, `crm.product.workspace.update`, and `crm.product.workspace.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmProductWorkspaceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `sales_facing_catalog_projections`, `bundles`, `entitlements`, `price_book_references`, `product_eligibility`, `governed_billing_ecommerce_synchronization`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm product workspace API
  version: 1.0.0
paths:
  /api/v1/crm/crm-product-workspace:
    get:
      operationId: crm.product.workspace.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/CrmProductWorkspaceResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-product-workspace`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-product-workspace`.
