...
**API package:** `module-crm-advertising-api`  
**Matching domain module:** `crm-advertising`  
**Application:** CRM  
**Source feature:** [Advertising](../features/advertising.md)  
**Architecture:** [API.md](../CRM.md) · [MODULES.md](../CRM.md) · [TESTING.md](../CRM.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Advertising domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- lead-ad ingestion
- CRM audiences
- offline/server conversions
- spend/performance sync
- attribution
...
- Use `/api/v1/crm/advertising` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/advertising`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/advertising?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/advertising/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/advertising` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/advertising/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/advertising/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/advertising/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "ad_account_connections": "example-value",
  "campaign_visibility": "example-value",
  "lead_ad_ingestion": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-advertising`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-advertising.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.advertising.list`, `crm.advertising.get`, `crm.advertising.create`, `crm.advertising.update`, and `crm.advertising.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmAdvertisingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `ad_account_connections`, `campaign_visibility`, `lead_ad_ingestion`, `crm_audiences`, `offline_server_conversions`, `spend_performance_sync`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm advertising API
  version: 1.0.0
paths:
  /api/v1/crm/advertising:
    get:
      operationId: crm.advertising.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmAdvertisingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-advertising`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-advertising`.
