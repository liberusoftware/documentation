# CRM: Templates and Snapshots API

## Canonical one-to-one API module specification

**API package:** `module-crm-templates-and-snapshots-api`  
...
This optional API presentation package exposes approved HTTP operations for the Templates and Snapshots domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Versioned bundles of fields
- pipelines
- workflows
- forms
- funnels
...
- Use `/api/v1/crm/templates-and-snapshots` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/templates-and-snapshots`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/templates-and-snapshots?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/templates-and-snapshots/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/templates-and-snapshots` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/templates-and-snapshots/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/templates-and-snapshots/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/templates-and-snapshots/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "versioned_bundles_of_fields": "example-value",
  "pipelines": "example-value",
  "workflows": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-templates-and-snapshots`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-templates-and-snapshots.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.templates.and.snapshots.list`, `crm.templates.and.snapshots.get`, `crm.templates.and.snapshots.create`, `crm.templates.and.snapshots.update`, and `crm.templates.and.snapshots.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmTemplatesAndSnapshotsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `versioned_bundles_of_fields`, `pipelines`, `workflows`, `forms`, `funnels`, `calendars`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm templates and snapshots API
  version: 1.0.0
paths:
  /api/v1/crm/templates-and-snapshots:
    get:
      operationId: crm.templates.and.snapshots.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmTemplatesAndSnapshotsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-templates-and-snapshots
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmTemplatesAndSnapshotsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-templates-and-snapshots`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-templates-and-snapshots`.
