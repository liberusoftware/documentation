# CRM: Partner Relationship Management API

## Canonical one-to-one API module specification

**API package:** `module-crm-partner-relationship-management-api`  
**Matching domain module:** `crm-partner-relationship-management`  
**Application:** CRM  
**Source feature:** [Partner Relationship Management](../features/partner-relationship-management.md)  
**Architecture:** [API.md](../CRM.md) · [MODULES.md](../CRM.md) · [TESTING.md](../CRM.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Partner Relationship Management domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/partner-relationship-management` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/partner-relationship-management`
...
|---|---|---|---|
| `GET` | `/api/v1/crm/partner-relationship-management?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/crm/partner-relationship-management/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/crm/partner-relationship-management` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/crm/partner-relationship-management/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/crm/partner-relationship-management/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/crm/partner-relationship-management/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "partner_accounts": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-partner-relationship-management`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-partner-relationship-management.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.partner.relationship.management.list`, `crm.partner.relationship.management.get`, `crm.partner.relationship.management.create`, `crm.partner.relationship.management.update`, and `crm.partner.relationship.management.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmPartnerRelationshipManagementResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `partner_accounts`, `contacts`, `tiers`, `recruitment`, `onboarding`, `competencies`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm partner relationship management API
  version: 1.0.0
paths:
  /api/v1/crm/partner-relationship-management:
    get:
      operationId: crm.partner.relationship.management.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CrmPartnerRelationshipManagementResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: crm-partner-relationship-management
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmPartnerRelationshipManagementResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-partner-relationship-management`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-partner-relationship-management`.
