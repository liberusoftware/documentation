# CRM: Predictive Models API

## Canonical one-to-one API module specification

**API package:** `module-crm-predictive-models-api`  
**Matching domain module:** `crm-predictive-models`  
**Application:** CRM  
**Source feature:** [Predictive Models](../features/predictive-models.md)  
**Architecture:** [API.md](../CRM.md) · [MODULES.md](../CRM.md) · [TESTING.md](../CRM.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Predictive Models domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/crm/predictive-models` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/crm/predictive-models`
...
| -------- | -------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/crm/predictive-models?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/crm/predictive-models/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/crm/predictive-models`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/crm/predictive-models/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/crm/predictive-models/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/crm/predictive-models/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "lead_opportunity_scoring": "example-value",
  "churn_renewal_risk": "example-value",
  "next_action_product": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `crm-predictive-models`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-predictive-models.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.predictive.models.list`, `crm.predictive.models.get`, `crm.predictive.models.create`, `crm.predictive.models.update`, and `crm.predictive.models.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmPredictiveModelsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `lead_opportunity_scoring`, `churn_renewal_risk`, `next_action_product`, `forecast`, `routing`, `model_registry`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm predictive models API
  version: 1.0.0
paths:
  /api/v1/crm/predictive-models:
    get:
      operationId: crm.predictive.models.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/CrmPredictiveModelsResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `crm-predictive-models`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `crm-predictive-models`.
