...
This module owns a versioned OpenAPI 3.1 fragment for `crm-sales-engagement`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/crm-sales-engagement.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `crm.sales.engagement.list`, `crm.sales.engagement.get`, `crm.sales.engagement.create`, `crm.sales.engagement.update`, and `crm.sales.engagement.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CrmSalesEngagementResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `sequences_cadences`, `multi_channel_steps`, `templates_snippets`, `task_queues`, `enrollment_re_entry`, `throttles`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: crm sales engagement API
  version: 1.0.0
paths:
  /api/v1/crm/sales-engagement:
    get:
      operationId: crm.sales.engagement.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CrmSalesEngagementResource'
  responses:
    ResourceCollection:
