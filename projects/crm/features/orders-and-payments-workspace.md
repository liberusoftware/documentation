# CRM: Orders and Payments Workspace
...
The Orders and Payments Workspace module owns Payment links, deposits, order/subscription references, invoice/payment status, refunds/disputes visibility, and Billing/Ecommerce handoff. It is the authoritative boundary for this capability and must remain independently installable, testable, versioned, enabled, and reusable across compatible Liberu applications.
...
- **Order/subscription references:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
- **Invoice/payment status:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
- **Refunds/disputes visibility:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
- **Billing/Ecommerce handoff:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- Provide matching optional presentation packages only when required: `module-crm-orders-and-payments-workspace-filament`, `module-crm-orders-and-payments-workspace-livewire`, and `module-crm-orders-and-payments-workspace-api`.
...
- Verify order/subscription references across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
- Verify invoice/payment status across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
- Verify refunds/disputes visibility across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
- Verify billing/Ecommerce handoff across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
