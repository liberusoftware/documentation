# Ecommerce: Warranty and Claims API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-warranty-and-claims-api`  
**Matching domain module:** `ecommerce-warranty-and-claims`  
**Application:** Ecommerce  
**Source feature:** [Warranty and Claims](../features/warranty-and-claims.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Warranty and Claims domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- evidence
- troubleshooting
- repair/replace/refund decisions
- providers
- history
...
- Use `/api/v1/ecommerce/warranty-and-claims` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/warranty-and-claims`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/warranty-and-claims?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/warranty-and-claims/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/warranty-and-claims` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/warranty-and-claims/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/warranty-and-claims/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/warranty-and-claims/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "warranty_terms": "example-value",
  "registration": "example-value",
  "claims": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-warranty-and-claims`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-warranty-and-claims.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.warranty.and.claims.list`, `ecommerce.warranty.and.claims.get`, `ecommerce.warranty.and.claims.create`, `ecommerce.warranty.and.claims.update`, and `ecommerce.warranty.and.claims.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceWarrantyAndClaimsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `warranty_terms`, `registration`, `claims`, `evidence`, `troubleshooting`, `repair_replace_refund_decisions`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce warranty and claims API
  version: 1.0.0
paths:
  /api/v1/ecommerce/warranty-and-claims:
    get:
      operationId: ecommerce.warranty.and.claims.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceWarrantyAndClaimsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-warranty-and-claims
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceWarrantyAndClaimsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-warranty-and-claims`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-warranty-and-claims`.
