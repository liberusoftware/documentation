...
**API package:** `module-ecommerce-invoices-and-documents-api`  
...
- Receipt/order/invoice/credit document projections
...
- Use `/api/v1/ecommerce/invoices-and-documents` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/invoices-and-documents`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/invoices-and-documents?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/invoices-and-documents/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/invoices-and-documents` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/invoices-and-documents/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/invoices-and-documents/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/invoices-and-documents/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "receipt_order_invoice_credit_document_projections": "example-value",
  "numbering_references": "example-value",
  "pdfs": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-invoices-and-documents`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-invoices-and-documents.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.invoices.and.documents.list`, `ecommerce.invoices.and.documents.get`, `ecommerce.invoices.and.documents.create`, `ecommerce.invoices.and.documents.update`, and `ecommerce.invoices.and.documents.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceInvoicesAndDocumentsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `receipt_order_invoice_credit_document_projections`, `numbering_references`, `pdfs`, `localization`, `delivery`, `billing_adapters`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce invoices and documents API
  version: 1.0.0
paths:
  /api/v1/ecommerce/invoices-and-documents:
    get:
      operationId: ecommerce.invoices.and.documents.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceInvoicesAndDocumentsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-invoices-and-documents
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceInvoicesAndDocumentsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-invoices-and-documents`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-invoices-and-documents`.
