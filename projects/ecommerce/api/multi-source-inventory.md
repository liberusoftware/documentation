# Ecommerce: Multi-Source Inventory API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-multi-source-inventory-api`  
...
This optional API presentation package exposes approved HTTP operations for the Multi-Source Inventory domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Sources/warehouses/stores/drop shippers
- stocks
- channel assignment
- aggregated salable quantity
- priority/distance rules
- source health
...
- Use `/api/v1/ecommerce/multi-source-inventory` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/multi-source-inventory`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/multi-source-inventory?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/multi-source-inventory/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/multi-source-inventory` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/multi-source-inventory/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/multi-source-inventory/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/multi-source-inventory/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "sources_warehouses_stores_drop_shippers": "example-value",
  "stocks": "example-value",
  "channel_assignment": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-multi-source-inventory`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-multi-source-inventory.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.multi.source.inventory.list`, `ecommerce.multi.source.inventory.get`, `ecommerce.multi.source.inventory.create`, `ecommerce.multi.source.inventory.update`, and `ecommerce.multi.source.inventory.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceMultiSourceInventoryResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `sources_warehouses_stores_drop_shippers`, `stocks`, `channel_assignment`, `aggregated_salable_quantity`, `priority_distance_rules`, `source_health`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce multi source inventory API
  version: 1.0.0
paths:
  /api/v1/ecommerce/multi-source-inventory:
    get:
      operationId: ecommerce.multi.source.inventory.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceMultiSourceInventoryResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-multi-source-inventory
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceMultiSourceInventoryResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-multi-source-inventory`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-multi-source-inventory`.
