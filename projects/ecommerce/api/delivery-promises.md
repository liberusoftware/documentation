# Ecommerce: Delivery Promises API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-delivery-promises-api`  
**Matching domain module:** `ecommerce-delivery-promises`  
**Application:** Ecommerce  
**Source feature:** [Delivery Promises](../features/delivery-promises.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Delivery Promises domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Cutoffs
- processing/transit calendars
- capacity
...
- Use `/api/v1/ecommerce/delivery-promises` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/delivery-promises`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/delivery-promises?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/delivery-promises/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/delivery-promises` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/delivery-promises/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/delivery-promises/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/delivery-promises/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "cutoffs": "example-value",
  "processing_transit_calendars": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-delivery-promises`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-delivery-promises.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.delivery.promises.list`, `ecommerce.delivery.promises.get`, `ecommerce.delivery.promises.create`, `ecommerce.delivery.promises.update`, and `ecommerce.delivery.promises.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceDeliveryPromisesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `cutoffs`, `processing_transit_calendars`, `capacity`, `inventory_source_selection`, `postcode_coverage`, `promise_dates`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce delivery promises API
  version: 1.0.0
paths:
  /api/v1/ecommerce/delivery-promises:
    get:
      operationId: ecommerce.delivery.promises.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceDeliveryPromisesResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-delivery-promises
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceDeliveryPromisesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-delivery-promises`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-delivery-promises`.
