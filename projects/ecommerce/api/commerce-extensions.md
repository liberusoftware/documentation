...
**API package:** `module-ecommerce-commerce-extensions-api`  
**Matching domain module:** `ecommerce-commerce-extensions`  
**Application:** Ecommerce  
**Source feature:** [Commerce Extensions](../features/commerce-extensions.md)  
...
- Use `/api/v1/ecommerce/commerce-extensions` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/commerce-extensions`
...
| -------- | ---------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/commerce-extensions?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/commerce-extensions/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/commerce-extensions`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/commerce-extensions/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/commerce-extensions/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/commerce-extensions/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "extension_manifests": "example-value",
  "install_update_uninstall": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-commerce-extensions`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-commerce-extensions.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.commerce.extensions.list`, `ecommerce.commerce.extensions.get`, `ecommerce.commerce.extensions.create`, `ecommerce.commerce.extensions.update`, and `ecommerce.commerce.extensions.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceCommerceExtensionsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `extension_manifests`, `install_update_uninstall`, `scopes`, `webhooks`, `functions`, `ui_extensions`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce commerce extensions API
  version: 1.0.0
paths:
  /api/v1/ecommerce/commerce-extensions:
    get:
      operationId: ecommerce.commerce.extensions.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceCommerceExtensionsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-commerce-extensions
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommerceCommerceExtensionsResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-commerce-extensions`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-commerce-extensions`.
