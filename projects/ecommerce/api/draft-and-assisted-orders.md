# Ecommerce: Draft and Assisted Orders API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-draft-and-assisted-orders-api`  
**Matching domain module:** `ecommerce-draft-and-assisted-orders`  
**Application:** Ecommerce  
**Source feature:** [Draft and Assisted Orders](../features/draft-and-assisted-orders.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Draft and Assisted Orders domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Staff-created carts/orders
- customer selection
- quotes
...
- Use `/api/v1/ecommerce/draft-and-assisted-orders` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/draft-and-assisted-orders`
...
| -------- | ---------------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/draft-and-assisted-orders?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/draft-and-assisted-orders/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/draft-and-assisted-orders`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/draft-and-assisted-orders/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/draft-and-assisted-orders/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/draft-and-assisted-orders/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "staff_created_carts_orders": "example-value",
  "customer_selection": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-draft-and-assisted-orders`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-draft-and-assisted-orders.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.draft.and.assisted.orders.list`, `ecommerce.draft.and.assisted.orders.get`, `ecommerce.draft.and.assisted.orders.create`, `ecommerce.draft.and.assisted.orders.update`, and `ecommerce.draft.and.assisted.orders.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceDraftAndAssistedOrdersResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `staff_created_carts_orders`, `customer_selection`, `quotes`, `negotiated_adjustments`, `payment_links`, `impersonation_safeguards`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce draft and assisted orders API
  version: 1.0.0
paths:
  /api/v1/ecommerce/draft-and-assisted-orders:
    get:
      operationId: ecommerce.draft.and.assisted.orders.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceDraftAndAssistedOrdersResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-draft-and-assisted-orders
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommerceDraftAndAssistedOrdersResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-draft-and-assisted-orders`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-draft-and-assisted-orders`.
