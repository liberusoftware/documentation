# Ecommerce: Attribution and Analytics API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-attribution-and-analytics-api`
...
This optional API presentation package exposes approved HTTP operations for the Attribution and Analytics domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- server-side conversion adapters
- consent
- reconciliation
...
- Use `/api/v1/ecommerce/attribution-and-analytics` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/attribution-and-analytics`
...
| -------- | ---------------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/attribution-and-analytics?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/attribution-and-analytics/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/attribution-and-analytics`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/attribution-and-analytics/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/attribution-and-analytics/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/attribution-and-analytics/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

`json
{
  "canonical_commerce_events": "example-value",
  "sessions": "example-value",
  "funnels": "example-value"
}
`
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-attribution-and-analytics`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-attribution-and-analytics.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.attribution.and.analytics.list`, `ecommerce.attribution.and.analytics.get`, `ecommerce.attribution.and.analytics.create`, `ecommerce.attribution.and.analytics.update`, and `ecommerce.attribution.and.analytics.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceAttributionAndAnalyticsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `canonical_commerce_events`, `sessions`, `funnels`, `campaign_source_mapping`, `server_side_conversion_adapters`, `consent`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce attribution and analytics API
  version: 1.0.0
paths:
  /api/v1/ecommerce/attribution-and-analytics:
    get:
      operationId: ecommerce.attribution.and.analytics.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceAttributionAndAnalyticsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-attribution-and-analytics
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceAttributionAndAnalyticsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-attribution-and-analytics`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-attribution-and-analytics`.
