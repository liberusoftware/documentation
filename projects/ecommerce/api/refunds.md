# Ecommerce: Refunds API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-refunds-api`  
**Matching domain module:** `ecommerce-refunds`  
**Application:** Ecommerce  
**Source feature:** [Refunds](../features/refunds.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Refunds domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Refund calculation
- original/alternative tender policy
- approval
- provider execution
- store credit
- failure recovery
...
- Use `/api/v1/ecommerce/refunds` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/refunds`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/refunds?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/refunds/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/refunds` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/refunds/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/refunds/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/refunds/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "refund_calculation": "example-value",
  "original_alternative_tender_policy": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-refunds`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-refunds.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.refunds.list`, `ecommerce.refunds.get`, `ecommerce.refunds.create`, `ecommerce.refunds.update`, and `ecommerce.refunds.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceRefundsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `refund_calculation`, `original_alternative_tender_policy`, `approval`, `provider_execution`, `store_credit`, `failure_recovery`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce refunds API
  version: 1.0.0
paths:
  /api/v1/ecommerce/refunds:
    get:
      operationId: ecommerce.refunds.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceRefundsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-refunds`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-refunds`.
