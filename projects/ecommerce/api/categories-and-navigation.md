# Ecommerce: Categories and Navigation API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-categories-and-navigation-api`  
**Matching domain module:** `ecommerce-categories-and-navigation`  
**Application:** Ecommerce  
**Source feature:** [Categories and Navigation](../features/categories-and-navigation.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Categories and Navigation domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/categories-and-navigation` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/categories-and-navigation`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/categories-and-navigation?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/categories-and-navigation/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/categories-and-navigation` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/categories-and-navigation/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/categories-and-navigation/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/categories-and-navigation/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "hierarchies": "example-value",
  "menus": "example-value",
  "landing_references": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-categories-and-navigation`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-categories-and-navigation.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.categories.and.navigation.list`, `ecommerce.categories.and.navigation.get`, `ecommerce.categories.and.navigation.create`, `ecommerce.categories.and.navigation.update`, and `ecommerce.categories.and.navigation.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceCategoriesAndNavigationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `hierarchies`, `menus`, `landing_references`, `layered_navigation`, `permissions`, `merchandising_positions`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce categories and navigation API
  version: 1.0.0
paths:
  /api/v1/ecommerce/categories-and-navigation:
    get:
      operationId: ecommerce.categories.and.navigation.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceCategoriesAndNavigationResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-categories-and-navigation
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceCategoriesAndNavigationResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-categories-and-navigation`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-categories-and-navigation`.
