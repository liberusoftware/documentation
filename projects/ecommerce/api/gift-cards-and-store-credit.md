# Ecommerce: Gift Cards and Store Credit API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-gift-cards-and-store-credit-api`  
...
This optional API presentation package exposes approved HTTP operations for the Gift Cards and Store Credit domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/gift-cards-and-store-credit` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/gift-cards-and-store-credit`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/gift-cards-and-store-credit?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/gift-cards-and-store-credit/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/gift-cards-and-store-credit` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/gift-cards-and-store-credit/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/gift-cards-and-store-credit/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/gift-cards-and-store-credit/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-gift-cards-and-store-credit`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-gift-cards-and-store-credit.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.gift.cards.and.store.credit.list`, `ecommerce.gift.cards.and.store.credit.get`, `ecommerce.gift.cards.and.store.credit.create`, `ecommerce.gift.cards.and.store.credit.update`, and `ecommerce.gift.cards.and.store.credit.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceGiftCardsAndStoreCreditResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `issue`, `activate`, `send`, `redeem`, `partial_use`, `balances`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce gift cards and store credit API
  version: 1.0.0
paths:
  /api/v1/ecommerce/gift-cards-and-store-credit:
    get:
      operationId: ecommerce.gift.cards.and.store.credit.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceGiftCardsAndStoreCreditResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-gift-cards-and-store-credit
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceGiftCardsAndStoreCreditResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-gift-cards-and-store-credit`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-gift-cards-and-store-credit`.
