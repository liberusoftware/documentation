# Ecommerce: Fraud Intelligence API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-fraud-intelligence-api`  
**Matching domain module:** `ecommerce-fraud-intelligence`  
**Application:** Ecommerce  
**Source feature:** [Fraud Intelligence](../features/fraud-intelligence.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Fraud Intelligence domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/fraud-intelligence` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/fraud-intelligence`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/fraud-intelligence?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/fraud-intelligence/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/fraud-intelligence` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/fraud-intelligence/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/fraud-intelligence/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/fraud-intelligence/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "risk_features": "example-value",
  "model_provider_scores": "example-value",
  "decision_explanations": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-fraud-intelligence`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-fraud-intelligence.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.fraud.intelligence.list`, `ecommerce.fraud.intelligence.get`, `ecommerce.fraud.intelligence.create`, `ecommerce.fraud.intelligence.update`, and `ecommerce.fraud.intelligence.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceFraudIntelligenceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `risk_features`, `model_provider_scores`, `decision_explanations`, `drift`, `review_quality`, `chargeback_feedback`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce fraud intelligence API
  version: 1.0.0
paths:
  /api/v1/ecommerce/fraud-intelligence:
    get:
      operationId: ecommerce.fraud.intelligence.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceFraudIntelligenceResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-fraud-intelligence
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceFraudIntelligenceResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-fraud-intelligence`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-fraud-intelligence`.
