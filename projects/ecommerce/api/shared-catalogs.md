# Ecommerce: Shared Catalogs API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-shared-catalogs-api`  
...
This optional API presentation package exposes approved HTTP operations for the Shared Catalogs domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Company/market-specific assortment
- price lists
- category access
- product visibility
- effective dates
...
- Use `/api/v1/ecommerce/shared-catalogs` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/shared-catalogs`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/shared-catalogs?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/shared-catalogs/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/shared-catalogs` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/shared-catalogs/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/shared-catalogs/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/shared-catalogs/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "company_market_specific_assortment": "example-value",
  "price_lists": "example-value",
  "category_access": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-shared-catalogs`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-shared-catalogs.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.shared.catalogs.list`, `ecommerce.shared.catalogs.get`, `ecommerce.shared.catalogs.create`, `ecommerce.shared.catalogs.update`, and `ecommerce.shared.catalogs.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceSharedCatalogsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `company_market_specific_assortment`, `price_lists`, `category_access`, `product_visibility`, `effective_dates`, `inheritance`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce shared catalogs API
  version: 1.0.0
paths:
  /api/v1/ecommerce/shared-catalogs:
    get:
      operationId: ecommerce.shared.catalogs.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceSharedCatalogsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-shared-catalogs
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceSharedCatalogsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-shared-catalogs`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-shared-catalogs`.
