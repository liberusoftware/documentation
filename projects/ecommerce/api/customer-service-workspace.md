...
**API package:** `module-ecommerce-customer-service-workspace-api`  
...
- Use `/api/v1/ecommerce/customer-service-workspace` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/customer-service-workspace`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/customer-service-workspace?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/customer-service-workspace/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/customer-service-workspace` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/customer-service-workspace/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/customer-service-workspace/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/customer-service-workspace/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "orders": "example-value",
  "payments": "example-value",
  "shipments": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-customer-service-workspace`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-customer-service-workspace.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.customer.service.workspace.list`, `ecommerce.customer.service.workspace.get`, `ecommerce.customer.service.workspace.create`, `ecommerce.customer.service.workspace.update`, and `ecommerce.customer.service.workspace.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceCustomerServiceWorkspaceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `orders`, `payments`, `shipments`, `returns`, `customer_timeline`, `safe_actions`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce customer service workspace API
  version: 1.0.0
paths:
  /api/v1/ecommerce/customer-service-workspace:
    get:
      operationId: ecommerce.customer.service.workspace.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceCustomerServiceWorkspaceResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-customer-service-workspace
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceCustomerServiceWorkspaceResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-customer-service-workspace`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-customer-service-workspace`.
