# Ecommerce: Product Comparison API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-product-comparison-api`  
**Matching domain module:** `ecommerce-product-comparison`  
**Application:** Ecommerce  
**Source feature:** [Product Comparison](../features/product-comparison.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Product Comparison domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/product-comparison` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/product-comparison`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/product-comparison?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/product-comparison/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/product-comparison` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/product-comparison/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/product-comparison/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/product-comparison/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "comparable_attributes": "example-value",
  "saved_comparisons": "example-value",
  "difference_highlighting": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-product-comparison`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-product-comparison.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.product.comparison.list`, `ecommerce.product.comparison.get`, `ecommerce.product.comparison.create`, `ecommerce.product.comparison.update`, and `ecommerce.product.comparison.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceProductComparisonResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `comparable_attributes`, `saved_comparisons`, `difference_highlighting`, `share_links`, `channel_rules`, `accessibility`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce product comparison API
  version: 1.0.0
paths:
  /api/v1/ecommerce/product-comparison:
    get:
      operationId: ecommerce.product.comparison.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceProductComparisonResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-product-comparison
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceProductComparisonResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-product-comparison`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-product-comparison`.
