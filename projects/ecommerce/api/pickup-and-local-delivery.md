# Ecommerce: Pickup and Local Delivery API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-pickup-and-local-delivery-api`  
...
This optional API presentation package exposes approved HTTP operations for the Pickup and Local Delivery domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/pickup-and-local-delivery` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/pickup-and-local-delivery`
...
| -------- | ---------------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/pickup-and-local-delivery?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/pickup-and-local-delivery/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/pickup-and-local-delivery`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/pickup-and-local-delivery/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/pickup-and-local-delivery/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/pickup-and-local-delivery/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-pickup-and-local-delivery`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-pickup-and-local-delivery.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.pickup.and.local.delivery.list`, `ecommerce.pickup.and.local.delivery.get`, `ecommerce.pickup.and.local.delivery.create`, `ecommerce.pickup.and.local.delivery.update`, and `ecommerce.pickup.and.local.delivery.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommercePickupAndLocalDeliveryResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `locations`, `availability`, `slots`, `preparation`, `ready_for_pickup`, `verification`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce pickup and local delivery API
  version: 1.0.0
paths:
  /api/v1/ecommerce/pickup-and-local-delivery:
    get:
      operationId: ecommerce.pickup.and.local.delivery.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommercePickupAndLocalDeliveryResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-pickup-and-local-delivery
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommercePickupAndLocalDeliveryResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-pickup-and-local-delivery`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-pickup-and-local-delivery`.
