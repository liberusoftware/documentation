...
**API package:** `module-ecommerce-multi-tender-payments-api`  
...
- Use `/api/v1/ecommerce/multi-tender-payments` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/multi-tender-payments`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/multi-tender-payments?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/multi-tender-payments/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/multi-tender-payments` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/multi-tender-payments/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/multi-tender-payments/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/multi-tender-payments/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "split_payments": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-multi-tender-payments`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-multi-tender-payments.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.multi.tender.payments.list`, `ecommerce.multi.tender.payments.get`, `ecommerce.multi.tender.payments.create`, `ecommerce.multi.tender.payments.update`, and `ecommerce.multi.tender.payments.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceMultiTenderPaymentsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `split_payments`, `gift_store_credit`, `deposits`, `installments_references`, `partial_payments`, `outstanding_balances`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce multi tender payments API
  version: 1.0.0
paths:
  /api/v1/ecommerce/multi-tender-payments:
    get:
      operationId: ecommerce.multi.tender.payments.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceMultiTenderPaymentsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-multi-tender-payments
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceMultiTenderPaymentsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-multi-tender-payments`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-multi-tender-payments`.
