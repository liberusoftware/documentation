# Ecommerce: Subscription Commerce API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-subscription-commerce-api`  
...
This optional API presentation package exposes approved HTTP operations for the Subscription Commerce domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Storefront plans/options
- signup
- trial
- prepaid terms
- add-ons
...
- Use `/api/v1/ecommerce/subscription-commerce` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/subscription-commerce`
...
| -------- | ------------------------------------------------------------------------ | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/subscription-commerce?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/subscription-commerce/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/subscription-commerce`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/subscription-commerce/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/subscription-commerce/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/subscription-commerce/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "storefront_plans_options": "example-value",
  "signup": "example-value",
  "trial": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-subscription-commerce`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-subscription-commerce.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.subscription.commerce.list`, `ecommerce.subscription.commerce.get`, `ecommerce.subscription.commerce.create`, `ecommerce.subscription.commerce.update`, and `ecommerce.subscription.commerce.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceSubscriptionCommerceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `storefront_plans_options`, `signup`, `trial`, `prepaid_terms`, `add_ons`, `customer_changes`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce subscription commerce API
  version: 1.0.0
paths:
  /api/v1/ecommerce/subscription-commerce:
    get:
      operationId: ecommerce.subscription.commerce.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceSubscriptionCommerceResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-subscription-commerce
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommerceSubscriptionCommerceResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-subscription-commerce`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-subscription-commerce`.
