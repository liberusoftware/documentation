# Ecommerce: Unified Commerce API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-unified-commerce-api`  
...
This optional API presentation package exposes approved HTTP operations for the Unified Commerce domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- inventory
- promotions
- fulfillment across online/POS channels
...
- Use `/api/v1/ecommerce/unified-commerce` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/unified-commerce`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/unified-commerce?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/unified-commerce/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/unified-commerce` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/unified-commerce/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/unified-commerce/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/unified-commerce/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "shared_customer": "example-value",
  "loyalty": "example-value",
  "gift_store_credit": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-unified-commerce`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-unified-commerce.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.unified.commerce.list`, `ecommerce.unified.commerce.get`, `ecommerce.unified.commerce.create`, `ecommerce.unified.commerce.update`, and `ecommerce.unified.commerce.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceUnifiedCommerceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `shared_customer`, `loyalty`, `gift_store_credit`, `orders`, `returns`, `inventory`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce unified commerce API
  version: 1.0.0
paths:
  /api/v1/ecommerce/unified-commerce:
    get:
      operationId: ecommerce.unified.commerce.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceUnifiedCommerceResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-unified-commerce
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceUnifiedCommerceResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-unified-commerce`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-unified-commerce`.
