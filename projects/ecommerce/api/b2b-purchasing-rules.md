# Ecommerce: B2B Purchasing Rules API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-b2b-purchasing-rules-api`
**Matching domain module:** `ecommerce-b2b-purchasing-rules`
**Application:** Ecommerce
**Source feature:** [B2B Purchasing Rules](../features/b2b-purchasing-rules.md)
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the B2B Purchasing Rules domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Minimum/maximum/increment quantities
- case packs
- order thresholds
...
- Use `/api/v1/ecommerce/b2b-purchasing-rules` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/b2b-purchasing-rules`
...
| -------- | ----------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/b2b-purchasing-rules?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/b2b-purchasing-rules/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/b2b-purchasing-rules`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/b2b-purchasing-rules/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/b2b-purchasing-rules/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/b2b-purchasing-rules/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

`json
{
  "minimum_maximum_increment_quantities": "example-value",
  "case_packs": "example-value",
  "order_thresholds": "example-value"
}
`
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-b2b-purchasing-rules`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-b2b-purchasing-rules.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.b2b.purchasing.rules.list`, `ecommerce.b2b.purchasing.rules.get`, `ecommerce.b2b.purchasing.rules.create`, `ecommerce.b2b.purchasing.rules.update`, and `ecommerce.b2b.purchasing.rules.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceB2bPurchasingRulesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `minimum_maximum_increment_quantities`, `case_packs`, `order_thresholds`, `payment_shipping_methods`, `deposits`, `po_numbers`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce b2b purchasing rules API
  version: 1.0.0
paths:
  /api/v1/ecommerce/b2b-purchasing-rules:
    get:
      operationId: ecommerce.b2b.purchasing.rules.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceB2bPurchasingRulesResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-b2b-purchasing-rules
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceB2bPurchasingRulesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-b2b-purchasing-rules`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-b2b-purchasing-rules`.
