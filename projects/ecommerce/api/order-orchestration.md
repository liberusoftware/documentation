# Ecommerce: Order Orchestration API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-order-orchestration-api`  
**Matching domain module:** `ecommerce-order-orchestration`  
**Application:** Ecommerce  
**Source feature:** [Order Orchestration](../features/order-orchestration.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Order Orchestration domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/order-orchestration` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/order-orchestration`
...
| -------- | ---------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/order-orchestration?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/order-orchestration/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/order-orchestration`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/order-orchestration/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/order-orchestration/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/order-orchestration/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |
...
```json
{
  ...
  "split_routing_decisions": "example-value",
  "holds": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-order-orchestration`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-order-orchestration.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.order.orchestration.list`, `ecommerce.order.orchestration.get`, `ecommerce.order.orchestration.create`, `ecommerce.order.orchestration.update`, and `ecommerce.order.orchestration.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceOrderOrchestrationResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `dependencies`, `split_routing_decisions`, `holds`, `releases`, `backorders`, `partial_operations`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce order orchestration API
  version: 1.0.0
paths:
  /api/v1/ecommerce/order-orchestration:
    get:
      operationId: ecommerce.order.orchestration.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceOrderOrchestrationResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-order-orchestration
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommerceOrderOrchestrationResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-order-orchestration`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-order-orchestration`.
