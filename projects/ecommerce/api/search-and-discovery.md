# Ecommerce: Search and Discovery API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-search-and-discovery-api`  
**Matching domain module:** `ecommerce-search-and-discovery`  
**Application:** Ecommerce  
**Source feature:** [Search and Discovery](../features/search-and-discovery.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Search and Discovery domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/search-and-discovery` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/search-and-discovery`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/search-and-discovery?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/search-and-discovery/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/search-and-discovery` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/search-and-discovery/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/search-and-discovery/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/search-and-discovery/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "full_text_search": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-search-and-discovery`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-search-and-discovery.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.search.and.discovery.list`, `ecommerce.search.and.discovery.get`, `ecommerce.search.and.discovery.create`, `ecommerce.search.and.discovery.update`, and `ecommerce.search.and.discovery.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceSearchAndDiscoveryResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `full_text_search`, `facets`, `autocomplete`, `synonyms`, `redirects`, `ranking_rules`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce search and discovery API
  version: 1.0.0
paths:
  /api/v1/ecommerce/search-and-discovery:
    get:
      operationId: ecommerce.search.and.discovery.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceSearchAndDiscoveryResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-search-and-discovery
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceSearchAndDiscoveryResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-search-and-discovery`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-search-and-discovery`.
