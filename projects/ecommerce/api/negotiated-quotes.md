# Ecommerce: Negotiated Quotes API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-negotiated-quotes-api`  
**Matching domain module:** `ecommerce-negotiated-quotes`  
**Application:** Ecommerce  
**Source feature:** [Negotiated Quotes](../features/negotiated-quotes.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Negotiated Quotes domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/negotiated-quotes` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/negotiated-quotes`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/negotiated-quotes?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/negotiated-quotes/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/negotiated-quotes` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/negotiated-quotes/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/negotiated-quotes/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/negotiated-quotes/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "buyer_seller_negotiation": "example-value",
  "messages": "example-value",
  "line_price_changes": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-negotiated-quotes`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-negotiated-quotes.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.negotiated.quotes.list`, `ecommerce.negotiated.quotes.get`, `ecommerce.negotiated.quotes.create`, `ecommerce.negotiated.quotes.update`, and `ecommerce.negotiated.quotes.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceNegotiatedQuotesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `buyer_seller_negotiation`, `messages`, `line_price_changes`, `versions`, `expiry`, `approval`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce negotiated quotes API
  version: 1.0.0
paths:
  /api/v1/ecommerce/negotiated-quotes:
    get:
      operationId: ecommerce.negotiated.quotes.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceNegotiatedQuotesResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-negotiated-quotes
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceNegotiatedQuotesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-negotiated-quotes`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-negotiated-quotes`.
