# Ecommerce: Sales Representative Workspace API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-sales-representative-workspace-api`  
**Matching domain module:** `ecommerce-sales-representative-workspace`  
**Application:** Ecommerce  
**Source feature:** [Sales Representative Workspace](../features/sales-representative-workspace.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Sales Representative Workspace domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Assigned companies
- assisted orders/quotes
- account pricing
- activity
- approvals
- order status
...
- Use `/api/v1/ecommerce/sales-representative-workspace` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/sales-representative-workspace`
...
| -------- | --------------------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/sales-representative-workspace?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/sales-representative-workspace/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/sales-representative-workspace`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/sales-representative-workspace/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/sales-representative-workspace/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/sales-representative-workspace/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "assigned_companies": "example-value",
  "assisted_orders_quotes": "example-value",
  "account_pricing": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-sales-representative-workspace`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-sales-representative-workspace.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.sales.representative.workspace.list`, `ecommerce.sales.representative.workspace.get`, `ecommerce.sales.representative.workspace.create`, `ecommerce.sales.representative.workspace.update`, and `ecommerce.sales.representative.workspace.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceSalesRepresentativeWorkspaceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `assigned_companies`, `assisted_orders_quotes`, `account_pricing`, `activity`, `approvals`, `order_status`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce sales representative workspace API
  version: 1.0.0
paths:
  /api/v1/ecommerce/sales-representative-workspace:
    get:
      operationId: ecommerce.sales.representative.workspace.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceSalesRepresentativeWorkspaceResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-sales-representative-workspace
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommerceSalesRepresentativeWorkspaceResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-sales-representative-workspace`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-sales-representative-workspace`.
