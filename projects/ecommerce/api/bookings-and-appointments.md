# Ecommerce: Bookings and Appointments API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-bookings-and-appointments-api`
**Matching domain module:** `ecommerce-bookings-and-appointments`
**Application:** Ecommerce
**Source feature:** [Bookings and Appointments](../features/bookings-and-appointments.md)
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Bookings and Appointments domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/bookings-and-appointments` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/bookings-and-appointments`
...
| -------- | ---------------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/bookings-and-appointments?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/bookings-and-appointments/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/bookings-and-appointments`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/bookings-and-appointments/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/bookings-and-appointments/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/bookings-and-appointments/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-bookings-and-appointments`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-bookings-and-appointments.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.bookings.and.appointments.list`, `ecommerce.bookings.and.appointments.get`, `ecommerce.bookings.and.appointments.create`, `ecommerce.bookings.and.appointments.update`, and `ecommerce.bookings.and.appointments.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceBookingsAndAppointmentsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `resources`, `services`, `staff`, `availability`, `durations`, `capacity`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce bookings and appointments API
  version: 1.0.0
paths:
  /api/v1/ecommerce/bookings-and-appointments:
    get:
      operationId: ecommerce.bookings.and.appointments.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceBookingsAndAppointmentsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-bookings-and-appointments
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommerceBookingsAndAppointmentsResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-bookings-and-appointments`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-bookings-and-appointments`.
