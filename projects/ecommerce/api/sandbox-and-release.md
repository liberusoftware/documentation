# Ecommerce: Sandbox and Release API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-sandbox-and-release-api`  
...
This optional API presentation package exposes approved HTTP operations for the Sandbox and Release domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Test stores/data policy
- configuration/catalog snapshots
...
- Use `/api/v1/ecommerce/sandbox-and-release` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/sandbox-and-release`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/sandbox-and-release?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/sandbox-and-release/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/sandbox-and-release` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/sandbox-and-release/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/sandbox-and-release/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/sandbox-and-release/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "test_stores_data_policy": "example-value",
  "configuration_catalog_snapshots": "example-value",
  "dependency_comparison": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-sandbox-and-release`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-sandbox-and-release.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.sandbox.and.release.list`, `ecommerce.sandbox.and.release.get`, `ecommerce.sandbox.and.release.create`, `ecommerce.sandbox.and.release.update`, and `ecommerce.sandbox.and.release.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceSandboxAndReleaseResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `test_stores_data_policy`, `configuration_catalog_snapshots`, `dependency_comparison`, `staging`, `promotion`, `rollback`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce sandbox and release API
  version: 1.0.0
paths:
  /api/v1/ecommerce/sandbox-and-release:
    get:
      operationId: ecommerce.sandbox.and.release.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceSandboxAndReleaseResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-sandbox-and-release
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceSandboxAndReleaseResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-sandbox-and-release`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-sandbox-and-release`.
