# Ecommerce: Checkout Extensibility API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-checkout-extensibility-api`  
**Matching domain module:** `ecommerce-checkout-extensibility`  
**Application:** Ecommerce  
**Source feature:** [Checkout Extensibility](../features/checkout-extensibility.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Checkout Extensibility domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/checkout-extensibility` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/checkout-extensibility`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/checkout-extensibility?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/checkout-extensibility/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/checkout-extensibility` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/checkout-extensibility/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/checkout-extensibility/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/checkout-extensibility/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "validated_extension_points": "example-value",
  "ui_blocks": "example-value",
  "delivery_payment_customization": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-checkout-extensibility`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-checkout-extensibility.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.checkout.extensibility.list`, `ecommerce.checkout.extensibility.get`, `ecommerce.checkout.extensibility.create`, `ecommerce.checkout.extensibility.update`, and `ecommerce.checkout.extensibility.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceCheckoutExtensibilityResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `validated_extension_points`, `ui_blocks`, `delivery_payment_customization`, `business_rules`, `scopes`, `compatibility`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce checkout extensibility API
  version: 1.0.0
paths:
  /api/v1/ecommerce/checkout-extensibility:
    get:
      operationId: ecommerce.checkout.extensibility.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceCheckoutExtensibilityResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-checkout-extensibility
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceCheckoutExtensibilityResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-checkout-extensibility`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-checkout-extensibility`.
