# Ecommerce: Fraud and Risk API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-fraud-and-risk-api`  
**Matching domain module:** `ecommerce-fraud-and-risk`  
**Application:** Ecommerce  
**Source feature:** [Fraud and Risk](../features/fraud-and-risk.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Fraud and Risk domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Rules
- velocity
- identity/device/provider signals
- review queues
- holds
- decisions
- evidence
...
- Use `/api/v1/ecommerce/fraud-and-risk` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/fraud-and-risk`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/fraud-and-risk?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/fraud-and-risk/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/fraud-and-risk` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/fraud-and-risk/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/fraud-and-risk/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/fraud-and-risk/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
```json
{
  ...
  "identity_device_provider_signals": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-fraud-and-risk`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-fraud-and-risk.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.fraud.and.risk.list`, `ecommerce.fraud.and.risk.get`, `ecommerce.fraud.and.risk.create`, `ecommerce.fraud.and.risk.update`, and `ecommerce.fraud.and.risk.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceFraudAndRiskResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `rules`, `velocity`, `identity_device_provider_signals`, `review_queues`, `holds`, `decisions`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce fraud and risk API
  version: 1.0.0
paths:
  /api/v1/ecommerce/fraud-and-risk:
    get:
      operationId: ecommerce.fraud.and.risk.list
      security:
        - sanctum: []
      parameters:
  ...
  schemas:
    EcommerceFraudAndRiskResource:
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-fraud-and-risk
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceFraudAndRiskResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-fraud-and-risk`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-fraud-and-risk`.
