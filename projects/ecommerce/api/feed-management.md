# Ecommerce: Feed Management API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-feed-management-api`  
**Matching domain module:** `ecommerce-feed-management`  
**Application:** Ecommerce  
**Source feature:** [Feed Management](../features/feed-management.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Feed Management domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Search/ad/affiliate product feeds
- field mappings
- rules
- schedules
- validation
...
- Use `/api/v1/ecommerce/feed-management` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/feed-management`
...
| -------- | ------------------------------------------------------------------ | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/feed-management?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/feed-management/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/feed-management`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/feed-management/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/feed-management/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/feed-management/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "search_ad_affiliate_product_feeds": "example-value",
  "field_mappings": "example-value",
  "rules": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-feed-management`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-feed-management.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.feed.management.list`, `ecommerce.feed.management.get`, `ecommerce.feed.management.create`, `ecommerce.feed.management.update`, and `ecommerce.feed.management.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceFeedManagementResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `search_ad_affiliate_product_feeds`, `field_mappings`, `rules`, `schedules`, `validation`, `destination_errors`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce feed management API
  version: 1.0.0
paths:
  /api/v1/ecommerce/feed-management:
    get:
      operationId: ecommerce.feed.management.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceFeedManagementResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-feed-management
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommerceFeedManagementResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-feed-management`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-feed-management`.
