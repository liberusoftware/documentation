# Ecommerce: Digital Fulfillment API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-digital-fulfillment-api`  
**Matching domain module:** `ecommerce-digital-fulfillment`  
**Application:** Ecommerce  
**Source feature:** [Digital Fulfillment](../features/digital-fulfillment.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Digital Fulfillment domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Entitlements
- secure downloads/streams
- limits
- expiry
- license-key adapters
- version access
- revocation
- evidence
...
- Use `/api/v1/ecommerce/digital-fulfillment` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/digital-fulfillment`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/digital-fulfillment?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/digital-fulfillment/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/digital-fulfillment` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/digital-fulfillment/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/digital-fulfillment/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/digital-fulfillment/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "entitlements": "example-value",
  "secure_downloads_streams": "example-value",
  "limits": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-digital-fulfillment`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-digital-fulfillment.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.digital.fulfillment.list`, `ecommerce.digital.fulfillment.get`, `ecommerce.digital.fulfillment.create`, `ecommerce.digital.fulfillment.update`, and `ecommerce.digital.fulfillment.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceDigitalFulfillmentResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `entitlements`, `secure_downloads_streams`, `limits`, `expiry`, `license_key_adapters`, `version_access`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce digital fulfillment API
  version: 1.0.0
paths:
  /api/v1/ecommerce/digital-fulfillment:
    get:
      operationId: ecommerce.digital.fulfillment.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceDigitalFulfillmentResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-digital-fulfillment
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceDigitalFulfillmentResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-digital-fulfillment`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-digital-fulfillment`.
