# Ecommerce: Order Tracking API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-order-tracking-api`  
**Matching domain module:** `ecommerce-order-tracking`  
**Application:** Ecommerce  
**Source feature:** [Order Tracking](../features/order-tracking.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Order Tracking domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Unified fulfillment/tracking timeline
- delivery estimates
- exceptions
- subscriptions
- guest access
...
- Use `/api/v1/ecommerce/order-tracking` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/order-tracking`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/order-tracking?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/order-tracking/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/order-tracking` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/order-tracking/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/order-tracking/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/order-tracking/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "unified_fulfillment_tracking_timeline": "example-value",
  "delivery_estimates": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-order-tracking`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-order-tracking.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.order.tracking.list`, `ecommerce.order.tracking.get`, `ecommerce.order.tracking.create`, `ecommerce.order.tracking.update`, and `ecommerce.order.tracking.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceOrderTrackingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `unified_fulfillment_tracking_timeline`, `delivery_estimates`, `exceptions`, `subscriptions`, `guest_access`, `carrier_fallback`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce order tracking API
  version: 1.0.0
paths:
  /api/v1/ecommerce/order-tracking:
    get:
      operationId: ecommerce.order.tracking.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceOrderTrackingResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-order-tracking
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceOrderTrackingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-order-tracking`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-order-tracking`.
