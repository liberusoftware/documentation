# Ecommerce: Customer Accounts API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-customer-accounts-api`  
**Matching domain module:** `ecommerce-customer-accounts`  
**Application:** Ecommerce  
**Source feature:** [Customer Accounts](../features/customer-accounts.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Customer Accounts domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Registration/guest conversion
- order history
- returns
- saved carts/lists
...
- Use `/api/v1/ecommerce/customer-accounts` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/customer-accounts`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/customer-accounts?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/customer-accounts/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/customer-accounts` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/customer-accounts/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/customer-accounts/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/customer-accounts/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "registration_guest_conversion": "example-value",
  "order_history": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-customer-accounts`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-customer-accounts.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.customer.accounts.list`, `ecommerce.customer.accounts.get`, `ecommerce.customer.accounts.create`, `ecommerce.customer.accounts.update`, and `ecommerce.customer.accounts.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceCustomerAccountsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `registration_guest_conversion`, `order_history`, `returns`, `saved_carts_lists`, `payment_method_references`, `preferences`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce customer accounts API
  version: 1.0.0
paths:
  /api/v1/ecommerce/customer-accounts:
    get:
      operationId: ecommerce.customer.accounts.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceCustomerAccountsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-customer-accounts
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceCustomerAccountsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-customer-accounts`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-customer-accounts`.
