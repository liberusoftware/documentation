# Ecommerce: Sales Channels API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-sales-channels-api`  
**Matching domain module:** `ecommerce-sales-channels`  
**Application:** Ecommerce  
**Source feature:** [Sales Channels](../features/sales-channels.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Sales Channels domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Channel registry
- assortment/pricing/inventory/order mappings
- publication
- sync cursors
- conflicts
- health
...
- Use `/api/v1/ecommerce/sales-channels` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/sales-channels`
...
| -------- | ----------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/sales-channels?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/sales-channels/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/sales-channels`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/sales-channels/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/sales-channels/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/sales-channels/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "channel_registry": "example-value",
  "assortment_pricing_inventory_order_mappings": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-sales-channels`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-sales-channels.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.sales.channels.list`, `ecommerce.sales.channels.get`, `ecommerce.sales.channels.create`, `ecommerce.sales.channels.update`, and `ecommerce.sales.channels.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceSalesChannelsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `channel_registry`, `assortment_pricing_inventory_order_mappings`, `publication`, `sync_cursors`, `conflicts`, `health`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce sales channels API
  version: 1.0.0
paths:
  /api/v1/ecommerce/sales-channels:
    get:
      operationId: ecommerce.sales.channels.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceSalesChannelsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-sales-channels
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommerceSalesChannelsResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-sales-channels`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-sales-channels`.
