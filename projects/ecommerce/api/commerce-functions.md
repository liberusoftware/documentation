# Ecommerce: Commerce Functions API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-commerce-functions-api`  
**Matching domain module:** `ecommerce-commerce-functions`  
**Application:** Ecommerce  
**Source feature:** [Commerce Functions](../features/commerce-functions.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Commerce Functions domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Sandboxed typed hooks for discounts
- validation
- delivery/payment customization
- routing
- transformations with limits and diagnostics
...
- Use `/api/v1/ecommerce/commerce-functions` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/commerce-functions`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/commerce-functions?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/commerce-functions/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/commerce-functions` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/commerce-functions/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/commerce-functions/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/commerce-functions/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "sandboxed_typed_hooks_for_discounts": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-commerce-functions`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-commerce-functions.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.commerce.functions.list`, `ecommerce.commerce.functions.get`, `ecommerce.commerce.functions.create`, `ecommerce.commerce.functions.update`, and `ecommerce.commerce.functions.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceCommerceFunctionsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `sandboxed_typed_hooks_for_discounts`, `validation`, `delivery_payment_customization`, `routing`, `transformations_with_limits_and_diagnostics`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce commerce functions API
  version: 1.0.0
paths:
  /api/v1/ecommerce/commerce-functions:
    get:
      operationId: ecommerce.commerce.functions.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceCommerceFunctionsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-commerce-functions
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceCommerceFunctionsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-commerce-functions`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-commerce-functions`.
