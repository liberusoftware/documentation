# Ecommerce: Store Templates API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-store-templates-api`  
...
This optional API presentation package exposes approved HTTP operations for the Store Templates domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Versioned bundles of configuration
- catalog samples
- pages
- navigation
- workflows
...
- Use `/api/v1/ecommerce/store-templates` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/store-templates`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/store-templates?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/store-templates/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/store-templates` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/store-templates/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/store-templates/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/store-templates/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "versioned_bundles_of_configuration": "example-value",
  "catalog_samples": "example-value",
  "pages": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-store-templates`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-store-templates.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.store.templates.list`, `ecommerce.store.templates.get`, `ecommerce.store.templates.create`, `ecommerce.store.templates.update`, and `ecommerce.store.templates.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceStoreTemplatesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `versioned_bundles_of_configuration`, `catalog_samples`, `pages`, `navigation`, `workflows`, `themes`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce store templates API
  version: 1.0.0
paths:
  /api/v1/ecommerce/store-templates:
    get:
      operationId: ecommerce.store.templates.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceStoreTemplatesResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-store-templates
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceStoreTemplatesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-store-templates`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-store-templates`.
