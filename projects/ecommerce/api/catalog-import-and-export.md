# Ecommerce: Catalog Import and Export API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-catalog-import-and-export-api`  
**Matching domain module:** `ecommerce-catalog-import-and-export`  
**Application:** Ecommerce  
**Source feature:** [Catalog Import and Export](../features/catalog-import-and-export.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Catalog Import and Export domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/catalog-import-and-export` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/catalog-import-and-export`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/catalog-import-and-export?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/catalog-import-and-export/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/catalog-import-and-export` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/catalog-import-and-export/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/catalog-import-and-export/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/catalog-import-and-export/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-catalog-import-and-export`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-catalog-import-and-export.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.catalog.import.and.export.list`, `ecommerce.catalog.import.and.export.get`, `ecommerce.catalog.import.and.export.create`, `ecommerce.catalog.import.and.export.update`, and `ecommerce.catalog.import.and.export.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceCatalogImportAndExportResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `schemas`, `mappings`, `validation`, `dry_run`, `media_relationship_resolution`, `batches`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce catalog import and export API
  version: 1.0.0
paths:
  /api/v1/ecommerce/catalog-import-and-export:
    get:
      operationId: ecommerce.catalog.import.and.export.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceCatalogImportAndExportResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-catalog-import-and-export
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceCatalogImportAndExportResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-catalog-import-and-export`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-catalog-import-and-export`.
