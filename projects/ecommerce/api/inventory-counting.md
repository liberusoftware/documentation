# Ecommerce: Inventory Counting API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-inventory-counting-api`  
...
This optional API presentation package exposes approved HTTP operations for the Inventory Counting domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/inventory-counting` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/inventory-counting`
...
| -------- | --------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/inventory-counting?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/inventory-counting/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/inventory-counting`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/inventory-counting/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/inventory-counting/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/inventory-counting/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "cycle_full_counts": "example-value",
  "blind_counts": "example-value",
  "approvals": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-inventory-counting`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-inventory-counting.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.inventory.counting.list`, `ecommerce.inventory.counting.get`, `ecommerce.inventory.counting.create`, `ecommerce.inventory.counting.update`, and `ecommerce.inventory.counting.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceInventoryCountingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `cycle_full_counts`, `blind_counts`, `approvals`, `variances`, `adjustments`, `freeze_policy`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce inventory counting API
  version: 1.0.0
paths:
  /api/v1/ecommerce/inventory-counting:
    get:
      operationId: ecommerce.inventory.counting.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceInventoryCountingResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-inventory-counting
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommerceInventoryCountingResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-inventory-counting`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-inventory-counting`.
