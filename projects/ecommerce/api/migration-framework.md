# Ecommerce: Migration Framework API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-migration-framework-api`  
**Matching domain module:** `ecommerce-migration-framework`  
...
This optional API presentation package exposes approved HTTP operations for the Migration Framework domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Shopify
- generic source adapters with mappings
- dry run
- batches
- resume
- media
...
- Use `/api/v1/ecommerce/migration-framework` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/migration-framework`
...
| -------- | ---------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/migration-framework?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/migration-framework/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/migration-framework`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/migration-framework/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/migration-framework/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/migration-framework/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "woocommerce": "example-value",
  "magento_adobe_commerce": "example-value",
  "shopify": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-migration-framework`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-migration-framework.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.migration.framework.list`, `ecommerce.migration.framework.get`, `ecommerce.migration.framework.create`, `ecommerce.migration.framework.update`, and `ecommerce.migration.framework.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceMigrationFrameworkResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `woocommerce`, `magento_adobe_commerce`, `shopify`, `generic_source_adapters_with_mappings`, `dry_run`, `batches`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce migration framework API
  version: 1.0.0
paths:
  /api/v1/ecommerce/migration-framework:
    get:
      operationId: ecommerce.migration.framework.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceMigrationFrameworkResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-migration-framework
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommerceMigrationFrameworkResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-migration-framework`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-migration-framework`.
