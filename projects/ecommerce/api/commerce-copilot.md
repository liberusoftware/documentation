# Ecommerce: Commerce Copilot API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-commerce-copilot-api`  
**Matching domain module:** `ecommerce-commerce-copilot`  
**Application:** Ecommerce  
**Source feature:** [Commerce Copilot](../features/commerce-copilot.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Commerce Copilot domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Permission-bounded product/order/customer search
- summaries
- catalog assistance
...
- Use `/api/v1/ecommerce/commerce-copilot` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/commerce-copilot`
...
| -------- | ------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/commerce-copilot?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/commerce-copilot/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/commerce-copilot`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/commerce-copilot/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/commerce-copilot/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/commerce-copilot/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "permission_bounded_product_order_customer_search": "example-value",
  "summaries": "example-value",
  "catalog_assistance": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-commerce-copilot`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-commerce-copilot.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.commerce.copilot.list`, `ecommerce.commerce.copilot.get`, `ecommerce.commerce.copilot.create`, `ecommerce.commerce.copilot.update`, and `ecommerce.commerce.copilot.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceCommerceCopilotResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `permission_bounded_product_order_customer_search`, `summaries`, `catalog_assistance`, `safe_operational_actions`, `confirmation`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce commerce copilot API
  version: 1.0.0
paths:
  /api/v1/ecommerce/commerce-copilot:
    get:
      operationId: ecommerce.commerce.copilot.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceCommerceCopilotResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-commerce-copilot
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommerceCommerceCopilotResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-commerce-copilot`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-commerce-copilot`.
