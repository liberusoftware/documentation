# Ecommerce: Transfers and Replenishment API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-transfers-and-replenishment-api`  
**Matching domain module:** `ecommerce-transfers-and-replenishment`  
**Application:** Ecommerce  
**Source feature:** [Transfers and Replenishment](../features/transfers-and-replenishment.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Transfers and Replenishment domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/transfers-and-replenishment` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/transfers-and-replenishment`
...
| -------- | ------------------------------------------------------------------------------ | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/transfers-and-replenishment?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/transfers-and-replenishment/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/transfers-and-replenishment`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/transfers-and-replenishment/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/transfers-and-replenishment/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/transfers-and-replenishment/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "transfer_orders": "example-value",
  "in_transit_stock": "example-value",
  "min_max_reorder": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-transfers-and-replenishment`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-transfers-and-replenishment.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.transfers.and.replenishment.list`, `ecommerce.transfers.and.replenishment.get`, `ecommerce.transfers.and.replenishment.create`, `ecommerce.transfers.and.replenishment.update`, and `ecommerce.transfers.and.replenishment.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceTransfersAndReplenishmentResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `transfer_orders`, `in_transit_stock`, `min_max_reorder`, `demand_signals`, `purchase_references`, `receiving`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce transfers and replenishment API
  version: 1.0.0
paths:
  /api/v1/ecommerce/transfers-and-replenishment:
    get:
      operationId: ecommerce.transfers.and.replenishment.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceTransfersAndReplenishmentResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-transfers-and-replenishment
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommerceTransfersAndReplenishmentResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-transfers-and-replenishment`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-transfers-and-replenishment`.
