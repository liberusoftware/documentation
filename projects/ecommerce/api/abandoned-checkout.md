# Ecommerce: Abandoned Checkout API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-abandoned-checkout-api`  
**Matching domain module:** `ecommerce-abandoned-checkout`  
**Application:** Ecommerce  
**Source feature:** [Abandoned Checkout](../features/abandoned-checkout.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Abandoned Checkout domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/abandoned-checkout` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/abandoned-checkout`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/abandoned-checkout?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/abandoned-checkout/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/abandoned-checkout` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/abandoned-checkout/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/abandoned-checkout/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/abandoned-checkout/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "cart_checkout_state": "example-value",
  "consent_aware_reminders": "example-value",
  "recovery_links": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-abandoned-checkout`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-abandoned-checkout.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.abandoned.checkout.list`, `ecommerce.abandoned.checkout.get`, `ecommerce.abandoned.checkout.create`, `ecommerce.abandoned.checkout.update`, and `ecommerce.abandoned.checkout.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceAbandonedCheckoutResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `cart_checkout_state`, `consent_aware_reminders`, `recovery_links`, `incentives`, `suppression`, `attribution`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce abandoned checkout API
  version: 1.0.0
paths:
  /api/v1/ecommerce/abandoned-checkout:
    get:
      operationId: ecommerce.abandoned.checkout.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceAbandonedCheckoutResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-abandoned-checkout
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceAbandonedCheckoutResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-abandoned-checkout`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-abandoned-checkout`.
