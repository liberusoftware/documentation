# Ecommerce: Express Checkout API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-express-checkout-api`  
...
This optional API presentation package exposes approved HTTP operations for the Express Checkout domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Wallet/provider accelerated checkout
- known-customer context
- shipping/payment callbacks
- totals validation
- fallback
...
- Use `/api/v1/ecommerce/express-checkout` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/express-checkout`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/express-checkout?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/express-checkout/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/express-checkout` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/express-checkout/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/express-checkout/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/express-checkout/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "wallet_provider_accelerated_checkout": "example-value",
  "known_customer_context": "example-value",
  "shipping_payment_callbacks": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-express-checkout`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-express-checkout.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.express.checkout.list`, `ecommerce.express.checkout.get`, `ecommerce.express.checkout.create`, `ecommerce.express.checkout.update`, and `ecommerce.express.checkout.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceExpressCheckoutResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `wallet_provider_accelerated_checkout`, `known_customer_context`, `shipping_payment_callbacks`, `totals_validation`, `fallback`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce express checkout API
  version: 1.0.0
paths:
  /api/v1/ecommerce/express-checkout:
    get:
      operationId: ecommerce.express.checkout.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceExpressCheckoutResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-express-checkout
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceExpressCheckoutResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-express-checkout`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-express-checkout`.
