# Ecommerce: Order Editing API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-order-editing-api`  
**Matching domain module:** `ecommerce-order-editing`  
**Application:** Ecommerce  
**Source feature:** [Order Editing](../features/order-editing.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Order Editing domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Governed pre/post-fulfillment changes
- recalculation
- payment difference
- stock impact
- approvals
...
- Use `/api/v1/ecommerce/order-editing` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/order-editing`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/order-editing?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/order-editing/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/order-editing` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/order-editing/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/order-editing/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/order-editing/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "governed_pre_post_fulfillment_changes": "example-value",
  "recalculation": "example-value",
  "payment_difference": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-order-editing`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-order-editing.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.order.editing.list`, `ecommerce.order.editing.get`, `ecommerce.order.editing.create`, `ecommerce.order.editing.update`, and `ecommerce.order.editing.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceOrderEditingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `governed_pre_post_fulfillment_changes`, `recalculation`, `payment_difference`, `stock_impact`, `approvals`, `customer_notice`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce order editing API
  version: 1.0.0
paths:
  /api/v1/ecommerce/order-editing:
    get:
      operationId: ecommerce.order.editing.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceOrderEditingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-order-editing`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-order-editing`.
