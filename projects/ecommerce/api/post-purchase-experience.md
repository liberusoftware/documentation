# Ecommerce: Post-Purchase Experience API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-post-purchase-experience-api`  
...
This optional API presentation package exposes approved HTTP operations for the Post-Purchase Experience domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/post-purchase-experience` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/post-purchase-experience`
...
| -------- | --------------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/post-purchase-experience?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/post-purchase-experience/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/post-purchase-experience`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/post-purchase-experience/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/post-purchase-experience/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/post-purchase-experience/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "branded_status": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-post-purchase-experience`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-post-purchase-experience.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.post.purchase.experience.list`, `ecommerce.post.purchase.experience.get`, `ecommerce.post.purchase.experience.create`, `ecommerce.post.purchase.experience.update`, and `ecommerce.post.purchase.experience.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommercePostPurchaseExperienceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `branded_status`, `instructions`, `documents`, `product_registration`, `review_referral_prompts`, `support_links`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce post purchase experience API
  version: 1.0.0
paths:
  /api/v1/ecommerce/post-purchase-experience:
    get:
      operationId: ecommerce.post.purchase.experience.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommercePostPurchaseExperienceResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-post-purchase-experience
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommercePostPurchaseExperienceResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-post-purchase-experience`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-post-purchase-experience`.
