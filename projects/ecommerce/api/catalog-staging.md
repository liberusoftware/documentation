# Ecommerce: Catalog Staging API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-catalog-staging-api`  
...
This optional API presentation package exposes approved HTTP operations for the Catalog Staging domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Draft/effective catalog changes
- grouped campaigns
- preview
- schedules
- conflicts
...
- Use `/api/v1/ecommerce/catalog-staging` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/catalog-staging`
...
| -------- | ------------------------------------------------------------------ | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/catalog-staging?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/catalog-staging/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/catalog-staging`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/catalog-staging/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/catalog-staging/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/catalog-staging/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "draft_effective_catalog_changes": "example-value",
  "grouped_campaigns": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-catalog-staging`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-catalog-staging.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.catalog.staging.list`, `ecommerce.catalog.staging.get`, `ecommerce.catalog.staging.create`, `ecommerce.catalog.staging.update`, and `ecommerce.catalog.staging.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceCatalogStagingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `draft_effective_catalog_changes`, `grouped_campaigns`, `preview`, `schedules`, `conflicts`, `approvals`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce catalog staging API
  version: 1.0.0
paths:
  /api/v1/ecommerce/catalog-staging:
    get:
      operationId: ecommerce.catalog.staging.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceCatalogStagingResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-catalog-staging
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommerceCatalogStagingResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-catalog-staging`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-catalog-staging`.
