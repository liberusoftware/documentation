# Ecommerce: Digital Assets API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-digital-assets-api`  
...
This optional API presentation package exposes approved HTTP operations for the Digital Assets domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/ecommerce/digital-assets` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/digital-assets`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/digital-assets?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/digital-assets/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/digital-assets` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/digital-assets/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/digital-assets/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/digital-assets/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "commerce_media": "example-value",
  "swatches": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-digital-assets`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-digital-assets.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.digital.assets.list`, `ecommerce.digital.assets.get`, `ecommerce.digital.assets.create`, `ecommerce.digital.assets.update`, and `ecommerce.digital.assets.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceDigitalAssetsResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `commerce_media`, `swatches`, `documents`, `manuals`, `rights`, `ordering`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce digital assets API
  version: 1.0.0
paths:
  /api/v1/ecommerce/digital-assets:
    get:
      operationId: ecommerce.digital.assets.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceDigitalAssetsResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-digital-assets
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceDigitalAssetsResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-digital-assets`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-digital-assets`.
