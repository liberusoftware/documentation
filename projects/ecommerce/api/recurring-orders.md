# Ecommerce: Recurring Orders API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-recurring-orders-api`  
**Matching domain module:** `ecommerce-recurring-orders`  
**Application:** Ecommerce  
**Source feature:** [Recurring Orders](../features/recurring-orders.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Recurring Orders domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Repeat schedules
- editable upcoming orders
- cutoff
- skips
- substitutions
- payment references
...
- Use `/api/v1/ecommerce/recurring-orders` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/recurring-orders`
...
| -------- | ------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/ecommerce/recurring-orders?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/ecommerce/recurring-orders/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/ecommerce/recurring-orders`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/ecommerce/recurring-orders/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/ecommerce/recurring-orders/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/ecommerce/recurring-orders/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "repeat_schedules": "example-value",
  "editable_upcoming_orders": "example-value",
  "cutoff": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-recurring-orders`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-recurring-orders.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.recurring.orders.list`, `ecommerce.recurring.orders.get`, `ecommerce.recurring.orders.create`, `ecommerce.recurring.orders.update`, and `ecommerce.recurring.orders.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceRecurringOrdersResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `repeat_schedules`, `editable_upcoming_orders`, `cutoff`, `skips`, `substitutions`, `payment_references`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce recurring orders API
  version: 1.0.0
paths:
  /api/v1/ecommerce/recurring-orders:
    get:
      operationId: ecommerce.recurring.orders.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceRecurringOrdersResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-recurring-orders
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/EcommerceRecurringOrdersResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-recurring-orders`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-recurring-orders`.
