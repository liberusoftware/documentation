# Ecommerce: Offline POS API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-offline-pos-api`  
**Matching domain module:** `ecommerce-offline-pos`  
**Application:** Ecommerce  
**Source feature:** [Offline POS](../features/offline-pos.md)  
**Architecture:** [API.md](../ECOMMERCE.md) · [MODULES.md](../ECOMMERCE.md) · [TESTING.md](../ECOMMERCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Offline POS domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Local catalog/price/stock subset
- offline orders/payments policy
- sequence IDs
- synchronization
- conflict resolution
- risk limits
...
- Use `/api/v1/ecommerce/offline-pos` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/offline-pos`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/offline-pos?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/offline-pos/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/offline-pos` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/offline-pos/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/offline-pos/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/offline-pos/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "local_catalog_price_stock_subset": "example-value",
  "offline_orders_payments_policy": "example-value",
  "sequence_ids": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-offline-pos`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-offline-pos.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.offline.pos.list`, `ecommerce.offline.pos.get`, `ecommerce.offline.pos.create`, `ecommerce.offline.pos.update`, and `ecommerce.offline.pos.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceOfflinePosResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `local_catalog_price_stock_subset`, `offline_orders_payments_policy`, `sequence_ids`, `synchronization`, `conflict_resolution`, `risk_limits`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce offline pos API
  version: 1.0.0
paths:
  /api/v1/ecommerce/offline-pos:
    get:
      operationId: ecommerce.offline.pos.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceOfflinePosResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-offline-pos`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-offline-pos`.
