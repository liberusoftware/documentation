# Ecommerce: Commerce Automation Pack API

## Canonical one-to-one API module specification

**API package:** `module-ecommerce-commerce-automation-pack-api`  
...
This optional API presentation package exposes approved HTTP operations for the Commerce Automation Pack domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Commerce triggers/actions/recipes
- versioning
- simulation
- approvals
- idempotency
...
- Use `/api/v1/ecommerce/commerce-automation-pack` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/ecommerce/commerce-automation-pack`
...
|---|---|---|---|
| `GET` | `/api/v1/ecommerce/commerce-automation-pack?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/ecommerce/commerce-automation-pack/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/ecommerce/commerce-automation-pack` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/ecommerce/commerce-automation-pack/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/ecommerce/commerce-automation-pack/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/ecommerce/commerce-automation-pack/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "commerce_triggers_actions_recipes": "example-value",
  "versioning": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `ecommerce-commerce-automation-pack`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/ecommerce-commerce-automation-pack.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `ecommerce.commerce.automation.pack.list`, `ecommerce.commerce.automation.pack.get`, `ecommerce.commerce.automation.pack.create`, `ecommerce.commerce.automation.pack.update`, and `ecommerce.commerce.automation.pack.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `EcommerceCommerceAutomationPackResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `commerce_triggers_actions_recipes`, `versioning`, `simulation`, `approvals`, `idempotency`, `generic_automation_integration`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: ecommerce commerce automation pack API
  version: 1.0.0
paths:
  /api/v1/ecommerce/commerce-automation-pack:
    get:
      operationId: ecommerce.commerce.automation.pack.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    EcommerceCommerceAutomationPackResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: ecommerce-commerce-automation-pack
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/EcommerceCommerceAutomationPackResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `ecommerce-commerce-automation-pack`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `ecommerce-commerce-automation-pack`.
