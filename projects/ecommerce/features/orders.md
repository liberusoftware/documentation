...
**Domain module:** `module-ecommerce-orders`
...
The Orders module owns Immutable product/price/tax/discount/address/terms snapshots, state machine, history, notes, documents, holds, and audit. It is the authoritative boundary for this capability and must remain independently installable, testable, versioned, enabled, and reusable across compatible Liberu applications.

## 2. Full feature scope

- **Immutable product/price/tax/discount/address/terms snapshots:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
- **State machine:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- Provide matching optional presentation packages only when required: `module-ecommerce-orders-filament`, `module-ecommerce-orders-livewire`, and `module-ecommerce-orders-api`.
...
- Verify immutable product/price/tax/discount/address/terms snapshots across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
- Verify state machine across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
