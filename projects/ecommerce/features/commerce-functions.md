...
**Domain module:** `module-ecommerce-commerce-functions`
...
The Commerce Functions module owns Sandboxed typed hooks for discounts, validation, delivery/payment customization, routing, and transformations with limits and diagnostics. It is the authoritative boundary for this capability and must remain independently installable, testable, versioned, enabled, and reusable across compatible Liberu applications.

## 2. Full feature scope

- **Sandboxed typed hooks for discounts:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- **Transformations with limits and diagnostics:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- Provide matching optional presentation packages only when required: `module-ecommerce-commerce-functions-filament`, `module-ecommerce-commerce-functions-livewire`, and `module-ecommerce-commerce-functions-api`.
...
- Verify sandboxed typed hooks for discounts across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
...
- Verify transformations with limits and diagnostics across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
