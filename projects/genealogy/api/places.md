# Genealogy: Places API

## Canonical one-to-one API module specification

**API package:** `module-genealogy-places-api`
**Matching domain module:** `genealogy-places`
**Application:** Genealogy
**Source feature:** [Places](../features/places.md)
**Architecture:** [API.md](../GENEALOGY.md) · [MODULES.md](../GENEALOGY.md) · [TESTING.md](../GENEALOGY.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Places domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Historical/current place hierarchy
- names over time
- coordinates
- jurisdictions
- maps
...
- Use `/api/v1/genealogy/places` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/genealogy/places`
...
| -------- | --------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/genealogy/places?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/genealogy/places/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/genealogy/places`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/genealogy/places/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/genealogy/places/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/genealogy/places/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

`json
{
  "historical_current_place_hierarchy": "example-value",
  "names_over_time": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `genealogy-places`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/genealogy-places.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `genealogy.places.list`, `genealogy.places.get`, `genealogy.places.create`, `genealogy.places.update`, and `genealogy.places.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `GenealogyPlacesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `historical_current_place_hierarchy`, `names_over_time`, `coordinates`, `jurisdictions`, `maps`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: genealogy places API
  version: 1.0.0
paths:
  /api/v1/genealogy/places:
    get:
      operationId: genealogy.places.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/GenealogyPlacesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `genealogy-places`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `genealogy-places`.
