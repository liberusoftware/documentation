# Genealogy: Timeline API

## Canonical one-to-one API module specification

**API package:** `module-genealogy-timeline-api`
**Matching domain module:** `genealogy-timeline`
**Application:** Genealogy
**Source feature:** [Timeline](../features/timeline.md)
**Architecture:** [API.md](../GENEALOGY.md) · [MODULES.md](../GENEALOGY.md) · [TESTING.md](../GENEALOGY.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Timeline domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/genealogy/timeline` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/genealogy/timeline`
...
| -------- | ----------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/genealogy/timeline?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/genealogy/timeline/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/genealogy/timeline`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/genealogy/timeline/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/genealogy/timeline/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/genealogy/timeline/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

`json
{
  "personal_family_timelines": "example-value",
  "historical_context": "example-value",
  "conflicts": "example-value"
}
`
...
This module owns a versioned OpenAPI 3.1 fragment for `genealogy-timeline`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/genealogy-timeline.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `genealogy.timeline.list`, `genealogy.timeline.get`, `genealogy.timeline.create`, `genealogy.timeline.update`, and `genealogy.timeline.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `GenealogyTimelineResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `personal_family_timelines`, `historical_context`, `conflicts`, `chronological_navigation`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: genealogy timeline API
  version: 1.0.0
paths:
  /api/v1/genealogy/timeline:
    get:
      operationId: genealogy.timeline.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/GenealogyTimelineResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `genealogy-timeline`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `genealogy-timeline`.
