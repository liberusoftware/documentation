...
**API package:** `module-genealogy-media-api`
...
This optional API presentation package exposes approved HTTP operations for the Media domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/genealogy/media` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/genealogy/media`
...
| -------- | -------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/genealogy/media?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/genealogy/media/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/genealogy/media`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/genealogy/media/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/genealogy/media/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/genealogy/media/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |
...
```json
{
  ...
  "photographs": "example-value",
  "audio_video": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `genealogy-media`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/genealogy-media.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `genealogy.media.list`, `genealogy.media.get`, `genealogy.media.create`, `genealogy.media.update`, and `genealogy.media.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `GenealogyMediaResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `documents`, `photographs`, `audio_video`, `transcription`, `rights`, `links`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: genealogy media API
  version: 1.0.0
paths:
  /api/v1/genealogy/media:
    get:
      operationId: genealogy.media.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/GenealogyMediaResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `genealogy-media`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `genealogy-media`.
