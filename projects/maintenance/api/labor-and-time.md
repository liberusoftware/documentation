# Maintenance: Labor and Time API

## Canonical one-to-one API module specification

**API package:** `module-maintenance-labor-and-time-api`
...
This optional API presentation package exposes approved HTTP operations for the Labor and Time domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/maintenance/labor-and-time` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/maintenance/labor-and-time`
...
| -------- | ------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/maintenance/labor-and-time?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/maintenance/labor-and-time/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/maintenance/labor-and-time`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/maintenance/labor-and-time/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/maintenance/labor-and-time/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/maintenance/labor-and-time/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "engineer_skills": "example-value",
  "attendance": "example-value",
  "time_entries": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `maintenance-labor-and-time`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/maintenance-labor-and-time.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `maintenance.labor.and.time.list`, `maintenance.labor.and.time.get`, `maintenance.labor.and.time.create`, `maintenance.labor.and.time.update`, and `maintenance.labor.and.time.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `MaintenanceLaborAndTimeResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `engineer_skills`, `attendance`, `time_entries`, `rates`, `expenses`, `approval`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: maintenance labor and time API
  version: 1.0.0
paths:
  /api/v1/maintenance/labor-and-time:
    get:
      operationId: maintenance.labor.and.time.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    MaintenanceLaborAndTimeResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: maintenance-labor-and-time
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/MaintenanceLaborAndTimeResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `maintenance-labor-and-time`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `maintenance-labor-and-time`.
