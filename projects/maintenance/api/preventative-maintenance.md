...
**API package:** `module-maintenance-preventative-maintenance-api`
...
- Use `/api/v1/maintenance/preventative-maintenance` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/maintenance/preventative-maintenance`
...
| -------- | ----------------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/maintenance/preventative-maintenance?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/maintenance/preventative-maintenance/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/maintenance/preventative-maintenance`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/maintenance/preventative-maintenance/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/maintenance/preventative-maintenance/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/maintenance/preventative-maintenance/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |
...
This module owns a versioned OpenAPI 3.1 fragment for `maintenance-preventative-maintenance`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/maintenance-preventative-maintenance.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `maintenance.preventative.maintenance.list`, `maintenance.preventative.maintenance.get`, `maintenance.preventative.maintenance.create`, `maintenance.preventative.maintenance.update`, and `maintenance.preventative.maintenance.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `MaintenancePreventativeMaintenanceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `plans`, `triggers`, `frequencies`, `meter_rules`, `forecasts`, `generation`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: maintenance preventative maintenance API
  version: 1.0.0
paths:
  /api/v1/maintenance/preventative-maintenance:
    get:
      operationId: maintenance.preventative.maintenance.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    MaintenancePreventativeMaintenanceResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: maintenance-preventative-maintenance
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/MaintenancePreventativeMaintenanceResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `maintenance-preventative-maintenance`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `maintenance-preventative-maintenance`.
