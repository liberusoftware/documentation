# Maintenance: Commercial API

## Canonical one-to-one API module specification

**API package:** `module-maintenance-commercial-api`
**Matching domain module:** `maintenance-commercial`
**Application:** Maintenance
**Source feature:** [Commercial](../features/commercial.md)
**Architecture:** [API.md](../MAINTENANCE.md) · [MODULES.md](../MAINTENANCE.md) · [TESTING.md](../MAINTENANCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Commercial domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/maintenance/commercial` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/maintenance/commercial`
...
| -------- | --------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/maintenance/commercial?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/maintenance/commercial/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/maintenance/commercial`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/maintenance/commercial/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/maintenance/commercial/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/maintenance/commercial/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |
...
```json
{
  ...
  "contracts": "example-value",
  "covered_assets_services": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `maintenance-commercial`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/maintenance-commercial.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `maintenance.commercial.list`, `maintenance.commercial.get`, `maintenance.commercial.create`, `maintenance.commercial.update`, and `maintenance.commercial.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `MaintenanceCommercialResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `quotes`, `contracts`, `covered_assets_services`, `rates`, `renewals`, `sla`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: maintenance commercial API
  version: 1.0.0
paths:
  /api/v1/maintenance/commercial:
    get:
      operationId: maintenance.commercial.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/MaintenanceCommercialResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `maintenance-commercial`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `maintenance-commercial`.
