# Maintenance: Procurement API

## Canonical one-to-one API module specification

**API package:** `module-maintenance-procurement-api`
**Matching domain module:** `maintenance-procurement`
**Application:** Maintenance
**Source feature:** [Procurement](../features/procurement.md)
**Architecture:** [API.md](../MAINTENANCE.md) · [MODULES.md](../MAINTENANCE.md) · [TESTING.md](../MAINTENANCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Procurement domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/maintenance/procurement` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/maintenance/procurement`
...
| -------- | ---------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/maintenance/procurement?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/maintenance/procurement/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/maintenance/procurement`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/maintenance/procurement/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/maintenance/procurement/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/maintenance/procurement/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

`json
{
  "suppliers": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `maintenance-procurement`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/maintenance-procurement.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `maintenance.procurement.list`, `maintenance.procurement.get`, `maintenance.procurement.create`, `maintenance.procurement.update`, and `maintenance.procurement.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `MaintenanceProcurementResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `suppliers`, `requests`, `approvals`, `purchase_orders`, `receipts`, `returns`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: maintenance procurement API
  version: 1.0.0
paths:
  /api/v1/maintenance/procurement:
    get:
      operationId: maintenance.procurement.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    MaintenanceProcurementResource:
      type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/MaintenanceProcurementResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `maintenance-procurement`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `maintenance-procurement`.
