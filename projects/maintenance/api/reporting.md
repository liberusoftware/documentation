# Maintenance: Reporting API

## Canonical one-to-one API module specification

**API package:** `module-maintenance-reporting-api`  
...
- Use `/api/v1/maintenance/reporting` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/maintenance/reporting`
...
|---|---|---|---|
| `GET` | `/api/v1/maintenance/reporting?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/maintenance/reporting/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/maintenance/reporting` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/maintenance/reporting/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/maintenance/reporting/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/maintenance/reporting/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "backlog": "example-value",
  "response": "example-value",
  "first_time_fix": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `maintenance-reporting`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/maintenance-reporting.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `maintenance.reporting.list`, `maintenance.reporting.get`, `maintenance.reporting.create`, `maintenance.reporting.update`, and `maintenance.reporting.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `MaintenanceReportingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `backlog`, `response`, `first_time_fix`, `downtime`, `cost`, `utilization`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
info:
  ...
  version: 1.0.0
paths:
  /api/v1/maintenance/reporting:
    get:
      operationId: maintenance.reporting.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/MaintenanceReportingResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `maintenance-reporting`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `maintenance-reporting`.
