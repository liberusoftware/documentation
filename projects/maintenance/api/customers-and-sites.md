# Maintenance: Customers and Sites API

## Canonical one-to-one API module specification

**API package:** `module-maintenance-customers-and-sites-api`  
...
**Architecture:** [API.md](../MAINTENANCE.md) · [MODULES.md](../MAINTENANCE.md) · [TESTING.md](../MAINTENANCE.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Customers and Sites domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/maintenance/customers-and-sites` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/maintenance/customers-and-sites`
...
|---|---|---|---|
| `GET` | `/api/v1/maintenance/customers-and-sites?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/maintenance/customers-and-sites/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/maintenance/customers-and-sites` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/maintenance/customers-and-sites/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/maintenance/customers-and-sites/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/maintenance/customers-and-sites/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "customers": "example-value",
  "contacts": "example-value",
  "sites": "example-value"
}
```
...
Successful reads and writes return the standard `data` envelope from [API.md](../MAINTENANCE.md). A create normally returns `201`, an update or query `200`, a successful delete `204`, and a queued or provider-dependent action `202` with an operation resource. Invalid, unauthorized, forbidden, conflicting, throttled, or unavailable requests use the documented HTTP status and RFC 9457 Problem Details shape.

## 4. OpenAPI schema

This module owns a versioned OpenAPI 3.1 fragment for `maintenance-customers-and-sites`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/maintenance-customers-and-sites.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `maintenance.customers.and.sites.list`, `maintenance.customers.and.sites.get`, `maintenance.customers.and.sites.create`, `maintenance.customers.and.sites.update`, and `maintenance.customers.and.sites.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `MaintenanceCustomersAndSitesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `customers`, `contacts`, `sites`, `locations`, `access_details`, `hazards`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
- Reuse the standard `data`, `links`, `meta`, and RFC 9457 Problem Details shapes from [API.md](../MAINTENANCE.md); do not serialize Eloquent models automatically.
...
openapi: 3.1.0
info:
  title: maintenance customers and sites API
  version: 1.0.0
paths:
  /api/v1/maintenance/customers-and-sites:
    get:
      operationId: maintenance.customers.and.sites.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    MaintenanceCustomersAndSitesResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: maintenance-customers-and-sites
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/MaintenanceCustomersAndSitesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `maintenance-customers-and-sites`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `maintenance-customers-and-sites`.
