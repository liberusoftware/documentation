...
**Domain module:** `module-maintenance-portals`
...
The Portals module owns Customer requests/approvals/status and engineer schedule/work/offline capture. It is the authoritative boundary for this capability and must remain independently installable, testable, versioned, enabled, and reusable across compatible Liberu applications.

## 2. Full feature scope

- **Customer requests/approvals/status and engineer schedule/work/offline capture:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- Provide matching optional presentation packages only when required: `module-maintenance-portals-filament`, `module-maintenance-portals-livewire`, and `module-maintenance-portals-api`.
...
- Verify customer requests/approvals/status and engineer schedule/work/offline capture across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
