...
The Maintenance Core module owns Organizations, service settings, statuses, numbering, priorities, and shared events. It is the authoritative boundary for this capability and must remain independently installable, testable, versioned, enabled, and reusable across compatible Liberu applications.
...
- **Service settings:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- Provide matching optional presentation packages only when required: `module-maintenance-core-filament`, `module-maintenance-core-livewire`, and `module-maintenance-core-api`.
...
- Verify service settings across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
