# SAP: Data and Intelligence API

## Canonical one-to-one API module specification

**API package:** `module-sap-data-and-intelligence-api`
...
This optional API presentation package exposes approved HTTP operations for the Data and Intelligence domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/sap/data-and-intelligence` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/sap/data-and-intelligence`
...
| -------- | ------------------------------------------------------------------ | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/sap/data-and-intelligence?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/sap/data-and-intelligence/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/sap/data-and-intelligence`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/sap/data-and-intelligence/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/sap/data-and-intelligence/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/sap/data-and-intelligence/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |
...
This module owns a versioned OpenAPI 3.1 fragment for `sap-data-and-intelligence`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/sap-data-and-intelligence.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `sap.data.and.intelligence.list`, `sap.data.and.intelligence.get`, `sap.data.and.intelligence.create`, `sap.data.and.intelligence.update`, and `sap.data.and.intelligence.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `SapDataAndIntelligenceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `governed_metrics`, `semantic_models`, `reports`, `planning`, `forecasts`, `data_quality`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: sap data and intelligence API
  version: 1.0.0
paths:
  /api/v1/sap/data-and-intelligence:
    get:
      operationId: sap.data.and.intelligence.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    SapDataAndIntelligenceResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: sap-data-and-intelligence
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/SapDataAndIntelligenceResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `sap-data-and-intelligence`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `sap-data-and-intelligence`.
