# Social Network: Communities API

## Canonical one-to-one API module specification

**API package:** `module-social-network-communities-api`
...
This optional API presentation package exposes approved HTTP operations for the Communities domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/social-network/communities` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/social-network/communities`
...
| -------- | ------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/social-network/communities?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/social-network/communities/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/social-network/communities`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/social-network/communities/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/social-network/communities/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/social-network/communities/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

`json
{
  "groups_pages": "example-value",
  "memberships": "example-value",
  "roles": "example-value"
}
`
...
This module owns a versioned OpenAPI 3.1 fragment for `social-network-communities`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/social-network-communities.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `social.network.communities.list`, `social.network.communities.get`, `social.network.communities.create`, `social.network.communities.update`, and `social.network.communities.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `SocialNetworkCommunitiesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `groups_pages`, `memberships`, `roles`, `rules`, `feeds`, `files`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: social network communities API
  version: 1.0.0
paths:
  /api/v1/social-network/communities:
    get:
      operationId: social.network.communities.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    SocialNetworkCommunitiesResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: social-network-communities
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/SocialNetworkCommunitiesResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `social-network-communities`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `social-network-communities`.
