...
**Domain module:** `module-social-network-federation`
...
The Federation module owns ActivityPub identities, inbox/outbox, signatures, delivery, mapping, moderation, and reconciliation. It is the authoritative boundary for this capability and must remain independently installable, testable, versioned, enabled, and reusable across compatible Liberu applications.

## 2. Full feature scope

- **ActivityPub identities:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
- **Inbox/outbox:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- Provide matching optional presentation packages only when required: `module-social-network-federation-filament`, `module-social-network-federation-livewire`, and `module-social-network-federation-api`.
...
- Verify activityPub identities across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
- Verify inbox/outbox across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
