# CMS: Views and Query Builder API

## Canonical one-to-one API module specification

**API package:** `module-cms-views-and-query-builder-api`  
**Matching domain module:** `cms-views-and-query-builder`  
**Application:** CMS  
**Source feature:** [Views and Query Builder](../features/views-and-query-builder.md)  
**Architecture:** [API.md](../CMS.md) · [MODULES.md](../CMS.md) · [TESTING.md](../CMS.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Views and Query Builder domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Permission-safe listings from content/fields/relationships with filters
- sorting
- grouping
- exposed filters
- pagination
...
- Use `/api/v1/cms/views-and-query-builder` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/views-and-query-builder`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/views-and-query-builder?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/views-and-query-builder/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/views-and-query-builder` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/views-and-query-builder/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/views-and-query-builder/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/views-and-query-builder/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "permission_safe_listings_from_content_fields_relationships_with_filters": "example-value",
  "sorting": "example-value",
  "grouping": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-views-and-query-builder`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-views-and-query-builder.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.views.and.query.builder.list`, `cms.views.and.query.builder.get`, `cms.views.and.query.builder.create`, `cms.views.and.query.builder.update`, and `cms.views.and.query.builder.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsViewsAndQueryBuilderResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `permission_safe_listings_from_content_fields_relationships_with_filters`, `sorting`, `grouping`, `exposed_filters`, `pagination`, `feeds`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms views and query builder API
  version: 1.0.0
paths:
  /api/v1/cms/views-and-query-builder:
    get:
      operationId: cms.views.and.query.builder.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CmsViewsAndQueryBuilderResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: cms-views-and-query-builder
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsViewsAndQueryBuilderResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-views-and-query-builder`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-views-and-query-builder`.
