...
**API package:** `module-cms-image-processing-api`  
**Matching domain module:** `cms-image-processing`  
**Application:** CMS  
**Source feature:** [Image Processing](../features/image-processing.md)  
...
- Use `/api/v1/cms/image-processing` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/image-processing`
...
| -------- | ------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/cms/image-processing?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/cms/image-processing/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/cms/image-processing`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/cms/image-processing/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/cms/image-processing/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/cms/image-processing/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |
...
```json
{
  ...
  "orientation": "example-value",
  "crops_focal_points": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-image-processing`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-image-processing.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.image.processing.list`, `cms.image.processing.get`, `cms.image.processing.create`, `cms.image.processing.update`, and `cms.image.processing.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsImageProcessingResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `validation`, `orientation`, `crops_focal_points`, `responsive_variants`, `format_conversion`, `quality`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms image processing API
  version: 1.0.0
paths:
  /api/v1/cms/image-processing:
    get:
      operationId: cms.image.processing.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/CmsImageProcessingResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-image-processing`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-image-processing`.
