...
**API package:** `module-cms-media-library-api`  
**Matching domain module:** `cms-media-library`  
**Application:** CMS  
**Source feature:** [Media Library](../features/media-library.md)  
**Architecture:** [API.md](../CMS.md) · [MODULES.md](../CMS.md) · [TESTING.md](../CMS.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Media Library domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Files/media entities
- folders
- tags
- collections
- metadata
...
- Use `/api/v1/cms/media-library` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/media-library`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/media-library?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/media-library/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/media-library` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/media-library/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/media-library/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/media-library/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "files_media_entities": "example-value",
  "folders": "example-value",
  "tags": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-media-library`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-media-library.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.media.library.list`, `cms.media.library.get`, `cms.media.library.create`, `cms.media.library.update`, and `cms.media.library.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsMediaLibraryResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `files_media_entities`, `folders`, `tags`, `collections`, `metadata`, `usage_references`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms media library API
  version: 1.0.0
paths:
  /api/v1/cms/media-library:
    get:
      operationId: cms.media.library.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsMediaLibraryResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-media-library`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-media-library`.
