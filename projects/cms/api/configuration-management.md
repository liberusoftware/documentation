# CMS: Configuration Management API

## Canonical one-to-one API module specification

**API package:** `module-cms-configuration-management-api`  
**Matching domain module:** `cms-configuration-management`  
**Application:** CMS  
**Source feature:** [Configuration Management](../features/configuration-management.md)  
**Architecture:** [API.md](../CMS.md) · [MODULES.md](../CMS.md) · [TESTING.md](../CMS.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Configuration Management domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Exportable versioned CMS configuration
...
- Use `/api/v1/cms/configuration-management` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/configuration-management`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/configuration-management?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/configuration-management/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/configuration-management` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/configuration-management/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/configuration-management/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/configuration-management/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "exportable_versioned_cms_configuration": "example-value",
  "environment_comparison": "example-value",
  "dependency_validation": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-configuration-management`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-configuration-management.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.configuration.management.list`, `cms.configuration.management.get`, `cms.configuration.management.create`, `cms.configuration.management.update`, and `cms.configuration.management.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsConfigurationManagementResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `exportable_versioned_cms_configuration`, `environment_comparison`, `dependency_validation`, `promotion`, `secrets_exclusion`, `rollback`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms configuration management API
  version: 1.0.0
paths:
  /api/v1/cms/configuration-management:
    get:
      operationId: cms.configuration.management.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    CmsConfigurationManagementResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: cms-configuration-management
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsConfigurationManagementResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-configuration-management`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-configuration-management`.
