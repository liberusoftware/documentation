# CMS: Layout Builder API

## Canonical one-to-one API module specification

**API package:** `module-cms-layout-builder-api`  
**Matching domain module:** `cms-layout-builder`  
**Application:** CMS  
**Source feature:** [Layout Builder](../features/layout-builder.md)  
**Architecture:** [API.md](../CMS.md) · [MODULES.md](../CMS.md) · [TESTING.md](../CMS.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Layout Builder domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/cms/layout-builder` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/layout-builder`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/layout-builder?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/layout-builder/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/layout-builder` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/layout-builder/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/layout-builder/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/layout-builder/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-layout-builder`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-layout-builder.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.layout.builder.list`, `cms.layout.builder.get`, `cms.layout.builder.create`, `cms.layout.builder.update`, and `cms.layout.builder.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsLayoutBuilderResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `regions`, `grids`, `sections`, `drag_drop_composition`, `type_defaults`, `per_record_overrides`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms layout builder API
  version: 1.0.0
paths:
  /api/v1/cms/layout-builder:
    get:
      operationId: cms.layout.builder.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsLayoutBuilderResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-layout-builder`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-layout-builder`.
