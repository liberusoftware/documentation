# CMS: Web Delivery API

## Canonical one-to-one API module specification

**API package:** `module-cms-web-delivery-api`  
...
This optional API presentation package exposes approved HTTP operations for the Web Delivery domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Canonical route rendering
- response metadata
- cache tags
- preview mode
- redirects
...
- Use `/api/v1/cms/web-delivery` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/web-delivery`
...
| -------- | --------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/cms/web-delivery?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/cms/web-delivery/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/cms/web-delivery`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/cms/web-delivery/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/cms/web-delivery/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/cms/web-delivery/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "canonical_route_rendering": "example-value",
  "response_metadata": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-web-delivery`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-web-delivery.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.web.delivery.list`, `cms.web.delivery.get`, `cms.web.delivery.create`, `cms.web.delivery.update`, and `cms.web.delivery.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsWebDeliveryResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `canonical_route_rendering`, `response_metadata`, `cache_tags`, `preview_mode`, `redirects`, `errors`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms web delivery API
  version: 1.0.0
paths:
  /api/v1/cms/web-delivery:
    get:
      operationId: cms.web.delivery.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/CmsWebDeliveryResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-web-delivery`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-web-delivery`.
