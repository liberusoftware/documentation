...
**API package:** `module-cms-form-builder-api`  
**Matching domain module:** `cms-form-builder`  
**Application:** CMS  
**Source feature:** [Form Builder](../features/form-builder.md)  
**Architecture:** [API.md](../CMS.md) · [MODULES.md](../CMS.md) · [TESTING.md](../CMS.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Form Builder domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Typed/conditional multi-step forms
- reusable fields
- uploads
- validation
- calculations
...
- Use `/api/v1/cms/form-builder` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/form-builder`
...
|---|---|---|---|
| `GET` | `/api/v1/cms/form-builder?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/cms/form-builder/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/cms/form-builder` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/cms/form-builder/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/cms/form-builder/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/cms/form-builder/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "typed_conditional_multi_step_forms": "example-value",
  "reusable_fields": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-form-builder`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-form-builder.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.form.builder.list`, `cms.form.builder.get`, `cms.form.builder.create`, `cms.form.builder.update`, and `cms.form.builder.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsFormBuilderResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `typed_conditional_multi_step_forms`, `reusable_fields`, `uploads`, `validation`, `calculations`, `drafts`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms form builder API
  version: 1.0.0
paths:
  /api/v1/cms/form-builder:
    get:
      operationId: cms.form.builder.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/CmsFormBuilderResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-form-builder`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-form-builder`.
