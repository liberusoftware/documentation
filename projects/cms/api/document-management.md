...
**API package:** `module-cms-document-management-api`  
**Matching domain module:** `cms-document-management`  
**Application:** CMS  
**Source feature:** [Document Management](../features/document-management.md)  
**Architecture:** [API.md](../CMS.md) · [MODULES.md](../CMS.md) · [TESTING.md](../CMS.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Document Management domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/cms/document-management` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/document-management`
...
| -------- | ---------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/cms/document-management?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/cms/document-management/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/cms/document-management`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/cms/document-management/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/cms/document-management/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/cms/document-management/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |
...
```json
{
  ...
  "ocr_text_extraction_adapters": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-document-management`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-document-management.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.document.management.list`, `cms.document.management.get`, `cms.document.management.create`, `cms.document.management.update`, and `cms.document.management.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsDocumentManagementResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `documents`, `previews`, `ocr_text_extraction_adapters`, `versions`, `access`, `downloads`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms document management API
  version: 1.0.0
paths:
  /api/v1/cms/document-management:
    get:
      operationId: cms.document.management.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/CmsDocumentManagementResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-document-management`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-document-management`.
