# CMS: Content Templates API

## Canonical one-to-one API module specification

**API package:** `module-cms-content-templates-api`  
**Matching domain module:** `cms-content-templates`  
**Application:** CMS  
**Source feature:** [Content Templates](../features/content-templates.md)  
**Architecture:** [API.md](../CMS.md) · [MODULES.md](../CMS.md) · [TESTING.md](../CMS.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Content Templates domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.

The domain capability includes:

- Starter content
- presets
- default fields/blocks/layouts
- template locking
- versioning
- controlled rollout
...
- Use `/api/v1/cms/content-templates` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/content-templates`
...
| -------- | -------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/cms/content-templates?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/cms/content-templates/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/cms/content-templates`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/cms/content-templates/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/cms/content-templates/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/cms/content-templates/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "starter_content": "example-value",
  "presets": "example-value",
  "default_fields_blocks_layouts": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-content-templates`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-content-templates.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.content.templates.list`, `cms.content.templates.get`, `cms.content.templates.create`, `cms.content.templates.update`, and `cms.content.templates.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsContentTemplatesResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `starter_content`, `presets`, `default_fields_blocks_layouts`, `template_locking`, `versioning`, `controlled_rollout`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms content templates API
  version: 1.0.0
paths:
  /api/v1/cms/content-templates:
    get:
      operationId: cms.content.templates.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/CmsContentTemplatesResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-content-templates`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-content-templates`.
