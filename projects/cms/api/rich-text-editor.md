# CMS: Rich Text Editor API

## Canonical one-to-one API module specification

**API package:** `module-cms-rich-text-editor-api`  
...
This optional API presentation package exposes approved HTTP operations for the Rich Text Editor domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/cms/rich-text-editor` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/cms/rich-text-editor`
...
| -------- | ------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/cms/rich-text-editor?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/cms/rich-text-editor/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/cms/rich-text-editor`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/cms/rich-text-editor/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/cms/rich-text-editor/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/cms/rich-text-editor/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "semantic_rich_text": "example-value",
  "links": "example-value",
...
This module owns a versioned OpenAPI 3.1 fragment for `cms-rich-text-editor`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/cms-rich-text-editor.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `cms.rich.text.editor.list`, `cms.rich.text.editor.get`, `cms.rich.text.editor.create`, `cms.rich.text.editor.update`, and `cms.rich.text.editor.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `CmsRichTextEditorResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `semantic_rich_text`, `links`, `tables`, `embeds`, `code`, `paste_cleanup`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: cms rich text editor API
  version: 1.0.0
paths:
  /api/v1/cms/rich-text-editor:
    get:
      operationId: cms.rich.text.editor.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/CmsRichTextEditorResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `cms-rich-text-editor`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `cms-rich-text-editor`.
