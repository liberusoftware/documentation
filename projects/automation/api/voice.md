# Automation: Voice API

## Canonical one-to-one API module specification

**API package:** `module-automation-voice-api`
**Matching domain module:** `automation-voice`
**Application:** Automation
**Source feature:** [Voice](../features/voice.md)
**Architecture:** [API.md](../AUTOMATION.md) · [MODULES.md](../AUTOMATION.md) · [TESTING.md](../AUTOMATION.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Voice domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/automation/voice` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/automation/voice`
...
| -------- | --------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/automation/voice?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/automation/voice/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/automation/voice`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/automation/voice/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/automation/voice/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/automation/voice/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "speech_to_text": "example-value",
  "text_to_speech": "example-value",
  "streaming_sessions": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `automation-voice`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/automation-voice.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `automation.voice.list`, `automation.voice.get`, `automation.voice.create`, `automation.voice.update`, and `automation.voice.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AutomationVoiceResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `speech_to_text`, `text_to_speech`, `streaming_sessions`, `interruption`, `transcripts`, `consent_controls`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: automation voice API
  version: 1.0.0
paths:
  /api/v1/automation/voice:
    get:
      operationId: automation.voice.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/AutomationVoiceResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `automation-voice`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `automation-voice`.
