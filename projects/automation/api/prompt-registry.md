# Automation: Prompt Registry API

## Canonical one-to-one API module specification

**API package:** `module-automation-prompt-registry-api`  
**Matching domain module:** `automation-prompt-registry`  
**Application:** Automation  
**Source feature:** [Prompt Registry](../features/prompt-registry.md)  
**Architecture:** [API.md](../AUTOMATION.md) · [MODULES.md](../AUTOMATION.md) · [TESTING.md](../AUTOMATION.md)

## 1. Purpose and ownership

This optional API presentation package exposes approved HTTP operations for the Prompt Registry domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/automation/prompt-registry` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/automation/prompt-registry`
...
|---|---|---|---|
| `GET` | `/api/v1/automation/prompt-registry?page[size]=25&sort=-created_at` | List authorized resources | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET` | `/api/v1/automation/prompt-registry/{id}` | Retrieve one resource | Opaque resource `id` in the path; no request body |
| `POST` | `/api/v1/automation/prompt-registry` | Create a resource | JSON body using the module schema and required team/context fields |
| `PATCH` | `/api/v1/automation/prompt-registry/{id}` | Update permitted fields | JSON body containing changed fields and `If-Match` when concurrency is supported |
| `DELETE` | `/api/v1/automation/prompt-registry/{id}` | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted |
| `POST` | `/api/v1/automation/prompt-registry/{id}/<explicit-action>` | Execute a documented domain action | Action-specific JSON body and `Idempotency-Key` for retryable writes |

Example create request (illustrative fields only):

```json
{
  "versioned_prompts": "example-value",
  "variables": "example-value",
  "evaluation_sets": "example-value"
}
```
...
Successful reads and writes return the standard `data` envelope from [API.md](../AUTOMATION.md). A create normally returns `201`, an update or query `200`, a successful delete `204`, and a queued or provider-dependent action `202` with an operation resource. Invalid, unauthorized, forbidden, conflicting, throttled, or unavailable requests use the documented HTTP status and RFC 9457 Problem Details shape.

## 4. OpenAPI schema

This module owns a versioned OpenAPI 3.1 fragment for `automation-prompt-registry`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/automation-prompt-registry.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `automation.prompt.registry.list`, `automation.prompt.registry.get`, `automation.prompt.registry.create`, `automation.prompt.registry.update`, and `automation.prompt.registry.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AutomationPromptRegistryResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `versioned_prompts`, `variables`, `evaluation_sets`, `brand_tenant_overrides`, `approvals`, `rollback`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
- Reuse the standard `data`, `links`, `meta`, and RFC 9457 Problem Details shapes from [API.md](../AUTOMATION.md); do not serialize Eloquent models automatically.
...
openapi: 3.1.0
info:
  title: automation prompt registry API
  version: 1.0.0
paths:
  /api/v1/automation/prompt-registry:
    get:
      operationId: automation.prompt.registry.list
      security:
        - sanctum: []
      parameters:
...
components:
  ...
  schemas:
    AutomationPromptRegistryResource:
      type: object
      ...
      properties:
        ...
        type:
          type: string
          const: automation-prompt-registry
        attributes:
          type: object
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: '#/components/schemas/AutomationPromptRegistryResource'
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `automation-prompt-registry`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `automation-prompt-registry`.
