# Automation: AI Gateway API

## Canonical one-to-one API module specification

**API package:** `module-automation-ai-gateway-api`
...
This optional API presentation package exposes approved HTTP operations for the AI Gateway domain module. It presents exactly one independent module, delegates all authoritative behavior to that module's public actions/queries/policies, and contains no other module's API logic.
...
- Use `/api/v1/automation/ai-gateway` as the default module route prefix unless the application owns and documents a compatible façade path.
...
**Base path:** `/api/v1/automation/ai-gateway`
...
| -------- | -------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `GET`    | `/api/v1/automation/ai-gateway?page[size]=25&sort=-created_at` | List authorized resources                     | Query parameters for pagination, filtering, sorting, field selection, and documented includes |
| `GET`    | `/api/v1/automation/ai-gateway/{id}`                           | Retrieve one resource                         | Opaque resource `id` in the path; no request body                                             |
| `POST`   | `/api/v1/automation/ai-gateway`                                | Create a resource                             | JSON body using the module schema and required team/context fields                            |
| `PATCH`  | `/api/v1/automation/ai-gateway/{id}`                           | Update permitted fields                       | JSON body containing changed fields and `If-Match` when concurrency is supported              |
| `DELETE` | `/api/v1/automation/ai-gateway/{id}`                           | Delete, archive, or deactivate when supported | Usually no body; use the documented lifecycle action when deletion is not permitted           |
| `POST`   | `/api/v1/automation/ai-gateway/{id}/<explicit-action>`         | Execute a documented domain action            | Action-specific JSON body and `Idempotency-Key` for retryable writes                          |

Example create request (illustrative fields only):

```json
{
  "provider_contracts": "example-value",
  "model_catalog": "example-value",
  "routing": "example-value"
}
```
...
This module owns a versioned OpenAPI 3.1 fragment for `automation-ai-gateway`. Keep the fragment at the repository's declared OpenAPI path, normally `openapi/v1/automation-ai-gateway.yaml`, and aggregate it only through the host application's API manifest. The fragment must document the base path, operation IDs, security requirements, parameters, request bodies, response envelopes, reusable schemas, errors, pagination, idempotency, concurrency, and deprecation metadata.

### Required schema elements

- Use stable operation IDs such as `automation.ai.gateway.list`, `automation.ai.gateway.get`, `automation.ai.gateway.create`, `automation.ai.gateway.update`, and `automation.ai.gateway.delete`; use an explicit domain action ID when applicable.
- Define the module's resource schema as `AutomationAiGatewayResource` with opaque `id`, stable `type`, field classification, relationships, state, timestamps, and only authorized attributes.
- Document possible module fields including `provider_contracts`, `model_catalog`, `routing`, `fallback`, `structured_output`, `tool_policy`; each field must state its type, required/nullable behavior, validation, example, sensitivity, and read/write authorization.
...
openapi: 3.1.0
info:
  title: automation ai gateway API
  version: 1.0.0
paths:
  /api/v1/automation/ai-gateway:
    get:
      operationId: automation.ai.gateway.list
      security:
        - sanctum: []
      parameters:
    ...
    ResourceCollection:
      ...
      properties:
        data:
          type: array
          items:
            $ref: "#/components/schemas/AutomationAiGatewayResource"
  responses:
    ResourceCollection:
...
- Add one-to-one architecture tests proving this package depends on and presents only `automation-ai-gateway`.
...
- The package name, manifest, Composer dependency, namespace, route prefix, and OpenAPI ownership all match `automation-ai-gateway`.
