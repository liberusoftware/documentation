# Automation: Prompt Registry

## Canonical independent module specification

**Domain module:** `module-automation-prompt-registry`
...
The Prompt Registry module owns Versioned prompts, variables, evaluation sets, brand/tenant overrides, approvals, and rollback. It is the authoritative boundary for this capability and must remain independently installable, testable, versioned, enabled, and reusable across compatible Liberu applications.

## 2. Full feature scope

- **Versioned prompts:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- **Evaluation sets:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
- **Brand/tenant overrides:** implement the complete lifecycle, validation, permissions, failure handling, audit evidence, and operator/user feedback required by this capability.
...
- Provide matching optional presentation packages only when required: `module-automation-prompt-registry-filament`, `module-automation-prompt-registry-livewire`, and `module-automation-prompt-registry-api`.
...
- Verify versioned prompts across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
...
- Verify evaluation sets across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
- Verify brand/tenant overrides across allowed, denied, invalid, duplicate, concurrent, partial-failure, recovery, and tenant-isolation paths where applicable.
