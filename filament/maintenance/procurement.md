...
**Filament package:** `module-maintenance-procurement-filament`  
...
This optional Filament 5 presentation package presents exactly one independent domain module. It contributes reusable resources, pages, widgets, schemas, tables, infolists, and actions to application-owned panels while delegating authorization, validation, tenancy, persistence, and business rules to the `maintenance-procurement` public boundary. It must not contain another module's UI or depend on application `App\` classes.
...
- Register a stable `module-maintenance-procurement-filament` plugin and discover only classes in this package's namespace; applications attach it explicitly to each eligible panel.
...
- Package identity, namespace, plugin ID, and dependency match `maintenance-procurement` one-to-one.
