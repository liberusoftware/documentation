...
Every push to `main` runs [CI](../standards/CI.md) and may build, scan, and publish an immutable staging image. Do not deploy that push directly to production. A protected `vX.Y.Z` tag or GitHub Release can promote the exact tested image digest only after all required checks, the 100% release-scope coverage gate, image scanning, smoke tests, and production environment approval pass.
